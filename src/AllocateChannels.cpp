...
// Allocate a channel interval that encompasses startTime and
// endTime.
// @param startTime is the first instant sound is to be played on
// the channel.
// @param endTime is the last such instant.
// @author Tom Breton (Tehom)
ChannelInterval FreeChannels::allocateChannelInterval(
    RealTime startTime, RealTime endTime, Instrument *instrument,
  ...
  // leastDuration's overflow bit.  See comments on thisOverflow
  // and thisDuration.
  ...
  if( !empty() ) {
    ...
    for( reverse_iterator i( upper_bound( dummy ) ); i != rend();
         ++i ) {
  ...
  if( bestMatch != end() ) {
    return allocateChannelIntervalFrom(
        bestMatch, startTime, endTime, instrument, marginBefore,
        marginAfter );
  } else {
...
// Free the given channel interval, setting old's channel to
// unused.
// @author Tom Breton (Tehom)
void FreeChannels::freeChannelInterval( ChannelInterval &old ) {
  if( !old.validChannel() ) { return; }
  // We are sometimes asked to free a zero-length interval.  If
  // so, do nothing.
  ...
  iterator atOrAfter =
      lower_bound( ChannelInterval( old.m_start ) );
  ...
  const ChannelInterval newChannelInterval(
      old.getChannelId(), ciBefore.m_start, ciAfter.m_end,
  ...
  // Add a channelsegment incorporating the whole contiguous
  // time.
...
// @param i an iterator indexing a ChannelInterval that includes
// the interval from start to end.
// @param start is the first instant sound is to be played on the
// channel.
// @param end is the last such instant.
// @returns A ChannelInterval, either a suitable one or
// non-playing.
// @author Tom Breton (Tehom)
ChannelInterval FreeChannels::allocateChannelIntervalFrom(
    iterator i, RealTime start, RealTime end,
    Instrument *instrument, RealTime marginBefore,
    RealTime marginAfter ) {
  const ChannelInterval cs = ( *i );

  erase( i );
  if( cs.m_start < start ) {
    // There's some length before `start'.  Insert a new piece.
    // (We can't alter it in place because the base class owns it
    // and makes it const)
    (void)insert( ChannelInterval( cs.getChannelId(), cs.m_start,
                                   start, cs.m_instrumentBefore,
                                   instrument, cs.m_marginBefore,
                                   marginBefore ) );
  } else {
  }
  ...
  if( cs.m_end > end ) {
    ...
    (void)insert( ChannelInterval(
        cs.getChannelId(), end, cs.m_end, instrument,
        cs.m_instrumentAfter, marginAfter, cs.m_marginAfter ) );
  } else {
  }

  return ChannelInterval( cs.getChannelId(), start, end, nullptr,
                          nullptr, RealTime::zeroTime,
                          RealTime::zeroTime );
}
...
// intervals.
// @author Tom Breton (Tehom)
void FreeChannels::removeChannel( ChannelId channelNb ) {
  // Filter the channels.  We build a temporary container, put
  // everything that stays into that container, then swap
  // contents.
  container els;
...
// @author Tom Breton (Tehom)
void AllocateChannels::reallocateToFit(
    Instrument &instrument, ChannelInterval &ci, RealTime start,
    RealTime end, RealTime marginBefore, RealTime marginAfter,
    bool changedInstrument ) {
  // If we already have a channel but it's the wrong type or it
  // changed instrument, always free it.
  if( ci.validChannel() &&
      ( ( changedInstrument &&
          ( end != ChannelInterval::m_latestTime ) ) ||
  ...
  if( instrument.isPercussion() ) {
    ...
    m_freeChannels.reallocateToFit( ci, start, end, &instrument,
                                    marginBefore, marginAfter );
  }
}
...
// @author Tom Breton (Tehom)
void AllocateChannels::freeChannelInterval(
    ChannelInterval &old ) {
  if( isPercussion( old ) ) {
    old.clearChannelId();
  } else {
...
// @author Tom Breton (Tehom)
void AllocateChannels::reserveFixedChannel( ChannelId channel ) {
  reserveChannel( channel, m_fixedChannels );

  // If m_thruChannels has it, release it from there too.  Fixed
  // channels have priority over thru channels because their
  // channel numbers are chosen by the user.  We do this last to
  // be very sure we're in a nice clean state when ControlBlock
  // looks for another thru channel.
...
ChannelId AllocateChannels::reallocateThruChannel(
  ...
  if( channel >= 0 ) {
    ...
    if( isInstrumentPercussion == isChannelPercussion ) {
      return channel;
    }
  }
...
// The signal connections this uses are made by ChannelManager.
// @author Tom Breton (Tehom)
ChannelId AllocateChannels::allocateThruChannel(
    Instrument &instrument ) {
  if( instrument.isPercussion() ) {
    return getPercussionChannel();
  }
  ...
  for( int channel = 15; channel >= 0; --channel ) {
    ...
    if( m_thruChannels.find( channel ) !=
        m_thruChannels.end() ) {
      continue;
    }

    // Avoid any channel in m_fixedChannels.
    if( m_fixedChannels.find( channel ) !=
        m_fixedChannels.end() ) {
      continue;
    }
  ...
  // Fixed channels has all the channels, so return invalid
  // channel.
  return -1;
}
...
// @author Tom Breton (Tehom)
void AllocateChannels::releaseReservedChannel(
...
// @author Tom Breton (Tehom)
void AllocateChannels::reserveChannel(
    ChannelId channel, FixedChannelSet &channelSet ) {
