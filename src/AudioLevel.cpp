...
struct FaderDescription {
  FaderDescription( float i_minDb, float i_maxDb,
                    float i_zeroPoint )
    : minDb( i_minDb ),
      maxDb( i_maxDb ),
...
static float iec_dB_to_fader( float db ) {
  ...
  if( db < -70.0f ) {
    ...
    def = ( db + 60.0f ) * 0.5f +
          2.5f; // corrected from 5.0f base, thanks Robin Gareus
  } else if( db < -40.0f ) {
...
static float iec_fader_to_dB(
    float def ) // Meter deflection %age
{
  float db = 0.0f;
...
float AudioLevel::fader_to_dB( int level, int maxLevel,
                               FaderType type ) {
  ...
  if( type == IEC268Meter || type == IEC268LongMeter ) {
    ...
    float percent =
        float( level ) * maxPercent / float( maxLevel );
    ...
    if( level >= zeroLevel ) {
      ...
      float scale =
          zeroLevel / sqrtf( 0.0 - faderTypes[type].minDb );
      value /= scale;
      float dB = powf( value, 2.0 );
      return 0.0 - dB;
    }
...
int AudioLevel::dB_to_fader( float dB, int maxLevel,
                             FaderType type ) {
  if( dB == DB_FLOOR ) return 0;

  if( type == IEC268Meter || type == IEC268LongMeter ) {
    // The IEC scale gives a "percentage travel" for a given dB
    // level, but it reaches 100% at 0dB.  So we want to treat
    // the result not as a percentage, but as a scale between 0
    // and whatever the "percentage" for our (possibly >0dB) max
    // dB is.

    float maxPercent = iec_dB_to_fader( faderTypes[type].maxDb );
    float percent    = iec_dB_to_fader( dB );
    int   faderLevel =
        int( ( maxLevel * percent ) / maxPercent + 0.01 );
    ...
    if( dB >= 0.0 ) {
      ...
      float scale = ( maxLevel - zeroLevel ) /
                    sqrtf( faderTypes[type].maxDb );
      value *= scale;
...
float AudioLevel::fader_to_multiplier( int level, int maxLevel,
                                       FaderType type ) {
  if( level == 0 ) return 0.0;
  return dB_to_multiplier(
      fader_to_dB( level, maxLevel, type ) );
}
...
const LevelList &getPreviewLevelCache( int levels ) {
...
int AudioLevel::multiplier_to_preview( float m, int levels ) {
...
float AudioLevel::panGainLeft(
    float pan ) // Apply panning law to left channel
{
  if( m_panLaw == 3 ) {
    // -3dB Panning Law (variant)
    //
    // This law has the same characteristics as the -3dB law
    // described below, except that a channel's gain begins at
    // +3dB and decreases to 0dB as the control is moved through
    // the center position.  This setting must be used with
    // caution, as the increased edge gains could introduce
    // clipping.
    //
    return sqrtf( std::abs( ( 100.0 - pan ) /
                            100.0 ) ); // -3dB pan law  (variant)

  } else if( m_panLaw == 2 ) {
    // -6dB Panning Law
    //
    // A channel's gain begins at 0dB and decreases to -6dB as
    // the control is moved to the center.  From there on the
    // signal continues to decrease until it is completely
    // attenuated at the opposite edge. The 3dB dip in the
    // combined power of both channels when the control is
    // centered will cause the extremes to sound louder than the
    // center.
    //
    ...
    // -3dB Panning Law
    //
    // A channel's gain begins at 0dB and decreases to -3dB as
    // the control is centered.  From there on the signal
    // continues to decrease until it is completely attenuated at
    // the opposite edge.  The combined power of both channels
    // remains constant throughout the panning range, resulting
    // in an apparent constant loudness.
    //
    return sqrtf( std::abs( ( 100.0 - pan ) / 200.0 ) );

  } else {
    // OdB Panning Law (default)
    //
    // A channel's gain begins at 0dB and remains at 0dB as the
    // control is moved to the the center.  From there on the
    // signal decreases linearly till it is completely attenuated
    // at the opposite edge. Since both channels have a gain of
    // 0dB at the center, the net effect is a 3dB boost in
    // apparent loudness when the control is centered.  This is a
    // basic balance pot.
    //
...
float AudioLevel::panGainRight(
    float pan ) // Apply panning law to right channel
{
  if( m_panLaw == 3 ) {
    return sqrtf( std::abs(
        ( 100.0 + pan ) / 100.0 ) ); // -3dB pannig law (variant)
    ...
    return sqrtf( std::abs( ( 100.0 + pan ) /
                            200.0 ) ); // -3dB panning law

  } else {
    return ( pan < 0.0 ) ? ( 100.0 + pan ) / 100.0
                         : 1.0; // 0dB panning law (default)
  }
}
