...
void ChannelManager::connectInstrument(
...
void ChannelManager::insertController(
    TrackId trackId, const Instrument *instrument,
    ChannelId channel, RealTime insertTime, MidiByte controller,
    MidiByte value, MappedInserterBase &inserter ) {
  MappedEvent mE( instrument->getId(),
                  MappedEvent::MidiController, controller,
                  value );
...
void ChannelManager::insertChannelSetup(
    TrackId trackId, const Instrument *instrument,
    ChannelId channel, RealTime insertTime,
  ...
  if( !instrument->hasFixedChannel() ||
      instrument->sendsBankSelect() ) {
    {
      // Bank Select MSB
      MappedEvent mE(
          instrument->getId(), MappedEvent::MidiController,
          MIDI_CONTROLLER_BANK_MSB, instrument->getMSB() );
      mE.setRecordedChannel( channel );
      mE.setEventTime( insertTime );
      mE.setTrackId( trackId );
      inserter.insertCopy( mE );
    }
    {
      ...
      MappedEvent mE(
          instrument->getId(), MappedEvent::MidiController,
          MIDI_CONTROLLER_BANK_LSB, instrument->getLSB() );
      mE.setRecordedChannel( channel );
      mE.setEventTime( insertTime );
      mE.setTrackId( trackId );
      inserter.insertCopy( mE );
  ...
  //        settings.value("allowresetallcontrollers",
  //        "true").toBool();
  // settings.endGroup();

  // if (allowReset) {
  //    // In case some controllers are on that we don't know
  //    about, turn
  ...
  const StaticControllers &list =
      controllerAndPBList.m_controllers;
  ...
  for( StaticControllerConstIterator cIt = list.begin();
    ...
    // RG_DEBUG << "insertChannelSetup() : inserting controller "
    // << (int)controlId << "value" << (int)controlValue << "on
    // channel" << (int)channel << "for time" << reftime;
  ...
  if( controllerAndPBList.m_havePitchbend ) {
    ...
    try {
      MappedEvent mE( instrument->getId(),
                      MappedEvent::MidiPitchBend, d1, d2 );
      mE.setRecordedChannel( channel );
      mE.setEventTime( insertTime );
      mE.setTrackId( trackId );
      inserter.insertCopy( mE );
...
void ChannelManager::insertEvent(
    TrackId                    trackId,
    const ControllerAndPBList &controllerAndPBList,
    RealTime reftime, MappedEvent &event, bool /*firstOutput*/,
    MappedInserterBase &inserter ) {
  // Profiler profiler("ChannelManager::insertEvent()", true);

  // RG_DEBUG << "insertEvent(): playing on" << (m_instrument ?
  // m_instrument->getPresentationName().c_str() : "nothing") <<
  // "at" << reftime << (firstOutput ? "needs init" : "doesn't
  // need init");

  // ??? firstOutput was always ignored.  What would happen if we
  // actually
  //     honored it?  E.g.:
  //       if (firstOutput)
  //           m_ready = false;
  ...
  // if a track becomes unmuted, until the meta-iterator gets
  // around to initting.
  if( !m_ready ) {
  ...
  // !!! These checks may not be needed now, could become
  // assertions.
...
bool ChannelManager::makeReady(
    TrackId trackId, RealTime time,
    const ControllerAndPBList &controllerAndPBList,
    MappedInserterBase &       inserter ) {
  // RG_DEBUG << "makeReady() for" << (m_instrument ?
  // m_instrument->getPresentationName().c_str() : "nothing") <<
  // "at" << time;
  ...
  if( !m_instrument->hasFixedChannel() ) {
    insertChannelSetup( trackId, time, controllerAndPBList,
                        inserter );
  }
...
void ChannelManager::insertChannelSetup(
    TrackId trackId, RealTime insertTime,
    const ControllerAndPBList &controllerAndPBList,
    MappedInserterBase &       inserter ) {
  // RG_DEBUG << "insertChannelSetup() : " << (m_instrument ?
  // "Got instrument" : "No instrument");
  ...
  if( m_instrument->getType() == Instrument::Midi ) {
    ...
    insertChannelSetup( trackId, m_instrument, channel,
                        insertTime, controllerAndPBList,
                        inserter );
  }
}

void ChannelManager::setChannelIdDirectly() {
  ...
  if( m_instrument->getType() == Instrument::Midi ) {
    ...
    if( m_instrument->isPercussion() ) {
      channel = ( m_instrument->hasFixedChannel()
                      ? m_instrument->getNaturalChannel()
                      : 9 );
    }
  }
...
AllocateChannels *ChannelManager::getAllocator() {
  if( !m_instrument ) return nullptr;

  return m_instrument->getDevice()->getAllocator();
}

void ChannelManager::connectAllocator() {
  if( !m_channelInterval.validChannel() ) return;
...
void ChannelManager::disconnectAllocator() {
  // if (m_instrument  &&  m_usingAllocator)
  //    disconnect(getAllocator(), nullptr, this, nullptr);
}

void ChannelManager::setAllocationMode(
    Instrument *instrument ) {
  if( !instrument ) {
    m_usingAllocator = false;
  } else {
    ...
    // If the allocation mode has changed, clear
    // m_channelInterval, otherwise its old value will appear
    // valid.
...
void ChannelManager::allocateChannelInterval(
    bool changedInstrument ) {
  // RG_DEBUG << "allocateChannelInterval(): " <<
  // (m_usingAllocator ? "using allocator" : "not using
  // allocator") << "for" << (void *)m_instrument;
  ...
  if( m_instrument ) {
    if( m_usingAllocator ) {
      ...
      getAllocator()->reallocateToFit(
          *m_instrument, m_channelInterval, m_start, m_end,
          m_startMargin, m_endMargin, changedInstrument );
  ...
  // RG_DEBUG << "allocateChannelInterval(): Channel is " <<
  // (m_channelInterval.validChannel() ? "valid" : "INVALID");
...
void ChannelManager::setInstrument( Instrument *instrument ) {
  // RG_DEBUG << "setInstrument(): Setting instrument to" <<
  // (void *)instrument << "It was" << (void *)m_instrument;

  // No change?  Bail.
  if( instrument == m_instrument ) return;

  if( m_instrument ) {
    Device *oldDevice = m_instrument->getDevice();
    Device *newDevice =
        instrument ? instrument->getDevice() : nullptr;
    // Don't hold onto a channel on a device we're no longer
    // playing thru.  Even if newDevice == 0, we free oldDevice's
    // channel.
    if( oldDevice != newDevice ) freeChannelInterval();
  }
...
void ChannelManager::slotVacateChannel( ChannelId channel ) {
  // Not our channel?  Bail.
  if( m_channelInterval.getChannelId() != channel ) return;

  m_channelInterval.clearChannelId();
  disconnectAllocator();
}

void ChannelManager::slotLosingDevice() {
  m_instrument = nullptr;
  m_channelInterval.clearChannelId();
}

void ChannelManager::slotLosingInstrument() {
  freeChannelInterval();
  m_instrument = nullptr;
}

void ChannelManager::slotChannelBecomesFixed() {
  // RG_DEBUG << "slotChannelBecomesFixed()" << (m_usingAllocator
  // ? "using allocator" : "not using allocator") << "for" <<
  // (void *)m_instrument;

  ChannelId channel = m_instrument->getNaturalChannel();

  // If we're already fixed and set to our natural channel,
  // there's nothing to do.
  if( !m_usingAllocator &&
      ( channel == m_channelInterval.getChannelId() ) )
    return;
...
void ChannelManager::slotChannelBecomesUnfixed() {
  // RG_DEBUG << "slotChannelBecomesUnfixed" << (m_usingAllocator
  // ? "using allocator" : "not using allocator") << "for" <<
  // (void *)m_instrument;
...
void ChannelManager::slotInstrumentChanged() {
  m_triedToGetChannel = false;
