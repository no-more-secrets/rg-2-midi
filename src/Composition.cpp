...
const PropertyName Composition::NoAbsoluteTimeProperty =
    "NoAbsoluteTime";
...
const PropertyName Composition::TargetTempoProperty =
    "TargetTempo";
const PropertyName Composition::TempoTimestampProperty =
    "TimestampSec";

bool Composition::ReferenceSegmentEventCmp::operator()(
...
Composition::ReferenceSegment::~ReferenceSegment() { clear(); }
...
timeT Composition::ReferenceSegment::getDuration() const {
...
Composition::ReferenceSegment::insertEvent( Event *e ) {
  if( !e->isa( m_eventType ) ) {
    throw Event::BadType(
        std::string( "event in ReferenceSegment" ), m_eventType,
        e->getType(), __FILE__, __LINE__ );
  }

  iterator i = find( e );

  if( i != end() &&
      ( *i )->getAbsoluteTime() == e->getAbsoluteTime() ) {
    Event *old = ( *i );
    ( *i )     = e;
    delete old;
    return i;
...
void Composition::ReferenceSegment::eraseEvent( Event *e ) {
  iterator i = find( e );
...
Composition::ReferenceSegment::iterator
Composition::ReferenceSegment::findNearestRealTime(
    RealTime t ) {
...
Composition::~Composition() {
  if( !m_observers.empty() ) {
    cerr << "Warning: Composition::~Composition() with "
         << m_observers.size() << " observers still extant"
         << endl;
    cerr << "Observers are:";
...
Composition::iterator Composition::weakAddSegment(
    Segment *segment ) {
...
void Composition::deleteSegment( Composition::iterator i ) {
  if( i == end() ) return;
  clearVoiceCaches();
...
bool Composition::deleteSegment( Segment *segment ) {
...
bool Composition::detachSegment( Segment *segment ) {
...
bool Composition::weakDetachSegment( Segment *segment ) {
  iterator i = findSegment( segment );
  if( i == end() ) return false;
  clearVoiceCaches();
...
void Composition::addAllSegments( SegmentMultiSet segments ) {
...
void Composition::addAllSegments( SegmentVec segments ) {
...
// @author Tom Breton (Tehom)
void Composition::detachAllSegments( SegmentMultiSet segments ) {
  for( SegmentMultiSet::iterator i = segments.begin();
       i != segments.end(); ++i ) {
    detachSegment( *i );
  }
...
void Composition::detachAllSegments( SegmentVec segments ) {
  for( SegmentVec::iterator i = segments.begin();
       i != segments.end(); ++i ) {
    detachSegment( *i );
  }
...
bool Composition::contains( const Segment *s ) {
  iterator i = findSegment( s );
  return ( i != end() );
}

Composition::iterator Composition::findSegment(
    const Segment *s ) {
  iterator i =
      m_segments.lower_bound( const_cast<Segment *>( s ) );

  while( i != end() ) {
    if( *i == s ) break;
    if( ( *i )->getStartTime() > s->getStartTime() )
      return end();
    ++i;
  }
...
void Composition::setSegmentStartTime( Segment *segment,
                                       timeT    startTime ) {
...
void Composition::clearVoiceCaches() {
...
void Composition::rebuildVoiceCaches() const {
  //    std::cerr << "Composition::rebuildVoiceCaches" <<
  //    std::endl;
  ...
  for( trackcontainer::const_iterator tci = m_tracks.begin();
    ...
    for( const_iterator i = begin(); i != end(); ++i ) {
      ...
      if( index >= m_trackVoiceCountCache[tid] ) {
        //                std::cerr << "count to " << index + 1
        //                << std::endl;
        m_trackVoiceCountCache[tid] = index + 1;
      }
      ends.insert( std::multimap<timeT, Segment *>::value_type(
          t1, *i ) );
    }
  }
}

int Composition::getMaxContemporaneousSegmentsOnTrack(
    TrackId track ) const {
  Profiler profiler(
      "Composition::getMaxContemporaneousSegmentsOnTrack" );

  if( m_trackVoiceCountCache.empty() ) { rebuildVoiceCaches(); }

  int count = m_trackVoiceCountCache[track];
  //    std::cerr << "max contemporaneous on track " << track <<
  //    " = " << count << std::endl;
  return count;
}

int Composition::getSegmentVoiceIndex(
...
void Composition::resetLinkedSegmentRefreshStatuses() {
...
TriggerSegmentRec *Composition::addTriggerSegment(
    Segment *s, int pitch, int velocity ) {
  TriggerSegmentId id = m_nextTriggerSegmentId;
  return addTriggerSegment( s, id, pitch, velocity );
}

TriggerSegmentRec *Composition::addTriggerSegment(
    Segment *s, TriggerSegmentId id, int pitch, int velocity ) {
  ...
  if( m_nextTriggerSegmentId <= id )
    m_nextTriggerSegmentId = id + 1;
  return rec;
}

void Composition::deleteTriggerSegment( TriggerSegmentId id ) {
  TriggerSegmentRec               dummyRec( id, nullptr );
  triggersegmentcontaineriterator i =
      m_triggerSegments.find( &dummyRec );
...
void Composition::detachTriggerSegment( TriggerSegmentId id ) {
  TriggerSegmentRec               dummyRec( id, nullptr );
  triggersegmentcontaineriterator i =
      m_triggerSegments.find( &dummyRec );
  if( i == m_triggerSegments.end() ) return;
...
void Composition::clearTriggerSegments() {
  for( triggersegmentcontaineriterator i =
           m_triggerSegments.begin();
...
int Composition::getTriggerSegmentId( Segment *s ) {
  for( triggersegmentcontaineriterator i =
           m_triggerSegments.begin();
       i != m_triggerSegments.end(); ++i ) {
...
Segment *Composition::getTriggerSegment( TriggerSegmentId id ) {
  TriggerSegmentRec *rec = getTriggerSegmentRec( id );
  if( !rec ) return nullptr;
  return rec->getSegment();
}

TriggerSegmentRec *Composition::getTriggerSegmentRec(
    TriggerSegmentId id ) {
  TriggerSegmentRec               dummyRec( id, nullptr );
  triggersegmentcontaineriterator i =
      m_triggerSegments.find( &dummyRec );
...
TriggerSegmentRec *Composition::getTriggerSegmentRec(
  ...
  const int id =
      e->get<Int>( BaseProperties::TRIGGER_SEGMENT_ID );
  return getTriggerSegmentRec( id );
}

TriggerSegmentId Composition::getNextTriggerSegmentId() const {
  return m_nextTriggerSegmentId;
}

void Composition::setNextTriggerSegmentId(
    TriggerSegmentId id ) {
  m_nextTriggerSegmentId = id;
}

void Composition::updateTriggerSegmentReferences() {
  std::map<TriggerSegmentId,
           TriggerSegmentRec::SegmentRuntimeIdSet>
      refs;
  ...
  for( std::map<TriggerSegmentId,
                TriggerSegmentRec::SegmentRuntimeIdSet>::iterator
           i = refs.begin();
       i != refs.end(); ++i ) {
...
timeT Composition::getDuration() const {
  timeT maxDuration = 0;
...
void Composition::setStartMarker( const timeT &sM ) {
  m_startMarker = sM;
  updateRefreshStatuses();
}

void Composition::setEndMarker( const timeT &eM ) {
...
void Composition::clear() {
  while( m_segments.size() > 0 ) { deleteSegment( begin() ); }
...
void Composition::calculateBarPositions() const {
  ...
  if( getStartMarker() < 0 ) {
    if( !t.empty() && ( *t.begin() )->getAbsoluteTime() <= 0 ) {
      barDuration =
          TimeSignature( **t.begin() ).getBarDuration();
    }
    lastBarNo   = getStartMarker() / barDuration;
    lastSigTime = getStartMarker();
#ifdef DEBUG_BAR_STUFF
    cerr << "Composition::calculateBarPositions: start marker = "
         << getStartMarker()
         << ", so initial bar number = " << lastBarNo << endl;
#endif
  }
  ...
  for( i = t.begin(); i != t.end(); ++i ) {
    ...
    // should only happen for first time sig, when it's at time <
    // 0:
...
int Composition::getNbBars() const {
...
int Composition::getBarNumber( timeT t ) const {
  calculateBarPositions();
  ReferenceSegment::iterator i =
      m_timeSigSegment.findNearestTime( t );
  int n;

  if( i ==
      m_timeSigSegment.end() ) { // precedes any time signatures
...
#ifdef DEBUG_BAR_STUFF
  cerr << "Composition::getBarNumber(" << t << "): returning "
       << n << endl;
#endif
  return n;
}

std::pair<timeT, timeT> Composition::getBarRangeForTime(
    timeT t ) const {
  return getBarRange( getBarNumber( t ) );
}

std::pair<timeT, timeT> Composition::getBarRange( int n ) const {
  calculateBarPositions();
  ...
  ReferenceSegment::iterator j = std::lower_bound(
      m_timeSigSegment.begin(), m_timeSigSegment.end(), &dummy,
      BarNumberComparator() );
  ...
  if( i == m_timeSigSegment.end() ||
    ...
    ++j; // j needs to point to following barline

  timeT start, finish;

  if( i ==
      m_timeSigSegment.end() ) { // precedes any time sig changes
...
#ifdef DEBUG_BAR_STUFF
    cerr << "Composition::getBarRange[1]: bar " << n << ": ("
         << start << " -> " << finish << ")" << endl;
#endif
...
#ifdef DEBUG_BAR_STUFF
    cerr << "Composition::getBarRange[2]: bar " << n << ": ("
         << start << " -> " << finish << ")" << endl;
#endif
  }

  // partial bar
  if( j != m_timeSigSegment.end() &&
      finish > ( *j )->getAbsoluteTime() ) {
    finish = ( *j )->getAbsoluteTime();
#ifdef DEBUG_BAR_STUFF
    cerr << "Composition::getBarRange[3]: bar " << n << ": ("
         << start << " -> " << finish << ")" << endl;
#endif
  }
...
template<class itr>
inline int distance( itr first, itr last ) {
  int n = 0;
...
int Composition::addTimeSignature( timeT         t,
                                   TimeSignature timeSig ) {
#ifdef DEBUG_BAR_STUFF
  cerr << "Composition::addTimeSignature(" << t << ", "
...
TimeSignature Composition::getTimeSignatureAt( timeT t ) const {
  TimeSignature timeSig;
  (void)getTimeSignatureAt( t, timeSig );
  return timeSig;
}

timeT Composition::getTimeSignatureAt(
    timeT t, TimeSignature &timeSig ) const {
...
TimeSignature Composition::getTimeSignatureInBar(
    int barNo, bool &isNew ) const {
  isNew   = false;
...
Composition::getTimeSignatureAtAux( timeT t ) const {
  ...
  // In negative time, if there's no time signature actually
  // defined prior to the point of interest then we use the next
  // time signature after it, so long as it's no later than time
  // zero. This is the only rational way to deal with count-in
  // bars where the correct time signature otherwise won't appear
  // until we hit bar zero.
...
int Composition::getTimeSignatureCount() const {
  return int( m_timeSigSegment.size() );
}

int Composition::getTimeSignatureNumberAt( timeT t ) const {
  ReferenceSegment::iterator i = getTimeSignatureAtAux( t );
  if( i == m_timeSigSegment.end() )
    return -1;
  else
...
void Composition::removeTimeSignature( int n ) {
...
tempoT Composition::getTempoAtTime( timeT t ) const {
  ReferenceSegment::iterator i =
      m_tempoSegment.findNearestTime( t );

  // In negative time, if there's no tempo event actually defined
  // prior to the point of interest then we use the next one
  // after it, so long as it's no later than time zero.  This is
  // the only rational way to deal with count-in bars where the
  // correct tempo otherwise won't appear until we hit bar zero.
  // See also getTimeSignatureAt

  if( i == m_tempoSegment.end() ) {
    if( t < 0 ) {
#ifdef DEBUG_TEMPO_STUFF
      cerr << "Composition: Negative time " << t
           << " for tempo, using 0" << endl;
#endif
  ...
  if( ( *i )->has( TargetTempoProperty ) ) {
    tempoT target =
        ( tempoT )( ( *i )->get<Int>( TargetTempoProperty ) );
    ReferenceSegment::iterator j = i;
    ++j;

    if( target > 0 ||
        ( target == 0 && j != m_tempoSegment.end() ) ) {
      timeT t0 = ( *i )->getAbsoluteTime();
      timeT t1 =
          ( j != m_tempoSegment.end() ? ( *j )->getAbsoluteTime()
                                      : getEndMarker() );
...
#ifdef DEBUG_TEMPO_STUFF
      cerr << "Composition: Calculated tempo " << result
           << " at " << t << endl;
#endif
...
#ifdef DEBUG_TEMPO_STUFF
  cerr << "Composition: Found tempo " << tempo << " at " << t
       << endl;
#endif
  return tempo;
}

int Composition::addTempoAtTime( timeT time, tempoT tempo,
                                 tempoT targetTempo ) {
  // If there's an existing tempo at this time, the
  // ReferenceSegment object will remove the duplicate, but we
  // have to ensure that the minimum and maximum tempos are
  ...
  if( n >= 0 ) {
    ...
    if( tc.first == time ) {
      if( tc.second == m_minTempo || tc.second == m_maxTempo ) {
        ...
        if( tr.first && ( tr.second == m_minTempo ||
                          tr.second == m_maxTempo ) ) {
          fullTempoUpdate = true;
        }
      }
    }
  ...
  ReferenceSegment::iterator i =
      m_tempoSegment.insertEvent( tempoEvent );

  if( fullTempoUpdate ) {
    updateExtremeTempos();

  } else {
    if( tempo < m_minTempo || m_minTempo == 0 )
      m_minTempo = tempo;
    if( targetTempo > 0 && targetTempo < m_minTempo )
      m_minTempo = targetTempo;

    if( tempo > m_maxTempo || m_maxTempo == 0 )
      m_maxTempo = tempo;
    if( targetTempo > 0 && targetTempo > m_maxTempo )
      m_maxTempo = targetTempo;
  }
...
#ifdef DEBUG_TEMPO_STUFF
  cerr << "Composition: Added tempo " << tempo << " at " << time
       << endl;
#endif
  notifyTempoChanged();
...
int Composition::getTempoChangeCount() const {
  return int( m_tempoSegment.size() );
}

int Composition::getTempoChangeNumberAt( timeT t ) const {
  ReferenceSegment::iterator i =
      m_tempoSegment.findNearestTime( t );
  if( i == m_tempoSegment.end() )
    return -1;
...
std::pair<timeT, tempoT> Composition::getTempoChange(
    int n ) const {
...
std::pair<bool, tempoT> Composition::getTempoRamping(
    int n, bool calculate ) const {
  tempoT target = -1;
  ...
  if( target == 0 ) {
    if( calculate ) {
      if( int( m_tempoSegment.size() ) > n + 1 ) {
        target =
            m_tempoSegment[n + 1]->get<Int>( TempoProperty );
      }
    }
  }
...
void Composition::removeTempoChange( int n ) {
  tempoT oldTempo = m_tempoSegment[n]->get<Int>( TempoProperty );
  tempoT oldTarget = -1;

  if( m_tempoSegment[n]->has( TargetTempoProperty ) ) {
    oldTarget =
        m_tempoSegment[n]->get<Int>( TargetTempoProperty );
  }

  m_tempoSegment.eraseEvent( m_tempoSegment[n] );
  m_tempoTimestampsNeedCalculating = true;

  if( oldTempo == m_minTempo || oldTempo == m_maxTempo ||
...
void Composition::updateExtremeTempos() {
  m_minTempo = 0;
  m_maxTempo = 0;
...
RealTime Composition::getElapsedRealTime( timeT t ) const {
  ...
  if( target > 0 ) {
    elapsed =
        ...
        time2RealTime(
            ...
            nextTempoTime - ( *i )->getAbsoluteTime(), target );
  } else {
...
timeT Composition::getElapsedTimeForRealTime(
    RealTime t ) const {
  calculateTempoTimestamps();

  ReferenceSegment::iterator i =
      m_tempoSegment.findNearestRealTime( t );
  if( i == m_tempoSegment.end() ) {
    i = m_tempoSegment.begin();
    if( t >= RealTime::zeroTime ||
        ( i == m_tempoSegment.end() ||
          ( *i )->getAbsoluteTime() > 0 ) ) {
  ...
  if( doError ) {
    ...
    cerr << "getElapsedTimeForRealTime: " << t << " -> "
         << elapsed << " (error " << ( cfReal - t ) << " or "
         << ( cfTimeT - elapsed ) << ", tempo "
         << ( *i )->getAbsoluteTime() << ":"
         << ( tempoT )( ( *i )->get<Int>( TempoProperty ) )
         << ")" << endl;
  }
...
void Composition::calculateTempoTimestamps() const {
...
#ifdef DEBUG_TEMPO_STUFF
  cerr << "Composition::calculateTempoTimestamps: Tempo events "
          "are:"
       << endl;
#endif
  ...
  for( ReferenceSegment::iterator i = m_tempoSegment.begin();
    ...
    if( target > 0 ) {
      myTime =
          lastRealTime +
          time2RealTime(
              ( *i )->getAbsoluteTime() - lastTimeT, tempo,
              ( *i )->getAbsoluteTime() - lastTimeT, target );
    } else {
      myTime =
          lastRealTime +
          time2RealTime( ( *i )->getAbsoluteTime() - lastTimeT,
                         tempo );
    }
    ...
    target              = -1;
    timeT nextTempoTime = 0;
    if( !getTempoTarget( i, target, nextTempoTime ) )
      target = -1;
  }
...
RealTime Composition::time2RealTime( timeT  t,
                                     tempoT tempo ) const {
  static timeT cdur = Note( Note::Crotchet ).getDuration();

  double dt =
      ( double( t ) * 100000 * 60 ) / ( double( tempo ) * cdur );
  ...
  if( !DEBUG_silence_recursive_tempo_printout ) {
    cerr << "Composition::time2RealTime: t " << t << ", sec "
         << sec << ", nsec " << nsec << ", tempo " << tempo
...
RealTime Composition::time2RealTime( timeT time, tempoT tempo,
                                     timeT  targetTime,
                                     tempoT targetTempo ) const {
  static timeT cdur = Note( Note::Crotchet ).getDuration();

  // The real time elapsed at musical time t, in seconds, during
  // a smooth tempo change from "tempo" at musical time zero to
...
#ifdef DEBUG_TEMPO_STUFF
  if( !DEBUG_silence_recursive_tempo_printout ) {
    cerr << "Composition::time2RealTime[2]: time " << time
         << ", tempo " << tempo << ", targetTime " << targetTime
         << ", targetTempo " << targetTempo << ": rt " << rt
         << endl;
    DEBUG_silence_recursive_tempo_printout = 1;
    //        RealTime nextRt = time2RealTime(targetTime, tempo,
    //        targetTime, targetTempo);
    timeT ct =
        realTime2Time( rt, tempo, targetTime, targetTempo );
    cerr << "cf. realTime2Time: rt " << rt << " -> " << ct
         << endl;
    DEBUG_silence_recursive_tempo_printout = 0;
  }
#endif
...
timeT Composition::realTime2Time( RealTime rt,
                                  tempoT   tempo ) const {
  static timeT cdur = Note( Note::Crotchet ).getDuration();

  double tsec = ( double( rt.sec ) * cdur ) *
                ( tempo / ( 60.0 * 100000.0 ) );
  double tnsec =
      ( double( rt.nsec ) * cdur ) * ( tempo / 100000.0 );
...
#ifdef DEBUG_TEMPO_STUFF
  if( !DEBUG_silence_recursive_tempo_printout ) {
    cerr << "Composition::realTime2Time: rt.sec " << rt.sec
         << ", rt.nsec " << rt.nsec << ", tempo " << tempo
         << ", cdur " << cdur << ", tsec " << tsec << ", tnsec "
         << tnsec << ", dt " << dt << ", t " << t << endl;
    DEBUG_silence_recursive_tempo_printout = 1;
    RealTime crt = time2RealTime( t, tempo );
    RealTime ert = rt - crt;
    timeT    et  = realTime2Time( ert, tempo );
    cerr << "cf. time2RealTime(" << t << ") -> " << crt
         << " [err " << ert << " (" << et << "?)]" << endl;
    DEBUG_silence_recursive_tempo_printout = 0;
  }
#endif
...
timeT Composition::realTime2Time( RealTime rt, tempoT tempo,
                                  timeT  targetTime,
                                  tempoT targetTempo ) const {
  static timeT cdur = Note( Note::Crotchet ).getDuration();
  ...
  double term2 =
      ( 2.0 * n * a ) * ( 2.0 * n * a ) + 8 * ( b - a ) * t * n;

  if( term2 < 0 ) {
    // We're screwed, but at least let's not crash
    std::cerr
        << "ERROR: Composition::realTime2Time: term2 < 0 (it's "
        << term2 << ")" << std::endl;
#ifdef DEBUG_TEMPO_STUFF
    std::cerr << "rt = " << rt << ", tempo = " << tempo
              << ", targetTime = " << targetTime
              << ", targetTempo = " << targetTempo << std::endl;
    std::cerr << "n = " << n << ", b = " << b << ", a = " << a
              << ", t = " << t << std::endl;
    std::cerr << "that's sqrt( ("
              << ( ( 2.0 * n * a * 2.0 * n * a ) ) << ") + "
              << ( 8 * ( b - a ) * t * n ) << " )" << endl;
...
// @author Tom Breton (Tehom)
tempoT Composition::timeRatioToTempo( RealTime &realTime,
  ...
  const double       qpm =
      beatsPerMinute * double( beatTime ) / double( cdur );
  tempoT averageTempo = Composition::getTempoForQpm( qpm );
  return averageTempo;
}

bool Composition::getTempoTarget(
    ReferenceSegment::const_iterator i, tempoT &target,
  ...
  if( ( *i )->has( TargetTempoProperty ) ) {
    ...
    if( target >= 0 ) {
      ...
      if( ++j != m_tempoSegment.end() ) {
        if( target == 0 )
          target = ( *j )->get<Int>( TempoProperty );
...
RealTime Composition::getTempoTimestamp( const Event *e ) {
  RealTime res;
  e->get<RealTimeT>( TempoTimestampProperty, res );
  return res;
}

void Composition::setTempoTimestamp( Event *e, RealTime t ) {
  e->setMaybe<RealTimeT>( TempoTimestampProperty, t );
}

void Composition::getMusicalTimeForAbsoluteTime(
    timeT absTime, int &bar, int &beat, int &fraction,
    int &remainder ) {
...
void Composition::getMusicalTimeForDuration(
    timeT absTime, timeT duration, int &bars, int &beats,
...
timeT Composition::getAbsoluteTimeForMusicalTime(
    int bar, int beat, int fraction, int remainder ) {
...
timeT Composition::getDurationForMusicalTime( timeT absTime,
                                              int   bars,
                                              int   beats,
                                              int   fractions,
                                              int   remainder ) {
  ...
  timeT         barDuration  = timeSig.getBarDuration();
  timeT         beatDuration = timeSig.getBeatDuration();
  timeT         t = bars * barDuration + beats * beatDuration +
            fractions * Note( Note::Shortest ).getDuration() +
            remainder;
  return t;
}

void Composition::setPosition( timeT position ) {
  m_position = position;
}
...
Track *Composition::getTrackById( TrackId track ) const {
  ...
  if( i != m_tracks.end() ) return ( *i ).second;

  for( trackconstiterator i = m_tracks.begin();
       i != m_tracks.end(); ++i ) {}

  return nullptr;
}

bool Composition::haveTrack( TrackId track ) const {
  trackconstiterator i = m_tracks.find( track );
...
InstrumentId Composition::getSelectedInstrumentId() const {
  if( m_selectedTrackId == NO_TRACK ) return NoInstrument;

  Track *track = getTrackById( m_selectedTrackId );

  if( !track ) return NoInstrument;
...
void Composition::setSelectedTrack( TrackId trackId ) {
  ...
  // SequenceManager needs to update ControlBlock for auto thru
  // routing to work.
...
void Composition::addTrack( Track *track ) {
  ...
  if( m_tracks.find( track->getId() ) == m_tracks.end() ) {
    ...
    std::cerr << "Composition::addTrack(" << track
              << "), id = " << track->getId()
...
bool Composition::detachTrack( Rosegarden::Track *track ) {
  ...
  for( ; it != m_tracks.end(); ++it ) {
    if( ( *it ).second == track ) break;
  }

  if( it == m_tracks.end() ) {
    std::cerr << "Composition::detachTrack() : no such track "
              << track << std::endl;
    throw Exception( "track id not found" );
    return false;
  }
...
void Composition::checkSelectedAndRecordTracks() {
  // reset m_selectedTrackId and m_recordTrack to the next valid
  // track id if the track they point to has been deleted

  if( m_tracks.find( m_selectedTrackId ) == m_tracks.end() ) {
    m_selectedTrackId =
        getClosestValidTrackId( m_selectedTrackId );
  ...
  for( recordtrackcontainer::iterator i = m_recordTracks.begin();
    ...
    // Increment before use.  This way deleting the element does
...
TrackId Composition::getClosestValidTrackId( TrackId id ) const {
  long    diff                = LONG_MAX;
  TrackId closestValidTrackId = 0;

  for( trackcontainer::const_iterator i = getTracks().begin();
       i != getTracks().end(); ++i ) {
    long cdiff = i->second->getId() - id;
    ...
    if( cdiff < diff ) {
      ...
      break; // std::map is sorted, so if the diff increases,
             // we're passed closest valid id
  }

  return closestValidTrackId;
}

TrackId Composition::getMinTrackId() const {
...
TrackId Composition::getMaxTrackId() const {
  if( getTracks().size() == 0 ) return 0;
...
void Composition::setTrackRecording( TrackId trackId,
                                     bool    recording ) {
  if( recording ) {
...
bool Composition::isTrackRecording( TrackId track ) const {
  return m_recordTracks.find( track ) != m_recordTracks.end();
}

bool Composition::isInstrumentRecording(
    InstrumentId instrumentID ) const {
  // For each Track in the Composition
  // ??? Performance: LINEAR SEARCH
  //     I see no easy fix.  Each Instrument would need to keep a
  //     list of the Tracks it is on.  Or something equally
  //     complicated.
  for( Composition::trackcontainer::const_iterator ti =
    ...
    if( track->getInstrument() == instrumentID ) {
      if( isTrackRecording( track->getId() ) ) {
        ...
        // Regardless, we only need to know that a Track is in
        // record mode.
        return true;
      }
    }
...
std::string Composition::toXmlString() const {
  ...
  for( recordtrackiterator i = m_recordTracks.begin();
       i != m_recordTracks.end(); ) {
    composition << *i;
    if( ++i != m_recordTracks.end() ) { composition << ","; }
  }
  ...
  // Place the number of the current pan law in the composition
  // tag.
  ...
  for( trackconstiterator tit = getTracks().begin();
       tit != getTracks().end(); ++tit ) {
    if( ( *tit ).second )
      composition << "  " << ( *tit ).second->toXmlString()
                  << endl;
  }
  ...
  for( ReferenceSegment::iterator i = m_timeSigSegment.begin();
    ...
    composition << "  <timesignature time=\""
                << ( *i )->getAbsoluteTime() << "\" numerator=\""
                << ( *i )->get<Int>(
                       TimeSignature::NumeratorPropertyName )
                << "\" denominator=\""
                << ( *i )->get<Int>(
                       TimeSignature::DenominatorPropertyName )
                << "\"";

    bool common = false;
    ( *i )->get<Bool>(
        TimeSignature::ShowAsCommonTimePropertyName, common );
    if( common ) composition << " common=\"true\"";

    bool hidden = false;
    ( *i )->get<Bool>( TimeSignature::IsHiddenPropertyName,
                       hidden );
    if( hidden ) composition << " hidden=\"true\"";

    bool hiddenBars = false;
    ( *i )->get<Bool>( TimeSignature::HasHiddenBarsPropertyName,
                       hiddenBars );
  ...
  for( ReferenceSegment::iterator i = m_tempoSegment.begin();
    ...
    composition << "  <tempo time=\""
                << ( *i )->getAbsoluteTime() << "\" bph=\""
                << ( ( tempo * 6 ) / 10000 ) << "\" tempo=\""
                << tempo;
    if( target >= 0 ) {
...
void Composition::clearTracks() {
  trackiterator it = m_tracks.begin();

  for( ; it != m_tracks.end(); ++it ) delete( ( *it ).second );

  m_tracks.erase( m_tracks.begin(), m_tracks.end() );
}

Track *Composition::getTrackByPosition( int position ) const {
...
int Composition::getTrackPositionById( TrackId id ) const {
  Track *track = getTrackById( id );
  if( !track ) return -1;
  return track->getPosition();
}

Rosegarden::TrackId Composition::getNewTrackId() const {
  // Re BR #1070325: another track deletion problem
  // Formerly this was returning the count of tracks currently in
  // existence -- returning a duplicate ID if some had been
  // deleted from the middle.  Let's find one that's really
  // available instead.
...
bool Composition::hasTrack( InstrumentId instrumentId ) const {
  // We don't return the TrackId since an Instrument can be on
  // more than one Track.  That would require a
  // std::vector<TrackId>.
...
SegmentMultiSet Composition::getInstrumentSegments(
    Segment *s, timeT t ) const {
...
void Composition::enforceArmRule( const Track *track ) {
...
void Composition::notifySegmentAdded( Segment *s ) const {
  // If there is an earlier repeating segment on the same track,
  // we need to notify the change of its repeat end time
...
void Composition::notifySegmentRemoved( Segment *s ) const {
  // If there is an earlier repeating segment on the same track,
  // we need to notify the change of its repeat end time
...
void Composition::notifySegmentRepeatChanged(
    Segment *s, bool repeat ) const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifySegmentRepeatEndChanged(
    Segment *s, timeT t ) const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifySegmentEventsTimingChanged(
    Segment *s, timeT delay, RealTime rtDelay ) const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
    ( *i )->segmentEventsTimingChanged( this, s, delay,
                                        rtDelay );
  }
}

void Composition::notifySegmentTransposeChanged(
    Segment *s, int transpose ) const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifySegmentTrackChanged(
    Segment *s, TrackId oldId, TrackId newId ) const {
...
void Composition::notifySegmentStartChanged( Segment *s,
                                             timeT    t ) {
  // not ideal, but best way to ensure track heights are
  // recomputed:
  clearVoiceCaches();
  updateRefreshStatuses();

  // If there is an earlier repeating segment on the same track,
  // we need to notify the change of its repeat end time (Copied
  // from notifySegmentTrackChanged())
  for( const_iterator i = begin(); i != end(); ++i ) {
    if( ( ( *i )->getTrack() == s->getTrack() ) &&
        ( ( *i )->isRepeating() ) &&
        ( ( *i )->getStartTime() < s->getStartTime() ) ) {
...
void Composition::notifySegmentEndMarkerChange( Segment *s,
                                                bool shorten ) {
  // not ideal, but best way to ensure track heights are
  // recomputed:
  clearVoiceCaches();
...
void Composition::notifyEndMarkerChange( bool shorten ) const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifyTrackChanged( Track *t ) {
  enforceArmRule( t );
...
void Composition::notifyTracksDeleted(
    std::vector<TrackId> trackIds ) const {
  // RG_DEBUG << "Composition::notifyTracksDeleted() notifying"
  // << m_observers.size() << "observers";
...
void Composition::notifyTracksAdded(
    std::vector<TrackId> trackIds ) const {
  // RG_DEBUG << "Composition::notifyTracksAdded() notifying" <<
  // m_observers.size() << "observers";
...
void Composition::notifyTrackSelectionChanged(
    TrackId t ) const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifyMetronomeChanged() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifyTimeSignatureChanged() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifyTempoChanged() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
    ( *i )->tempoChanged( this );
  }
...
void Composition::notifySelectedTrackChanged() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::notifySourceDeletion() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Composition::clearMarkers() {
  markerconstiterator it = m_markers.begin();

  for( ; it != m_markers.end(); ++it ) { delete *it; }

  m_markers.clear();
}

void Composition::addMarker( Rosegarden::Marker *marker ) {
  m_markers.push_back( marker );
  updateRefreshStatuses();
}

bool Composition::detachMarker( Rosegarden::Marker *marker ) {
...
void Composition::setSegmentColourMap(
...
void Composition::setGeneralColourMap(
    Rosegarden::ColourMap &newmap ) {
...
void Composition::distributeVerses() {
  ...
  for( SegmentMap::iterator i = tracks.begin();
       i != tracks.end(); ++i ) {
    // Reset all verse indexes and look for repeated segments
    repeats.clear();
    for( SegmentMultiSet::iterator j = i->second.begin();
         j != i->second.end(); ++j ) {
      Segment *s = *j;
      s->setVerse( 0 );
      if( s->isPlainlyLinked() ) {
        repeats[s->getLinker()->getSegmentLinkerId()].insert(
            s );
      }
    }

    // Set verse indexes where needed
    for( SegmentMap::iterator j = repeats.begin();
         j != repeats.end(); ++j ) {
      int verse = 0;
      for( SegmentMultiSet::iterator k = j->second.begin();
           k != j->second.end(); ++k ) {
        Segment *s = *k;
        s->setVerse( verse++ );
      }
...
void Composition::dump( std::ostream &out, bool ) const {
  out << "Composition segments : " << endl;

  for( const_iterator i = begin(); i != end(); ++i ) {
    Segment *s = *i;

    out << "Segment start : " << s->getStartTime()
        << " - end : " << s->getEndMarkerTime()
        << " - repeating : " << s->isRepeating()
        << " - track id : " << s->getTrack() << " - label : "
        << s->getLabel()
        //            << " - verse : " << s->getVerse()
        << endl;
  }
