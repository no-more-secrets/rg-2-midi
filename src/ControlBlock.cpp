...
ControlBlock *ControlBlock::getInstance() {
...
void ControlBlock::clearTracks() {
  // ??? Giving TrackInfo a proper default ctor would simplify
  // this.
  //       m_trackInfo[i] = TrackInfo();
  //     It would also mean that this wouldn't need to be called
  //     in the ctor.
...
void ControlBlock::setDocument( RosegardenDocument *doc ) {
  clearTracks();
  m_doc        = doc;
...
void ControlBlock::updateTrackData( Track *t ) {
  if( t ) {
    ...
    setTrackChannelFilter( t->getId(),
                           t->getMidiInputChannel() );
    setTrackThruRouting( t->getId(), t->getThruRouting() );
    if( t->getId() > m_maxTrackId ) m_maxTrackId = t->getId();
  }
}

void ControlBlock::setInstrumentForTrack( TrackId      trackId,
                                          InstrumentId instId ) {
...
InstrumentId ControlBlock::getInstrumentForTrack(
...
void ControlBlock::setTrackArchived( TrackId trackId,
                                     bool    archived ) {
  if( trackId < CONTROLBLOCK_MAX_NB_TRACKS )
...
bool ControlBlock::isSolo( TrackId trackId ) const {
  if( trackId >= CONTROLBLOCK_MAX_NB_TRACKS ) return false;
...
bool ControlBlock::isAnyTrackInSolo() const {
  ...
  for( unsigned int i = 0; i <= m_maxTrackId; ++i ) {
    ...
    if( track.m_deleted ) continue;

    if( track.m_solo ) return true;
  }
...
void ControlBlock::setTrackDeleted( TrackId trackId,
                                    bool    deleted ) {
  if( trackId >= CONTROLBLOCK_MAX_NB_TRACKS ) { return; }
...
void ControlBlock::setTrackChannelFilter( TrackId trackId,
                                          char    channel ) {
  if( trackId < CONTROLBLOCK_MAX_NB_TRACKS )
...
void ControlBlock::setTrackDeviceFilter( TrackId  trackId,
                                         DeviceId device ) {
  if( trackId < CONTROLBLOCK_MAX_NB_TRACKS )
...
bool ControlBlock::isInstrumentMuted(
...
bool ControlBlock::isInstrumentUnused(
    InstrumentId instrumentId ) const {
  for( unsigned int i = 0; i <= m_maxTrackId; ++i ) {
    if( m_trackInfo[i].m_instrumentId == instrumentId &&
...
void ControlBlock::setSelectedTrack( TrackId track ) {
  // Undo the old selected track.  Safe even if it referred to
  // the same track or to no track.
...
InstrumentAndChannel ControlBlock::getInstAndChanForEvent(
  ...
  for( unsigned i = 0; i <= m_maxTrackId; ++i ) {
    ...
    // if the event doesn't match this track's filters, try the
    // next track
    if( !deviceMatch || !channelMatch ) continue;
...
// Kick all tracks' thru-channels off channel and arrange to find
// new homes for them.  This is called by AllocateChannels when a
// fixed channel has commandeered the channel.
// @author Tom Breton (Tehom)
void ControlBlock::vacateThruChannel( int channel ) {
  for( unsigned int i = 0; i <= m_maxTrackId; ++i ) {
    TrackInfo &track = m_trackInfo[i];
...
// @author Tom Breton (Tehom)
void ControlBlock::instrumentChangedProgram(
    InstrumentId instrumentId ) {
  for( unsigned int i = 0; i <= m_maxTrackId; ++i ) {
    TrackInfo &track = m_trackInfo[i];
    if( track.m_hasThruChannel &&
        ( track.m_instrumentId == instrumentId ) ) {
...
// @author Tom Breton (Tehom)
void ControlBlock::instrumentChangedFixity(
    InstrumentId instrumentId ) {
  for( unsigned int i = 0; i <= m_maxTrackId; ++i ) {
    TrackInfo &track = m_trackInfo[i];
    if( track.m_hasThruChannel &&
...
void TrackInfo::conform( Studio &studio ) {
  bool thruAuto = ( m_thruRouting == Track::Auto );
  bool shouldHaveThru =
      ( !thruAuto || m_armed || m_selected ) && !m_deleted;
#ifdef DEBUG_CONTROL_BLOCK
#endif
...
InstrumentAndChannel TrackInfo::getChannelAsReady(
    Studio &studio ) {
  if( !m_hasThruChannel ) { return InstrumentAndChannel(); }

  // If our channel might not have the right program, send it
  // now.
  if( !m_isThruChannelReady ) { makeChannelReady( studio ); }
  return InstrumentAndChannel( m_instrumentId, m_thruChannel );
}

void TrackInfo::makeChannelReady( Studio &studio ) {
#ifdef DEBUG_CONTROL_BLOCK
#endif
  Instrument *instrument =
      studio.getInstrumentById( m_instrumentId );

  // If we have deleted a device, we may get a nullptr
  // instrument.  In that case, we can't do much.
  if( !instrument ) { return; }

  // We can get non-Midi instruments here.  There's nothing to do
  // for them.  For fixed, sendChannelSetup is slightly wrong,
  // but could be adapted and parameterized by trackId.
  if( ( instrument->getType() == Instrument::Midi ) &&
    ...
    if( allocator ) {
      m_thruChannel = allocator->reallocateThruChannel(
          *instrument, m_thruChannel );
...
// @author Tom Breton (Tehom)
void TrackInfo::allocateThruChannel( Studio &studio ) {
  Instrument *instrument =
      studio.getInstrumentById( m_instrumentId );
  ...
  // This value of fixity holds until releaseThruChannel is
  // called.
...
void TrackInfo::releaseThruChannel( Studio &studio ) {
  ...
  // If we recently deleted a device, we may get a nullptr
  // instrument. In that case, we can't actively release it but
  // we don't need to, we can just mark it released.
...
void TrackInfo::instrumentChangedFixity( Studio &studio ) {
