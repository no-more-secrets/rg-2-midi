...
ControlParameter::ControlParameter(
    ...
    const std::string &description, int min, int max, int def,
    MidiByte controllerValue, unsigned int colour,
    int ipbPosition )
  : m_name( name ),
    m_type( type ),
...
ControlParameter &ControlParameter::operator=(
    const ControlParameter &control ) {
  m_name            = control.getName();
  m_type            = control.getType();
  m_description     = control.getDescription();
...
bool ControlParameter::operator==(
    const ControlParameter &control ) {
  return m_type == control.getType() &&
         m_controllerValue == control.getControllerValue() &&
         m_min == control.getMin() && m_max == control.getMax();
}

bool operator<( const ControlParameter &a,
                const ControlParameter &b ) {
...
std::string ControlParameter::toXmlString() const {
...
Event *ControlParameter::newEvent( timeT time,
                                   int   value ) const {
...
// @author Tom Breton (Tehom)
bool ControlParameter::matches( Event *e ) const {
  return e->isa( m_type ) &&
...
// These exists to support calling PitchBendSequenceDialog
// because some calls to PitchBendSequenceDialog always pitchbend
// or expression rather than getting a ControlParameter from a
// ruler or device.  This can't be just a static member of
// ControlParameter, in order to prevent the "static
// initialization order fiasco".
const ControlParameter &ControlParameter::getPitchBend() {
  static const ControlParameter pitchBend(
      "PitchBend", Rosegarden::PitchBend::EventType, "<none>", 0,
      16383, 8192, MidiByte( 1 ), 4, -1 );
  return pitchBend;
}

const ControlParameter &ControlParameter::getExpression() {
  static const ControlParameter expression(
      "Expression", Rosegarden::Controller::EventType, "<none>",
      0, 127, 100, MidiByte( 11 ), 2, -1 );
  return expression;
}
