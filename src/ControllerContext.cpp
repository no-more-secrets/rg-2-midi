...
/*** ControllerSearch ***/
ControllerSearch::ControllerSearch( const std::string eventType,
                                    int controllerId )
  : m_eventType( eventType ), m_controllerId( controllerId ) {}

// Return the last value of controller before noLaterThan in
// segment s.
// @author Tom Breton (Tehom)
ControllerSearch::Maybe ControllerSearch::searchSegment(
    const Segment *s, timeT noEarlierThan,
  ...
  for( Segment::reverse_iterator j = latest; j != s->rend();
       ++j ) {
    timeT t = ( *j )->getAbsoluteTime();

    // Event is too early.  No controller we can find will
    // satisfy the search.
    if( ( t <= noEarlierThan ) ) { break; }
...
// Search Segments A and B for the latest controller value.
// Search A first.  B may be nullptr but A must exist.
ControllerSearch::Maybe ControllerSearch::doubleSearch(
    Segment *a, Segment *b, timeT noLaterThan ) const {
  Profiler profiler( "ControllerSearch::doubleSearch", false );
  ControllerSearch::Maybe runningResult = searchSegment(
      a, std::numeric_limits<int>::min(), noLaterThan );
  if( b ) {
    timeT noEarlierThan = runningResult.first
                              ? runningResult.second.m_when
                              : std::numeric_limits<int>::min();
    ControllerSearch::Maybe result2 =
        searchSegment( b, noEarlierThan, noLaterThan );
    if( result2.first ) { runningResult = result2; }
  }

  return runningResult;
}

// Return true just if event e is what we're processing in the
// current search.
// @author Tom Breton (Tehom)
bool ControllerSearch::matches( Event *e ) const {
  return e->isa( m_eventType ) &&
         ( ( m_eventType != Controller::EventType ) ||
           ( e->has( Controller::NUMBER ) &&
             e->get<Int>( Controller::NUMBER ) ==
                 m_controllerId ) );
}

// Get the static value for the controller we are searching
// about.
// @author Tom Breton (Tehom)
int ControllerContextMap::getStaticValue(
    Instrument *instrument, const std::string eventType,
    int controllerId ) {
...
// @author Tom Breton (Tehom)
int ControllerContextMap::getControllerValue(
    Instrument *instrument, Segment *a, Segment *b,
    timeT searchTime, const std::string eventType,
    int controllerId ) {
  Profiler profiler( "ControllerContextMap::getControllerValue",
                     false );
  ...
  // If cache doesn't have that controller, then no such
  // controller is in this segment, so use the static value.
  if( !valueExists ) {
    return getStaticValue( instrument, eventType, controllerId );
  }

  // If we are servicing a repeating segment, the segment may
  // repeat many times but we only have to search it once. Figure
  // out what time we really should look at; mutate searchTime
  // accordingly. Segment A governs timing and repitition.
  bool firstRepeat;
  //
  if( a->isRepeating() ) {
    ...
    if( searchTime >= segmentEndTime ) {
      timeT timeRelativeToStart =
          ( searchTime - segmentStartTime );
      firstRepeat = false;
      searchTime  = ( timeRelativeToStart % segmentDuration ) +
                   segmentStartTime;
    } else {
      firstRepeat = true;
    }
  ...
  // the cache.  This lets us essentially skip segments that have
  // no controller of that type and gives us relative controllers
  // for triggers immediately too.
  if( lastValue->time() < searchTime ) {
    return lastValue->value();
  }

  // Some non-static values exist for this controller but the
  // last value isn't it, so search.
  ...
  // If this is a repeat, we've wrapped around, so the value is
  // the last value from a previous repeat, which is the same as
  // cached value.
  if( !firstRepeat ) { return lastValue->value(); }
...
ControllerContextMap::getControlParameter(
    Instrument *instrument, const std::string eventType,
...
// @author Tom Breton (Tehom)
int ControllerContextMap::makeAbsolute(
    const ControlParameter *controlParameter, int value ) const {
...
// @param A
// A segment to search if the cached value doesn't fit (is later
// than at).
// @param B
// An optional second segment to search
// @author Tom Breton (Tehom)
void ControllerContextMap::makeControlValueAbsolute(
    Instrument *instrument, Segment *a, Segment *b, Event *e,
    timeT at ) {
  Profiler profiler(
      "ControllerContextMap::makeControlValueAbsolute", false );
  ...
  if( result.first ) {
    ...
    baseline =
        getStaticValue( instrument, eventType, controllerId );
  }
...
// controller event.
// @author Tom Breton (Tehom)
void ControllerContextMap::storeLatestValue( Event *e ) {
  Profiler profiler( "ControllerContextMap::storeLatestValue",
                     false );
...
// Clear the cache.
// @author Tom Breton (Tehom)
void ControllerContextMap::clear() {
  m_latestValues.clear();
  m_PitchBendLatestValue =
      Maybe( false, ControllerSearchValue() );
}
