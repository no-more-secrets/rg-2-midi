...
// PluginContainer::~PluginContainer() { clearPlugins(); }

// void PluginContainer::addPlugin(
//    AudioPluginInstance *instance ) {
//  m_audioPlugins.push_back( instance );
//}

// bool PluginContainer::removePlugin( unsigned int position ) {
...
// void PluginContainer::clearPlugins() {
//  PluginInstanceIterator it = m_audioPlugins.begin();
//  for( ; it != m_audioPlugins.end(); ++it ) delete( *it );

//  m_audioPlugins.erase( m_audioPlugins.begin(),
//                        m_audioPlugins.end() );
//}

// void PluginContainer::emptyPlugins() {
//  PluginInstanceIterator it = m_audioPlugins.begin();
//  for( ; it != m_audioPlugins.end(); ++it ) {
...
////
// AudioPluginInstance *PluginContainer::getPlugin(
//    unsigned int position ) const {
//  // For each plugin
//  for( PluginInstanceConstIterator it = m_audioPlugins.begin();
//       it != m_audioPlugins.end(); ++it ) {
//    // Found?  Return it.
//    if( ( *it )->getPosition() == position ) return *it;
//  }
...
Instrument::Instrument( InstrumentId id, InstrumentType it,
  ...
  if( it == SoftSynth ) {
    // addPlugin(
    //    new AudioPluginInstance( SYNTH_PLUGIN_POSITION ) );
  }
}

Instrument::Instrument( InstrumentId id, InstrumentType it,
                        const std::string &name,
                        MidiByte channel, Device *device )
  : m_id( id ),
    m_name( name ),
    m_alias( "" ),
    m_type( it ),
  ...
  if( it == Audio || it == SoftSynth ) {
    ...
    /*
     *
     * Let's try getting rid of this default behavior, and
     replacing it with a
     * change to the factory autoload instead, because this just
     doesn't work out
     * very well, and it's fiddly trying to sort the overall
     behavior into something
     * less quirky (dmm)
     *
            // Also defined in Midi.h but we don't use that - not
     here
...
Instrument::Instrument( const Instrument &ins )
  : XmlExportable(),
    // PluginContainer( ins.getType() == Audio ||
    //                 ins.getType() == SoftSynth ),
    m_id( ins.getId() ),
    m_name( ins.getName() ),
  ...
  StaticControllerConstIterator cIt =
      ins.m_staticControllers.begin();
...
Instrument::~Instrument() { clearStaticControllers(); }

std::string Instrument::getPresentationName() const {
  return m_name;
}

// std::string Instrument::getLocalizedPresentationName() const {
//  // This is hacky, but m_name is always set externally, and I
//  // don't feel like tracking down all the different places
//  where
//  // this could occur.  Instead, we at least limit the spread
//  of
//  // hackery to one centralized function to cut the name string
//  // apart and translate the relevant part of it.

//  // take everything left of the # - 1 to get the "General MIDI
//  // Device" out of a string like "General MIDI Device #16"
//  std::string iname  = m_name;
//  std::string inameL = iname;
//  std::string inameR = iname;

//  // translate the left piece (we'll leave the #1..#n as an
//  // untranslatable Rosegarden-specific concept unless people
//  are
//  // really bothered by it)
//  return inameL + " " + inameR;
//}

unsigned int Instrument::getPresentationNumber() const {
  // Again, m_name is always set externally.  Instruments are
  // numbered sequentially in a way that makes determining the
  // correct channel association tricky, so we pick it back apart
  // from the string to return what channel a given instrument
  // should be set to.
  // std::string iname = std::string::fromStdString( m_name );
  // std::string number =
  //    iname.mid( iname.indexOf( "#" ) + 1, iname.length() );
...
std::string Instrument::getAlias() const {
  // If there is no alias, return the "presentation name".
  if( m_alias.empty() ) return m_name;

  return m_alias;
}

void Instrument::sendChannelSetup() {
  // MIDI only
  if( m_type != Midi ) return;
...
void Instrument::setProgram( const MidiProgram &program ) {
  m_program = program;
  // changedChannelSetup();
  ControlBlock::getInstance()->instrumentChangedProgram(
      getId() );
}

bool Instrument::isProgramValid() const {
  ...
  for( BankList::const_iterator oBankIter = validBanks.begin();
       oBankIter != validBanks.end(); ++oBankIter ) {
  ...
  ProgramList programList =
      md->getPrograms( m_program.getBank() );

  bool programChangeValid = false;

  // For each program in this bank
  for( ProgramList::const_iterator programIter =
           programList.begin();
       programIter != programList.end(); ++programIter ) {
  ...
  if( !programChangeValid ) return false;

  return true;
}

void Instrument::setProgramChange( MidiByte program ) {
  setProgram( MidiProgram( m_program.getBank(), program ) );
}

MidiByte Instrument::getProgramChange() const {
  return m_program.getProgram();
}

void Instrument::setMSB( MidiByte msb ) {
  setProgram(
      MidiProgram( MidiBank( m_program.getBank().isPercussion(),
                             msb, m_program.getBank().getLSB() ),
                   m_program.getProgram() ) );
}

MidiByte Instrument::getMSB() const {
  return m_program.getBank().getMSB();
}

void Instrument::setLSB( MidiByte lsb ) {
  setProgram(
      MidiProgram( MidiBank( m_program.getBank().isPercussion(),
                             m_program.getBank().getMSB(), lsb ),
                   m_program.getProgram() ) );
}

MidiByte Instrument::getLSB() const {
  return m_program.getBank().getLSB();
}

void Instrument::pickFirstProgram( bool percussion ) {
  MidiDevice *md = dynamic_cast<MidiDevice *>( m_device );
  if( !md ) return;
  ...
  // setProgram() will notify the rest of the system of this
  // change.
...
void Instrument::setPercussion( bool percussion ) {
  setProgram( MidiProgram(
      MidiBank( percussion, m_program.getBank().getMSB(),
                m_program.getBank().getLSB() ),
      m_program.getProgram() ) );
}

bool Instrument::isPercussion() const {
  return m_program.getBank().isPercussion();
}

void Instrument::setAudioInputToBuss( BussId buss,
                                      int    channel ) {
  m_audioInput        = buss;
  m_audioInputChannel = channel;
}

void Instrument::setAudioInputToRecord( int recordIn,
                                        int channel ) {
  m_audioInput        = recordIn + 1000;
  m_audioInputChannel = channel;
}

int Instrument::getAudioInput( bool &isBuss,
                               int & channel ) const {
...
std::string Instrument::toXmlString() const {
  ...
  if( m_type == Midi ) {
    ...
    instrument << "            <bank send=\""
               << ( m_sendBankSelect ? "true" : "false" )
               << "\" percussion=\""
               << ( isPercussion() ? "true" : "false" )
               << "\" msb=\"" << (int)getMSB() << "\" lsb=\""
               << (int)getLSB() << "\"/>" << std::endl;
    ...
    instrument << "            <pan value=\"" << (int)m_pan
               << "\"/>" << std::endl;

    instrument << "            <volume value=\"" << (int)m_volume
               << "\"/>" << std::endl;

    for( StaticControllerConstIterator it =
             m_staticControllers.begin();
         it != m_staticControllers.end(); ++it ) {
      instrument << "            <controlchange type=\""
                 << int( it->first ) << "\" value=\""
  ...
  } else // Audio or SoftSynth
  {
    ...
    instrument << "            <level value=\"" << m_level
               << "\"/>" << std::endl;
    ...
    instrument << "            <audioInput value=\"" << ai
               << "\" type=\"" << ( aibuss ? "buss" : "record" )
               << "\" channel=\"" << channel << "\"/>"
               << std::endl;
    ...
    instrument << "            <alias value=\"" << m_alias
               << "\"/>" << std::endl;

    // std::vector<AudioPluginInstance *>::const_iterator it =
    //    m_audioPlugins.begin();
    // for( ; it != m_audioPlugins.end(); ++it ) {
...
// we send it or not)
//
std::string Instrument::getProgramName() const {
  if( m_sendProgramChange == false ) return std::string( "" );

  MidiProgram program( m_program );

  if( !m_sendBankSelect )
    program = MidiProgram( MidiBank( isPercussion(), 0, 0 ),
                           program.getProgram() );
...
void Instrument::sendController( MidiByte controller,
                                 MidiByte value ) {
  if( hasFixedChannel() )
    StudioControl::sendController( this, m_channel, controller,
                                   value );
}

void Instrument::setControllerValue( MidiByte controller,
                                     MidiByte value ) {
  // two special cases
  ...
  m_staticControllers.push_back(
      std::pair<MidiByte, MidiByte>( controller, value ) );

  // changedChannelSetup();
}

MidiByte Instrument::getControllerValue(
    MidiByte controller ) const {
  for( StaticControllerConstIterator it =
           m_staticControllers.begin();
       it != m_staticControllers.end(); ++it ) {
    if( it->first == controller ) { return it->second; }
  }

  throw std::string( "<no controller of that value>" );
}

void Instrument::removeStaticController( MidiByte controller ) {
  for( StaticControllerIterator it = m_staticControllers.begin();
       it != m_staticControllers.end(); ++it ) {
    if( it->first == controller ) {
...
const MidiKeyMapping *Instrument::getKeyMapping() const {
  MidiDevice *md = dynamic_cast<MidiDevice *>( m_device );
  if( !md ) return nullptr;

  const MidiKeyMapping *mkm =
      md->getKeyMappingForProgram( m_program );
  if( mkm ) return mkm;

  if( isPercussion() ) { // if any key mapping is available, use
                         // it
    const KeyMappingList &kml = md->getKeyMappings();
    if( kml.begin() != kml.end() ) { return &( *kml.begin() ); }
  }
...
// accordingly.
void Instrument::setFixedChannel() {
  if( m_fixed ) { return; }
  ...
  if( allocator ) {
    ...
    m_fixed = true;
    // channelBecomesFixed();
    ControlBlock::getInstance()->instrumentChangedFixity(
        getId() );
  }
}
...
void Instrument::releaseFixedChannel() {
  if( !m_fixed ) { return; }
  ...
  m_fixed = false;
  // channelBecomesUnfixed();
  ControlBlock::getInstance()->instrumentChangedFixity(
      getId() );
}

// std::shared_ptr<InstrumentStaticSignals>
...
//  static std::shared_ptr<InstrumentStaticSignals>
//      instrumentStaticSignals;

//  // Delayed creation to avoid static construction order
//  fiasco. if( !instrumentStaticSignals )
//    instrumentStaticSignals =
//        std::shared_ptr<InstrumentStaticSignals>(
//            new InstrumentStaticSignals );

//  // If you want to avoid the complementary static destruction
//  // order fiasco, hold on to a copy of this in a member
//  // std::shared_ptr.  This is only really an issue if your
//  // object also has static lifespan. Still, it's a good habit.
...
Buss::Buss( BussId id )
  : m_id( id ), m_level( 0.0 ), m_pan( 100 ), m_mappedId( 0 ) {}

Buss::~Buss() {}

std::string Buss::toXmlString() const {
  std::stringstream buss;

  buss << "    <buss id=\"" << m_id << "\">" << std::endl;
  buss << "       <pan value=\"" << (int)m_pan << "\"/>"
       << std::endl;
  buss << "       <level value=\"" << m_level << "\"/>"
       << std::endl;
...
std::string Buss::getName() const {
  char buffer[20];
  sprintf( buffer, "Submaster %d", m_id );
  return buffer;
}

std::string Buss::getPresentationName() const {
  return getName();
}

std::string Buss::getAlias() const { return getName(); }

/***** RecordIn *****/

RecordIn::RecordIn() : m_mappedId( 0 ) {}

RecordIn::~RecordIn() {}

std::string RecordIn::toXmlString() const {
  // We don't actually save these, as they have nothing
  // persistent in them.  The studio just remembers how many
  // there should be.
  return "";
}
