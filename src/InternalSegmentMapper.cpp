...
#include "Midi.h" // For MIDI_SYSTEM_EXCLUSIVE
#include "MidiTypes.h"
...
InternalSegmentMapper::InternalSegmentMapper(
    RosegardenDocument *doc, Segment *segment )
...
RealTime InternalSegmentMapper::toRealTime( Composition &comp,
                                            timeT        t ) {
  return comp.getElapsedRealTime( t ) +
         m_segment->getRealTimeDelay();
}
...
void InternalSegmentMapper::fillBuffer() {
...
#ifdef DEBUG_INTERNAL_SEGMENT_MAPPER
  SEQMAN_DEBUG << "InternalSegmentMapper::fillBuffer"
               << (void *)this << "Segment" << (void *)m_segment
               << "with repeat count" << repeatCount << endl;
#endif
  ...
  for( int repeatNo = 0; repeatNo <= repeatCount; ++repeatNo ) {
    ...
    for( Segment::iterator j = m_segment->begin();
      ...
      if( !usingImplied ) {
        long triggerId = -1;
        ( **k )->get<Int>( BaseProperties::TRIGGER_SEGMENT_ID,
                           triggerId );
        ...
        if( triggerId >= 0 ) {
          ...
          ControllerContextParams params(
              refTime, getInstrument(), m_segment,
          ...
          bool insertedSomething =
              rec && rec->ExpandInto( m_triggeredEvents, j,
                                      m_segment, &params );
          if( insertedSomething ) {
            // Re-find `implied'
            implied = Segment::iterator(
      ...
      if( !( **k )->isa( Note::EventRestType ) ) {
        ...
        timeT playTime = helper.getSoundingAbsoluteTime( *k ) +
                         timeForRepeats;
        if( playTime >= repeatEndTime ) break;

        timeT playDuration = helper.getSoundingDuration( *k );

        // Ignore notes without duration -- they're probably in a
        // tied series but not as first note
        //
        if( playDuration > 0 ||
          ...
          const RealTime eventTime =
              toRealTime( comp, playTime );

          // slightly quicker than calling
          // helper.getRealSoundingDuration()
          RealTime endTime =
...
#ifdef DEBUG_INTERNAL_SEGMENT_MAPPER
            SEQMAN_DEBUG << "SegmentMapper::fillBuffer - caught "
      ...
      ++*k; // increment either i or j, whichever one we just
            // used
    }
  }
  ...
  if( anything ) {
    ...
    // ??? Shouldn't we add the duration of the event?
    // getDuration().
    maxRealTime = getBuffer()[size() - 1].getEventTime();

    // Fix for bug #1378.  Start slightly before the first note
    // so that program etc is sent then.  We'll allow it to be
    // before zeroTime, since MappedBufMetaIterator can handle
    // early start-times.
    static const RealTime preparationTime =
        RealTime::fromSeconds( 0.5 );
  ...
  if( !ControlBlock::getInstance()->isTrackMuted(
          track->getId() ) &&
      !ControlBlock::getInstance()->isTrackArchived(
          track->getId() ) ) {
  ...
  // We have changed the contents, so force a reinit.  Even if
  // the length is the same, the current controllers for a given
...
bool InternalSegmentMapper::haveEarlierNoteoff( timeT t ) {
  return ( !m_noteOffs.empty() ) &&
         ( m_noteOffs.begin()->first <= t );
}

void InternalSegmentMapper::enqueueNoteoff( timeT time,
                                            int   pitch ) {
...
void InternalSegmentMapper::popInsertNoteoff(
    int trackid, Composition &comp ) {
  // Look at top element
...
int InternalSegmentMapper::addSize( int size, Segment *s ) {
  int repeatCount = getSegmentRepeatCount();
  // Double the size because we may get a noteoff for every
  // noteon
  return size + ( repeatCount + 1 ) * 2 * int( s->size() );
}

int InternalSegmentMapper::calculateSize() {
  if( !m_segment ) { return 0; }
  return addSize( 0, m_segment );
}

// Make the channel ready to be played on.
void InternalSegmentMapper::makeReady(
    MappedInserterBase &inserter, RealTime time ) {
  Instrument *instrument = m_doc->getInstrument( m_segment );
  m_channelManager.setInstrument( instrument );
  m_channelManager.makeReady( m_segment->getTrack(), time,
                              getControllers( instrument, time ),
                              inserter );
}

void InternalSegmentMapper::insertChannelSetup(
    MappedInserterBase &inserter ) {
  Instrument *instrument = m_doc->getInstrument( m_segment );

  // If this segment's Instrument is in Auto channels mode, bail.
  if( !instrument->hasFixedChannel() ) return;

  m_channelManager.setInstrument( instrument );
  m_channelManager.insertChannelSetup(
      m_segment->getTrack(), RealTime::zeroTime,
      getControllers( instrument, RealTime::zeroTime ),
      inserter );
}

void InternalSegmentMapper::doInsert(
    MappedInserterBase &inserter, MappedEvent &evt,
    RealTime start, bool dirtyIter ) {
  Instrument *instrument = m_doc->getInstrument( m_segment );

  if( dirtyIter ) m_channelManager.setInstrument( instrument );

  m_channelManager.insertEvent(
      m_segment->getTrack(), // trackId
      getControllers( instrument, start ), start, evt,
      dirtyIter, // firstOutput
      inserter );
}

int InternalSegmentMapper::getControllerValue(
    timeT searchTime, const std::string eventType,
    int controllerId ) {
  return m_controllerCache.getControllerValue(
      getInstrument(), m_segment, m_triggeredEvents, searchTime,
      eventType, controllerId );
}

bool InternalSegmentMapper::shouldPlay( MappedEvent *evt,
                                        RealTime sliceStart ) {
  // #1048388:
  // Ensure sysex heeds mute status, but ensure clocks etc still
  // get through
...
ControllerAndPBList InternalSegmentMapper::getControllers(
    Instrument *instrument, RealTime start ) {
  Profiler profiler( "InternalSegmentMapper::getControllers()",
                     false );
  timeT    startTime =
      m_doc->getComposition().getElapsedTimeForRealTime( start );

  // If time is at or before all events, we can just use static
  // values.
  ...
  // Get a value for each controller type.  Always get a value,
  // just in case the controller used to be set on this channel.
  ...
  for( StaticControllerConstIterator cIt = list.begin();
    ...
    MidiByte controlValue = getControllerValue(
        startTime, Controller::EventType, controlId );
    returnValue.m_controllers.push_back(
        std::pair<MidiByte, MidiByte>( controlId,
                                       controlValue ) );
  }

  // Do the same for PitchBend, of which we only treat one (ie,
  // we ignore GM2's other PitchBend types)
  {
    MidiByte controlValue =
        getControllerValue( startTime, PitchBend::EventType, 0 );
