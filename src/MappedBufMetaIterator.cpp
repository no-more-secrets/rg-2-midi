...
MappedBufMetaIterator::~MappedBufMetaIterator() { clear(); }

void MappedBufMetaIterator::addSegment(
    std::shared_ptr<MappedEventBuffer> mappedEventBuffer ) {
  // BUG #1349 (was #3546135)
  // If we already have this segment, bail, or else we'll have
  // two iterators pointing to the same segment.  That will
  // eventually cause an access to freed memory and a subsequent
  // crash. This seems to happen when recording and we pass the
...
void MappedBufMetaIterator::removeSegment(
    std::shared_ptr<MappedEventBuffer> mappedEventBuffer ) {
  ...
  for( SegmentIterators::iterator i = m_iterators.begin();
    ...
    if( ( *i )->getSegment() == mappedEventBuffer ) {
      delete( *i );
      // Now mappedEventBuffer may not be a valid address since
      // the iterator we just deleted may have been the last
      // "owner" of the MappedEventBuffer.  See
      // MappedEventBuffer::iterator's dtor and
...
void MappedBufMetaIterator::clear() {
...
void MappedBufMetaIterator::reset() {
...
void MappedBufMetaIterator::jumpToTime( const RealTime &time ) {
  reset();
...
void MappedBufMetaIterator::moveIteratorToTime(
    MappedEventBuffer::iterator &iter, const RealTime &time ) {
  // ??? Move this routine to
  // MappedEventBuffer::iterator::moveTo(time).
...
void MappedBufMetaIterator::fetchFixedChannelSetup(
  ...
  for( MappedSegments::iterator i = m_segments.begin();
       i != m_segments.end(); ++i ) {
    std::shared_ptr<MappedEventBuffer> mappedEventBuffer = *i;

    TrackId trackID = mappedEventBuffer->getTrackID();

    // If we've already seen this track, try the next segment.
    if( tracks.find( trackID ) != tracks.end() ) continue;

    tracks.insert( trackID );

    // Insert channel setup if this track is in Fixed channel
    // mode.
    mappedEventBuffer->insertChannelSetup( inserter );
  }
}

void MappedBufMetaIterator::fetchEvents(
    MappedInserterBase &inserter, const RealTime &startTime,
    const RealTime &endTime ) {
  Profiler profiler( "MappedBufMetaIterator::fetchEvents",
                     false );
  ...
  // instance sending their initializations while another is
  // playing on the channel, we divide the timeslice into
  // sub-slices during which no new mappers start and pass each
  // sub-slice to fetchEventsNoncompeting.  We could re-slice it
  // smarter but this suffices.

  // Make a queue of all segment starts that occur during the
  // slice.
  // ??? Why not use std::set instead?  All this is doing is
  // sorting
  //     the start times.  std::vector and std::sort() should be
  //     another option.  Using std::priority_queue implies there
  //     will be a Compare predicate that is more interesting
  //     than std::greater.
  ...
  while( !segStarts.empty() ) {
    ...
    segStarts.pop();
    // If it starts exactly at innerStart, it doesn't need its
    // own slice.
  ...
  // Do one more slice to take us to the end time.  This is
  // always correct to do, since segStarts can't contain a start
  // equal to endTime.
...
void MappedBufMetaIterator::fetchEventsNoncompeting(
  ...
  Profiler profiler(
      "MappedBufMetaIterator::fetchEventsNoncompeting", false );

  m_currentTime = endTime;

  // For each segment, activate segments that have anything
  // playing.
  for( SegmentIterators::iterator i = m_iterators.begin();
       i != m_iterators.end(); ++i ) {
    RealTime start;
    ...
    // slice.  We include segments that end exactly when we
    // start, but not segments that start exactly when we end.
    bool active = ( start < endTime && end >= startTime );
    ( *i )->setActive( active, startTime );
  }

  // State variable to allow the outer (round-robin) loop to run
  // until the inner (segment) loop has nothing to do.
  ...
  //         rof
  //     rof
  //     Is this round-robin approach intended to spread the
  //     locks out? Is it really necessary? Prior to the
  //     auto-channels feature, this routine was called
  ...
  do {
    ...
    for( size_t i = 0; i < m_iterators.size(); ++i ) {
      ...
      if( !event || !event->isValid() ) continue;
      ...
      // If this event starts prior to the end of the slice, take
      // it.
...
void MappedBufMetaIterator::resetIteratorForSegment(
    std::shared_ptr<MappedEventBuffer> mappedEventBuffer,
    bool                               immediate ) {
  // For each segment
  for( SegmentIterators::iterator i = m_iterators.begin();
...
void MappedBufMetaIterator::getAudioEvents(
  ...
  for( MappedSegments::iterator i = m_segments.begin();
    ...
    while( !iter.atEnd() ) {
      ...
      if( event.getType() != MappedEvent::Audio ) continue;
      ...
      // If we're in solo mode and this event isn't on the solo
      // track, try the next event.
      ...
      // ??? Why does this need to contain copies?  Can we
      // simplify
