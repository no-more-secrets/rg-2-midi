...
bool MappedEventBuffer::refresh() {
  bool resized = false;
...
int MappedEventBuffer::capacity() const { return m_capacity; }

int MappedEventBuffer::size() const { return m_size; }

void MappedEventBuffer::reserve( int newSize ) {
...
#ifdef DEBUG_MAPPED_EVENT_BUFFER
  SEQUENCER_DEBUG << "MappedEventBuffer::reserve: Resized to "
                  << newSize << " events";
#endif
...
void MappedEventBuffer::resize( int newFill ) {
  m_size = newFill;
}

void MappedEventBuffer::mapAnEvent( MappedEvent *e ) {
  if( size() >= capacity() ) {
  ...
  getBuffer()[size()] = e;
  // Some mappers need this to be done now because they may
  // resize the buffer later, which will only copy the filled
  // part.
  resize( size() + 1 );
}

void MappedEventBuffer::doInsert( MappedInserterBase &inserter,
                                  MappedEvent &       evt,
                                  RealTime /*start*/,
                                  bool /*firstOutput*/ ) {
...
// ++prefix
MappedEventBuffer::iterator &MappedEventBuffer::iterator::
                             operator++() {
  int fill = m_s->size();
...
bool MappedEventBuffer::iterator::operator==(
    const iterator &it ) {
  return ( m_index == it.m_index ) && ( m_s == it.m_s );
}

void MappedEventBuffer::iterator::reset() { m_index = 0; }

MappedEvent MappedEventBuffer::iterator::operator*() {
...
MappedEvent *MappedEventBuffer::iterator::peek() const {
  // The lock formerly here has moved out to callers.

  // If we're at the end, return nullptr
  if( m_index >= m_s->size() ) return nullptr;

  // Otherwise return a pointer into the buffer.
  return &m_s->m_buffer[m_index];
}

bool MappedEventBuffer::iterator::atEnd() const {
  int size = m_s->size();
  return ( m_index >= size );
}

void MappedEventBuffer::iterator::doInsert(
    MappedInserterBase &inserter, MappedEvent &evt ) {
  // Get time when note starts sounding, eg for finding the
  // correct controllers.  It can't simply be event time, because
  // if we jumped into the middle of a long note, we'd wrongly
  // find the controller values as they are at the time the note
  // starts.
  if( evt.getEventTime() > m_currentTime )
    m_currentTime = evt.getEventTime();

  // Mapper does the actual insertion.
  getSegment()->doInsert( inserter, evt, m_currentTime,
                          !isReady() );
  setReady( true );
}
