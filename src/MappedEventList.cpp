...
MappedEventList::~MappedEventList() { clear(); }
...
MappedEventList &MappedEventList::operator=(
    const MappedEventList &c ) {
  if( &c == this ) return *this;

  clear();

  for( MappedEventList::const_iterator it = c.begin();
       it != c.end(); ++it )
    insert( new MappedEvent( **it ) );
...
void MappedEventList::merge( const MappedEventList &mC ) {
  for( MappedEventList::const_iterator it = mC.begin();
       it != mC.end(); ++it )
    insert( new MappedEvent( **it ) ); // deep copy
}

void MappedEventList::clear() {
