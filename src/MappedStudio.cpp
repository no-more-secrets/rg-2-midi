...
#ifdef DEBUG_MAPPEDSTUDIO
  std::cerr << "Acquiring MappedStudio container lock at "
            << file << ":" << line << ": count "
            << approxLockCount++ << std::endl;
#else
  (void)file;
  (void)line;
#endif
...
static inline void releaseLock( const char *file, int line ) {
  ...
  std::cerr << "Released container lock at " << file << ":"
            << line << ": count " << --approxLockCount
            << std::endl;
#else
  (void)file;
  (void)line;
...
const MappedObjectProperty MappedObject::Instrument =
    "instrument";
const MappedObjectProperty MappedObject::Position = "position";

const MappedObjectProperty
    MappedConnectableObject::ConnectionsIn = "connectionsIn";
const MappedObjectProperty
    MappedConnectableObject::ConnectionsOut = "connectionsOut";

const MappedObjectProperty MappedAudioFader::Channels =
    "channels";
const MappedObjectProperty MappedAudioFader::FaderLevel =
    "faderLevel";
const MappedObjectProperty MappedAudioFader::FaderRecordLevel =
    "faderRecordLevel";
const MappedObjectProperty MappedAudioFader::Pan = "pan";
const MappedObjectProperty MappedAudioFader::InputChannel =
    "inputChannel";
...
const MappedObjectProperty MappedAudioInput::InputNumber =
    "inputNumber";

const MappedObjectProperty MappedPluginSlot::Identifier =
    "identifier";
const MappedObjectProperty MappedPluginSlot::PluginName =
    "pluginname";
const MappedObjectProperty MappedPluginSlot::Label  = "label";
const MappedObjectProperty MappedPluginSlot::Author = "author";
const MappedObjectProperty MappedPluginSlot::Copyright =
    "copyright";
const MappedObjectProperty MappedPluginSlot::Category =
    "category";
const MappedObjectProperty MappedPluginSlot::PortCount =
    "portcount";
const MappedObjectProperty MappedPluginSlot::Ports = "ports";
const MappedObjectProperty MappedPluginSlot::Instrument =
    "instrument";
const MappedObjectProperty MappedPluginSlot::Position =
    "position";
const MappedObjectProperty MappedPluginSlot::Bypassed =
    "bypassed";
const MappedObjectProperty MappedPluginSlot::Programs =
    "programs";
const MappedObjectProperty MappedPluginSlot::Program = "program";
const MappedObjectProperty MappedPluginSlot::Configuration =
    "configuration";

const MappedObjectProperty MappedPluginPort::PortNumber =
    "portnumber";
...
const MappedObjectProperty MappedPluginPort::DisplayHint =
    "displayhint";
...
void MappedObject::addChild( MappedObject *object ) {
...
void MappedObject::removeChild( MappedObject *object ) {
  std::vector<MappedObject *>::iterator it = m_children.begin();
  for( ; it != m_children.end(); ++it ) {
    if( ( *it ) == object ) {
      m_children.erase( it );
...
//
MappedObjectPropertyList MappedObject::getChildren() {
  MappedObjectPropertyList              list;
  std::vector<MappedObject *>::iterator it = m_children.begin();
  for( ; it != m_children.end(); ++it )
    list.push_back( std::to_string( ( *it )->getId() ) );
...
void MappedObject::destroyChildren() {
...
//
void MappedObject::destroy() {
  MappedObject *studioObject = getParent();
  while( !dynamic_cast<MappedStudio *>( studioObject ) )
    studioObject = studioObject->getParent();

  MappedStudio *studio =
      dynamic_cast<MappedStudio *>( studioObject );

  // The destroy method on each child calls studio->clearObject,
  // which calls back on the parent (in this case us) to remove
  // the child.  (That's necessary for the case of destroying a
  // plugin, where we need to remove it from its plugin manager
  // -- etc.)  So we don't want to be iterating over m_children
  // here, as it will change from under us.

  std::vector<MappedObject *> children = m_children;
  m_children.clear();

  std::vector<MappedObject *>::iterator it = children.begin();
  for( ; it != children.end(); ++it ) { ( *it )->destroy(); }
...
MappedStudio::MappedStudio()
  : MappedObject( nullptr, "MappedStudio", Studio, 0 ),
    m_runningObjectId( 1 ) {
...
// Object factory
//
MappedObject *MappedStudio::createObject(
    MappedObjectType type ) {
  GET_LOCK;
...
MappedObject *MappedStudio::createObject( MappedObjectType type,
                                          MappedObjectId   id ) {
  GET_LOCK;
  ...
  if( mO ) {
#ifdef DEBUG_MAPPEDSTUDIO
    std::cerr << "Adding object " << id << " to category "
              << type << std::endl;
#endif
...
MappedObject *MappedStudio::getObjectOfType(
    MappedObjectType type ) {
...
std::vector<MappedObject *> MappedStudio::getObjectsOfType(
    MappedObjectType type ) {
...
unsigned int MappedStudio::getObjectCount(
    MappedObjectType type ) {
...
bool MappedStudio::destroyObject( MappedObjectId id ) {
  GET_LOCK;
...
bool MappedStudio::connectObjects( MappedObjectId mId1,
                                   MappedObjectId mId2 ) {
  GET_LOCK;
  ...
  MappedConnectableObject *obj1 =
      dynamic_cast<MappedConnectableObject *>(
          getObjectById( mId1 ) );
...
bool MappedStudio::disconnectObjects( MappedObjectId mId1,
                                      MappedObjectId mId2 ) {
  GET_LOCK;
...
bool MappedStudio::disconnectObject( MappedObjectId mId ) {
  GET_LOCK;
...
bool MappedStudio::clearObject( MappedObjectId id ) {
  bool rv = false;
...
MappedObjectPropertyList MappedStudio::getPropertyList(
...
bool MappedStudio::getProperty( const MappedObjectProperty &,
                                MappedObjectValue & ) {
  return false;
}

MappedObject *MappedStudio::getObjectById( MappedObjectId id ) {
  GET_LOCK;
  MappedObject *rv = nullptr;
...
MappedObject *MappedStudio::getObjectByIdAndType(
    MappedObjectId id, MappedObjectType type ) {
  GET_LOCK;
  MappedObject *rv = nullptr;

  MappedObjectCategory &         category = m_objects[type];
  MappedObjectCategory::iterator i        = category.find( id );
  if( i != category.end() ) { rv = i->second; }

  RELEASE_LOCK;
  return rv;
}

MappedObject *MappedStudio::getFirst( MappedObjectType type ) {
  return getObjectOfType( type );
}

MappedObject *MappedStudio::getNext( MappedObject *object ) {
  GET_LOCK;
...
void MappedStudio::setProperty(
    const MappedObjectProperty &property,
    MappedObjectValue /*value*/ ) {
  if( property == "" ) {}
}

MappedAudioFader *MappedStudio::getAudioFader(
    InstrumentId id ) {
  GET_LOCK;
  ...
  for( MappedObjectCategory::iterator i = category.begin();
    ...
    MappedAudioFader *fader =
        dynamic_cast<MappedAudioFader *>( i->second );
...
MappedAudioBuss *MappedStudio::getAudioBuss( int bussNumber ) {
  GET_LOCK;
  ...
  for( MappedObjectCategory::iterator i = category.begin();
    ...
    MappedAudioBuss *buss =
        dynamic_cast<MappedAudioBuss *>( i->second );
...
MappedAudioInput *MappedStudio::getAudioInput(
    int inputNumber ) {
  GET_LOCK;
  ...
  for( MappedObjectCategory::iterator i = category.begin();
       i != category.end(); ++i ) {
    MappedAudioInput *input =
        dynamic_cast<MappedAudioInput *>( i->second );
...
MappedConnectableObject::MappedConnectableObject(
    MappedObject *parent, const std::string &name,
    MappedObjectType type, MappedObjectId id )
  : MappedObject( parent, name, type, id ) {}

MappedConnectableObject::~MappedConnectableObject() {}

void MappedConnectableObject::setConnections(
    ConnectionDirection dir, MappedObjectValueList conns ) {
  if( dir == In )
...
void MappedConnectableObject::addConnection(
    ConnectionDirection dir, MappedObjectId id ) {
  MappedObjectValueList &list =
...
void MappedConnectableObject::removeConnection(
    ConnectionDirection dir, MappedObjectId id ) {
  MappedObjectValueList &list =
      ( dir == In ? m_connectionsIn : m_connectionsOut );
...
MappedObjectValueList MappedConnectableObject::getConnections(
...
MappedAudioFader::MappedAudioFader( MappedObject *    parent,
  ...
  : MappedConnectableObject( parent, "MappedAudioFader",
                             AudioFader, id ),
    m_level( 0.0 ), // dB
    m_recordLevel( 0.0 ),
    m_instrumentId( 0 ),
...
MappedObjectPropertyList MappedAudioFader::getPropertyList(
    const MappedObjectProperty &property ) {
  MappedObjectPropertyList list;
  ...
  if( property == "" ) {
    ...
    list.push_back( std::to_string( m_instrumentId ) );
  } else if( property == MappedAudioFader::FaderLevel ) {
    list.push_back( std::to_string( m_level ) );
  } else if( property == MappedAudioFader::FaderRecordLevel ) {
    list.push_back( std::to_string( m_recordLevel ) );
  } else if( property == MappedAudioFader::Channels ) {
    list.push_back( std::to_string( m_channels ) );
  } else if( property == MappedAudioFader::InputChannel ) {
    list.push_back( std::to_string( m_inputChannel ) );
  } else if( property == MappedAudioFader::Pan ) {
    list.push_back( std::to_string( m_pan ) );
  } else if( property ==
             MappedConnectableObject::ConnectionsIn ) {
    MappedObjectValueList::const_iterator it =
        m_connectionsIn.begin();

    for( ; it != m_connectionsIn.end(); ++it ) {
      list.push_back( std::to_string( *it ) );
    }
  } else if( property ==
             MappedConnectableObject::ConnectionsOut ) {
    MappedObjectValueList::const_iterator it =
        m_connectionsOut.begin();
...
bool MappedAudioFader::getProperty(
    const MappedObjectProperty &property,
    MappedObjectValue &         value ) {
  if( property == FaderLevel ) {
    ...
    std::cerr << "MappedAudioFader::getProperty - "
              << "unsupported or non-scalar property"
              << std::endl;
#endif
...
void MappedAudioFader::setProperty(
    const MappedObjectProperty &property,
    MappedObjectValue           value ) {
...
MappedAudioBuss::MappedAudioBuss( MappedObject * parent,
                                  MappedObjectId id )
  : MappedConnectableObject( parent, "MappedAudioBuss",
                             AudioBuss, id ),
    m_bussId( 0 ),
    m_level( 0 ),
    m_pan( 0 ) {}
...
MappedObjectPropertyList MappedAudioBuss::getPropertyList(
    const MappedObjectProperty &property ) {
  MappedObjectPropertyList list;
  ...
  if( property == "" ) {
    ...
    list.push_back( std::to_string( m_bussId ) );
...
bool MappedAudioBuss::getProperty(
    const MappedObjectProperty &property,
    MappedObjectValue &         value ) {
  if( property == BussId ) {
    value = m_bussId;
...
void MappedAudioBuss::setProperty(
    const MappedObjectProperty &property,
    MappedObjectValue           value ) {
  bool updateLevels = false;
...
std::vector<InstrumentId> MappedAudioBuss::getInstruments() {
  ...
  for( std::vector<MappedObject *>::iterator i = objects.begin();
       i != objects.end(); ++i ) {
    MappedAudioFader *fader =
        dynamic_cast<MappedAudioFader *>( *i );
    if( fader ) {
      MappedObjectValueList connections =
          fader->getConnections( MappedConnectableObject::Out );
      if( !connections.empty() &&
          ( *connections.begin() == getId() ) ) {
...
MappedAudioInput::MappedAudioInput( MappedObject * parent,
                                    MappedObjectId id )
  : MappedConnectableObject( parent, "MappedAudioInput",
                             AudioInput, id ) {}

MappedAudioInput::~MappedAudioInput() {}

MappedObjectPropertyList MappedAudioInput::getPropertyList(
    const MappedObjectProperty &property ) {
  MappedObjectPropertyList list;
  ...
  if( property == "" ) {
    ...
    list.push_back( std::to_string( m_inputNumber ) );
  }

  return list;
}

bool MappedAudioInput::getProperty(
    const MappedObjectProperty &property,
    MappedObjectValue &         value ) {
...
void MappedAudioInput::setProperty(
    const MappedObjectProperty &property,
    MappedObjectValue           value ) {
  if( property == InputNumber ) {
    m_inputNumber = value;
...
MappedPluginSlot::MappedPluginSlot( MappedObject * parent,
                                    MappedObjectId id )
  : MappedObject( parent, "MappedPluginSlot", PluginSlot, id ) {
#ifdef DEBUG_MAPPEDSTUDIO
  std::cerr << "MappedPluginSlot::MappedPluginSlot: id = " << id
            << std::endl;
#endif
}
...
#ifdef DEBUG_MAPPEDSTUDIO
  std::cerr << "MappedPluginSlot::~MappedPluginSlot: id = "
            << getId() << ", identifier = " << m_identifier
            << std::endl;
#endif
...
MappedObjectPropertyList MappedPluginSlot::getPropertyList(
    const MappedObjectProperty &property ) {
  MappedObjectPropertyList list;
  ...
  if( property == "" ) {
    ...
    if( studio ) {
      // std::stringList programs =
      //    studio->getSoundDriver()->getPluginInstancePrograms(
...
bool MappedPluginSlot::getProperty(
    const MappedObjectProperty &property,
    MappedObjectValue &         value ) {
...
bool MappedPluginSlot::getStringProperty(
    const MappedObjectProperty &property, std::string &value ) {
...
std::string MappedPluginSlot::getProgram( int bank,
                                          int program ) {
  MappedStudio *studio =
      dynamic_cast<MappedStudio *>( getParent() );

  if( studio ) {
    // return studio->getSoundDriver()->getPluginInstanceProgram(
    //    m_instrument, m_position, bank, program );
  }

  return std::string();
}

unsigned long MappedPluginSlot::getProgram( std::string name ) {
  MappedStudio *studio =
      dynamic_cast<MappedStudio *>( getParent() );

  if( studio ) {
    // return studio->getSoundDriver()->getPluginInstanceProgram(
    //    m_instrument, m_position, name );
  }

  return 0;
}

void MappedPluginSlot::setProperty(
    const MappedObjectProperty &property,
    MappedObjectValue           value ) {
  if( property == Instrument ) {
    ...
    if( studio ) {
      // studio->getSoundDriver()->setPluginInstanceBypass(
      //    m_instrument, m_position, m_bypassed );
    }
  }
}

void MappedPluginSlot::setStringProperty(
    const MappedObjectProperty &property, std::string value ) {
  if( property == Identifier ) {
    ...
    if( studio ) {
      ...
      //    // for the new plugin instance to be unloaded and
      //    then
      ...
      //    PluginFactory::instanceFor(m_identifier); if
      //    (!factory) {
      //        std::cerr << "WARNING:
      //        MappedPluginSlot::setProperty(identifier): No
      //        plugin factory for identifier " << m_identifier
      //        << "!" << std::endl; m_identifier = ""; return ;
      //    }
    ...
    if( studio ) {
      // studio->getSoundDriver()->setPluginInstanceProgram(
      //    m_instrument, m_position, value );
    }
  } else {
#ifdef DEBUG_MAPPEDSTUDIO
...
void MappedPluginSlot::setPropertyList(
    const MappedObjectProperty &    property,
  ...
  if( property == Configuration ) {
    ...
    for( MappedObjectPropertyList::const_iterator i =
             values.begin();
         i != values.end(); ++i ) {
      std::string key   = *i;
      std::string value = *++i;
      ...
      if( studio ) {
        // std::string rv =
        //    studio->getSoundDriver()->configurePlugin(
        //        m_instrument, m_position, key, value );
        // if( !rv.isEmpty() ) { throw( rv ); }
      }
    }
...
void MappedPluginSlot::setPort( unsigned long portNumber,
                                float         value ) {
  ...
  for( ; it != ports.end(); ++it ) {
    ...
    if( port &&
        (unsigned long)port->getPortNumber() == portNumber ) {
      port->setValue( value );
    }
  }
}

float MappedPluginSlot::getPort( unsigned long portNumber ) {
  std::vector<MappedObject *> ports          = getChildObjects();
  std::vector<MappedObject *>::iterator it   = ports.begin();
  MappedPluginPort *                    port = nullptr;
...
MappedPluginPort::MappedPluginPort( MappedObject * parent,
                                    MappedObjectId id )
  : MappedObject( parent, "MappedPluginPort", PluginPort, id ) {}

MappedPluginPort::~MappedPluginPort() {}

MappedObjectPropertyList MappedPluginPort::getPropertyList(
    const MappedObjectProperty &property ) {
  MappedObjectPropertyList list;
...
bool MappedPluginPort::getProperty(
    const MappedObjectProperty &property,
    MappedObjectValue &         value ) {
...
bool MappedPluginPort::getStringProperty(
    const MappedObjectProperty &property, std::string &value ) {
  if( property == Name ) {
    value = m_name;
  } else {
...
void MappedPluginPort::setValue( MappedObjectValue value ) {
...
float MappedPluginPort::getValue() const {
  ...
  if( slot ) {
    ...
    if( studio ) {
      ...
      // if (drv) {
      //    return
      //    drv->getPluginInstancePortValue(slot->getInstrument(),
      //                                           slot->getPosition(),
      //                                           m_portNumber);
      //}
    }
...
void MappedPluginPort::setProperty(
    const MappedObjectProperty &property,
    MappedObjectValue           value ) {
  if( property == PortNumber ) {
...
void MappedPluginPort::setStringProperty(
    const MappedObjectProperty &property, std::string value ) {
  if( property == Name ) {
    m_name = value;
  } else {
