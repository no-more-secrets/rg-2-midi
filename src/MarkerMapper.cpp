...
MarkerMapper::MarkerMapper( RosegardenDocument *doc )
...
void MarkerMapper::fillBuffer() {
  ...
  for( Composition::markerconstiterator i = marks.begin();
    ...
    RealTime    eventTime =
        comp.getElapsedRealTime( ( *i )->getTime() );
...
int MarkerMapper::calculateSize() {
  return m_doc->getComposition().getMarkers().size();
}

// Markers always "play".  When we don't want them, we simply
// don't include a marker mapper.
bool MarkerMapper::shouldPlay( MappedEvent * /*evt*/,
                               RealTime /*startTime*/ ) {
  return true;
}
