...
MidiDevice::MidiDevice( DeviceId id, InstrumentId ibase,
  ...
  : Device( id, name, Device::Midi ),
    ...
    m_direction( dir ),
    m_variationType( NoVariations ),
    m_librarian( std::pair<std::string, std::string>(
        "<none>", "<none>" ) ),
    m_allocator( new AllocateChannels( ChannelSetup::MIDI ) ) {
...
AllocateChannels *MidiDevice::getAllocator() {
  return m_allocator;
}

void MidiDevice::createInstruments( InstrumentId base ) {
  for( int i = 0; i < 16; ++i ) {
...
void MidiDevice::renameInstruments() {
  for( int i = 0; i < 16; ++i ) {
    auto res = std::string( "" ) + getName().c_str() + " #" +
               std::to_string( i + 1 ) +
               ( isPercussionNumber( i ) ? "[D]" : "" );
    m_instruments[i]->setName( res );
  }
}

void MidiDevice::generatePresentationList() {
  // Fill the presentation list for the instruments
  //
  m_presentationInstrumentList.clear();

  InstrumentList::iterator it;
  for( it = m_instruments.begin(); it != m_instruments.end();
       ++it ) {
...
void MidiDevice::generateDefaultControllers() {
  m_controlList.clear();

  static std::string controls[][9] = {
      {"Pan", Rosegarden::Controller::EventType, "<none>", "0",
       "127", "64", "10", "2", "0"},
      {"Chorus", Rosegarden::Controller::EventType, "<none>",
       "0", "127", "0", "93", "3", "1"},
      {"Volume", Rosegarden::Controller::EventType, "<none>",
       "0", "127", "100", "7", "1", "2"},
      {"Reverb", Rosegarden::Controller::EventType, "<none>",
       "0", "127", "0", "91", "3", "3"},
      {"Sustain", Rosegarden::Controller::EventType, "<none>",
       "0", "127", "0", "64", "4", "-1"},
      {"Expression", Rosegarden::Controller::EventType, "<none>",
       "0", "127", "100", "11", "2", "-1"},
      {"Modulation", Rosegarden::Controller::EventType, "<none>",
       "0", "127", "0", "1", "4", "-1"},
  ...
  for( size_t i = 0;
    ...
    Rosegarden::ControlParameter con(
        controls[i][0], controls[i][1], controls[i][2],
...
void MidiDevice::deviceToInstrControllerPush() {
  // Copy the instruments
  //
  ...
  for( ; iIt != m_instruments.end(); ++iIt ) {
    ...
    for( ; cIt != m_controlList.end(); ++cIt ) {
      ...
      if( isVisibleControlParameter( *cIt ) ) {
        ...
        ( *iIt )->setControllerValue( controllerValue,
                                      defaultValue );
      }
    }
  }
...
void MidiDevice::clearBankList() { m_bankList.clear(); }

void MidiDevice::clearProgramList() { m_programList.clear(); }

void MidiDevice::clearKeyMappingList() {
  m_keyMappingList.clear();
}

void MidiDevice::clearControlList() {
...
void MidiDevice::addProgram( const MidiProgram &prog ) {
  // Refuse duplicates
...
void MidiDevice::addBank( const MidiBank &bank ) {
  m_bankList.push_back( bank );
}

void MidiDevice::setMetronome( const MidiMetronome &metronome ) {
  delete m_metronome;
  m_metronome = new MidiMetronome( metronome );
}

BankList MidiDevice::getBanks( bool percussion ) const {
  BankList banks;
...
BankList MidiDevice::getBanksByMSB( bool     percussion,
                                    MidiByte msb ) const {
  BankList banks;
...
BankList MidiDevice::getBanksByLSB( bool     percussion,
                                    MidiByte lsb ) const {
  BankList banks;
...
const MidiBank *MidiDevice::getBankByName(
...
MidiByteList MidiDevice::getDistinctMSBs( bool percussion,
                                          int  lsb ) const {
  ...
  for( BankList::const_iterator it = m_bankList.begin();
    ...
    if( it->isPercussion() == percussion &&
        ( lsb == -1 || it->getLSB() == lsb ) )
      msbs.insert( it->getMSB() );
  }

  MidiByteList v;
  for( std::set<MidiByte>::iterator i = msbs.begin();
       i != msbs.end(); ++i ) {
    v.push_back( *i );
  }
...
MidiByteList MidiDevice::getDistinctLSBs( bool percussion,
                                          int  msb ) const {
  ...
  for( BankList::const_iterator it = m_bankList.begin();
    ...
    if( it->isPercussion() == percussion &&
        ( msb == -1 || it->getMSB() == msb ) )
      lsbs.insert( it->getLSB() );
  }

  MidiByteList v;
  for( std::set<MidiByte>::iterator i = lsbs.begin();
       i != lsbs.end(); ++i ) {
    v.push_back( *i );
  }
...
ProgramList MidiDevice::getPrograms(
    const MidiBank &bank ) const {
...
ProgramList MidiDevice::getPrograms0thVariation(
    bool percussion, const MidiBank &bank ) const {
...
std::string MidiDevice::getBankName(
    const MidiBank &bank ) const {
  for( BankList::const_iterator it = m_bankList.begin();
       it != m_bankList.end(); ++it ) {
...
void MidiDevice::addKeyMapping( const MidiKeyMapping &mapping ) {
  //!!! handle dup names
  m_keyMappingList.push_back( mapping );
}

const MidiKeyMapping *MidiDevice::getKeyMappingByName(
    const std::string &name ) const {
  for( KeyMappingList::const_iterator i =
           m_keyMappingList.begin();
...
const MidiKeyMapping *MidiDevice::getKeyMappingForProgram(
    const MidiProgram &program ) const {
  ProgramList::const_iterator it;

  for( it = m_programList.begin(); it != m_programList.end();
       ++it ) {
...
void MidiDevice::setKeyMappingForProgram(
    const MidiProgram &program, std::string mapping ) {
...
std::string MidiDevice::toXmlString() const {
  std::stringstream midiDevice;

  midiDevice << "    <device id=\"" << m_id << "\" name=\""
             << m_name << "\" direction=\""
             << ( m_direction == Play ? "play" : "record" )
             << "\" variation=\""
             << ( m_variationType == VariationFromLSB
                      ? "LSB"
  ...
  midiDevice << "        <librarian name=\""
             << encode( m_librarian.first ) << "\" email=\""
             << encode( m_librarian.second ) << "\"/>"
             << std::endl;
  ...
  if( m_metronome ) {
    ...
    midiDevice << "        <metronome "
               ...
               << "\" "
               << "barpitch=\""
               << (int)m_metronome->getBarPitch() << "\" "
               << "beatpitch=\""
               << (int)m_metronome->getBeatPitch() << "\" "
               << "subbeatpitch=\""
               << (int)m_metronome->getSubBeatPitch() << "\" "
               << "depth=\"" << (int)m_metronome->getDepth()
               << "\" "
               << "barvelocity=\""
               << (int)m_metronome->getBarVelocity() << "\" "
               << "beatvelocity=\""
               << (int)m_metronome->getBeatVelocity() << "\" "
               << "subbeatvelocity=\""
               << (int)m_metronome->getSubBeatVelocity()
               << "\"/>" << std::endl
               << std::endl;
  }
  ...
  for( it = m_bankList.begin(); it != m_bankList.end(); ++it ) {
    ...
    //
    for( pt = m_programList.begin(); pt != m_programList.end();
         ++pt ) {
  ...
  for( cIt = m_controlList.begin(); cIt != m_controlList.end();
       ++cIt )
  ...
  // Add instruments
  //
  for( iit = m_instruments.begin(); iit != m_instruments.end();
       ++iit )
    midiDevice << ( *iit )->toXmlString();

  KeyMappingList::const_iterator kit;

  for( kit = m_keyMappingList.begin();
       kit != m_keyMappingList.end(); ++kit ) {
    midiDevice << "        <keymapping "
...
//
InstrumentList MidiDevice::getAllInstruments() const {
  return m_instruments;
}

// Omitting special system Instruments
//
InstrumentList MidiDevice::getPresentationInstruments() const {
  return m_presentationInstrumentList;
}

void MidiDevice::addInstrument( Instrument *instrument ) {
  // Check / add controls to this instrument
  // No controls are pushed when called from the contructor since
  ...
  for( ; it != m_controlList.end(); ++it ) {
    if( isVisibleControlParameter( *it ) ) {
      ...
      try {
        ...
        instrument->setControllerValue( controller,
                                        it->getDefault() );
      }
    }
  }
...
std::string MidiDevice::getProgramName(
    const MidiProgram &program ) const {
  ProgramList::const_iterator it;
...
void MidiDevice::replaceBankList( const BankList &bankList ) {
  m_bankList = bankList;
}

void MidiDevice::replaceProgramList(
    const ProgramList &programList ) {
  m_programList = programList;
}

void MidiDevice::replaceKeyMappingList(
...
//
void MidiDevice::mergeBankList( const BankList &bankList ) {
  BankList::const_iterator it;
  BankList::iterator       oIt;
  bool                     clash = false;

  for( it = bankList.begin(); it != bankList.end(); ++it ) {
    for( oIt = m_bankList.begin(); oIt != m_bankList.end();
         ++oIt ) {
...
void MidiDevice::mergeProgramList(
  ...
  for( it = programList.begin(); it != programList.end();
       ++it ) {
    for( oIt = m_programList.begin(); oIt != m_programList.end();
         ++oIt ) {
      if( it->partialCompare( *oIt ) ) {
        clash = true;
        break;
...
void MidiDevice::mergeKeyMappingList(
    const KeyMappingList &keyMappingList ) {
  KeyMappingList::const_iterator it;
  KeyMappingList::iterator       oIt;
  bool                           clash = false;

  for( it = keyMappingList.begin(); it != keyMappingList.end();
       ++it ) {
    for( oIt = m_keyMappingList.begin();
         oIt != m_keyMappingList.end(); ++oIt ) {
...
void MidiDevice::addControlParameter(
    const ControlParameter &con, bool propagateToInstruments ) {
  if( isUniqueControlParameter(
          con ) ) { // Don't allow duplicates
    m_controlList.push_back( con );
    if( propagateToInstruments &&
        isVisibleControlParameter( con ) ) {
...
void MidiDevice::addControlParameter(
    const ControlParameter &con, int index,
  ...
  // Rebuild the ControlList entry by entry, placing CON at
  // INDEX. For entry INDEX we do two push_back's, for other
  // entries we do one.
...
bool MidiDevice::removeControlParameter( int index ) {
...
bool MidiDevice::modifyControlParameter(
    const ControlParameter &con, int index ) {
  if( index < 0 || index > (int)m_controlList.size() )
    return false;
...
void MidiDevice::replaceControlParameters(
    const ControlList &con ) {
  // Clear down instrument controllers in preparation for
  // replace.
  InstrumentList           insList = getAllInstruments();
  InstrumentList::iterator iIt     = insList.begin();
...
// type must be unique or in the case of Controller::EventType
// the ControllerValue must be unique.
//
// Controllers (Control type)
//
//
bool MidiDevice::isUniqueControlParameter(
    const ControlParameter &con ) const {
  return findControlParameter( con.getType(),
                               con.getControllerValue() ) ==
         nullptr;
}

const ControlParameter *MidiDevice::findControlParameter(
    std::string type, MidiByte conNumber ) const {
  ControlList::const_iterator it = m_controlList.begin();
...
bool MidiDevice::isVisibleControlParameter(
    const ControlParameter &con ) const {
  return ( con.getIPBPosition() > -1 );
}

void MidiDevice::addControlToInstrument(
    const ControlParameter &con ) {
  if( !isVisibleControlParameter( con ) ) { return; }

  // Run through all of this devices instruments and add default
  // controls and values to them.
  InstrumentList           insList = getAllInstruments();
  InstrumentList::iterator iIt     = insList.begin();
...
void MidiDevice::removeControlFromInstrument(
    const ControlParameter &con ) {
  InstrumentList           insList = getAllInstruments();
  InstrumentList::iterator iIt     = insList.begin();
...
ControlList MidiDevice::getIPBControlParameters() const {
  ControlList retList;
...
ControlParameter *MidiDevice::getControlParameter( int index ) {
  if( index >= 0 && ( (unsigned int)index ) <
                        (unsigned int)m_controlList.size() )
    return &m_controlList[index];

  return nullptr;
}

const ControlParameter *MidiDevice::getControlParameter(
    int index ) const {
  return ( (MidiDevice *)this )->getControlParameter( index );
}

ControlParameter *MidiDevice::getControlParameter(
    const std::string &  type,
    Rosegarden::MidiByte controllerValue ) {
  ControlList::iterator it = m_controlList.begin();
...
const ControlParameter *MidiDevice::getControlParameter(
  ...
  return ( (MidiDevice *)this )
      ->getControlParameter( type, controllerValue );
}
