...
// long MidiFile::midiBytesToLong( const std::string &bytes ) {
//  if( bytes.length() != 4 ) {
//    RG_WARNING << "midiBytesToLong(): WARNING: Wrong length for
//    "
//                  "long data ("
//               << bytes.length() << ", should be 4)";

//    // TRANSLATOR: "long" is a C++ data type
//    throw Exception( qstrtostr( QObject::tr(
//        "Wrong length for long data in MIDI stream" ) ) );
//  }

//  long longRet =
//      static_cast<long>( static_cast<MidiByte>( bytes[0] ) )
//          << 24 |
...
// int MidiFile::midiBytesToInt( const std::string &bytes ) {
//  if( bytes.length() != 2 ) {
//    RG_WARNING << "midiBytesToInt(): WARNING: Wrong length for
//    "
//                  "int data ("
...
//    throw Exception( qstrtostr( QObject::tr(
//        "Wrong length for int data in MIDI stream" ) ) );
//  }
...
// MidiByte MidiFile::read( std::ifstream *midiFile ) {
//  return static_cast<MidiByte>( read( midiFile, 1 )[0] );
//}

// std::string MidiFile::read( std::ifstream *midiFile,
//                            unsigned long  numberOfBytes ) {
//  if( midiFile->eof() ) {
//    RG_WARNING << "read(): MIDI file EOF - got 0 bytes out of "
//               << numberOfBytes;
...
//  // For each track section we can read only m_trackByteCount
//  // bytes.
//  if( m_decrementCount &&
//      numberOfBytes >
//          static_cast<unsigned long>( m_trackByteCount ) ) {
//    RG_WARNING << "read(): Attempt to get more bytes than "
//                  "allowed on Track ("
//               << numberOfBytes << " > " << m_trackByteCount
//               << ")";
...
//  // ??? Reading one byte at a time is terribly slow.  However,
//  // this
...
//    RG_WARNING << "read(): Attempt to read past file end - got
//    "
...
//  if( m_decrementCount ) m_trackByteCount -= numberOfBytes;
...
//        throw Exception(
//            qstrtostr( QObject::tr( "Cancelled by user" ) ) );
...
// long MidiFile::readNumber( std::ifstream *midiFile,
//                           int            firstByte ) {
//  if( midiFile->eof() ) return 0;
...
// void MidiFile::findNextTrack( std::ifstream *midiFile ) {
//  // Conforms to recommendation in the MIDI spec, section 4,
//  page
//  // 3: "Your programs should /expect/ alien chunks and treat
//  // them as if they weren't there."  (Emphasis theirs.)
...
//    RG_DEBUG << "findNextTrack(): skipping alien chunk.  Type:"
//             << chunkType;

//    // Alien chunk encountered, initiate evasive maneuvers
//    (skip
//    // it).
...
// bool MidiFile::read( const QString &filename ) {
//  RG_DEBUG << "read(): filename = " << filename;

//  clearMidiComposition();

//  // Open the file
//  std::ifstream *midiFile = new std::ifstream(
//      filename.toLocal8Bit(), std::ios::in | std::ios::binary
//      );
...
//  // The parsing process throws string exceptions back up here
//  if
//  // we run into trouble which we can then pass back out to
//  // whomever called us using m_error and a nice bool.
//  try {
//    // Parse the MIDI header first.
//    parseHeader( midiFile );

//    // For each track chunk in the MIDI file.
//    for( unsigned track = 0; track < m_numberOfTracks;
//         ++track ) {
...
//      RG_DEBUG << "read(): Track has " << m_trackByteCount
//               << " bytes";
...
//    RG_WARNING << "read() - caught exception - "
//               << e.getMessage();
...
// void MidiFile::parseHeader( std::ifstream *midiFile ) {
...
//    throw Exception(
//        qstrtostr( QObject::tr( "Not a MIDI file" ) ) );
//  }

//  if( midiHeader.compare( 0, 4, MIDI_FILE_HEADER ) != 0 ) {
//    RG_WARNING
//        << "parseHeader() - file header not found or
//        malformed";
//    throw Exception(
//        qstrtostr( QObject::tr( "Not a MIDI file" ) ) );
//  }
...
//  m_numberOfTracks =
//      midiBytesToInt( midiHeader.substr( 10, 2 ) );
//  m_timingDivision =
//      midiBytesToInt( midiHeader.substr( 12, 2 ) );
//  m_timingFormat = MIDI_TIMING_PPQ_TIMEBASE;

//  if( m_format == MIDI_SEQUENTIAL_TRACK_FILE ) {
//    RG_WARNING << "parseHeader() - can't load sequential track
//    "
//                  "(Format 2) MIDI file";
//    throw Exception( qstrtostr(
//        QObject::tr( "Unexpected MIDI file format" ) ) );
//  }
...
//    // MIDI spec section 4, page 5: "[...] more parameters may
//    be
//    // added to the MThd chunk in the future: it is important
//    to
...
// void MidiFile::parseTrack( std::ifstream *midiFile ) {
//  // The term "Track" is overloaded in this routine.  The first
//  // meaning is a track in the MIDI file.  That is what this
//  // routine processes.  A single track from a MIDI file.  The
//  // second meaning is a track in m_midiComposition.  This is
//  the
//  // most common usage. To improve clarity, "MIDI file track"
//  // will be used to refer to the first sense of the term.
//  // Occasionally, "m_midiComposition track" will be used to
//  // refer to the second sense.

//  // Absolute time of the last event on any track.
//  unsigned long eventTime = 0;

//  // lastTrackNum is the track for all events provided they're
//  // all on the same channel.  If we find events on more than
//  one
//  // channel, we increment lastTrackNum and record the mapping
//  // from channel to trackNum in channelToTrack.
//  TrackId lastTrackNum = m_midiComposition.size();

//  // MIDI channel to m_midiComposition track.
//  // Note: This would be a vector<TrackId> but TrackId is
//  // unsigned
//  //       and we need -1 to indicate "not yet used"
//  std::vector<int> channelToTrack( 16, -1 );

//  // This is used to store the last absolute time found on each
//  // track, allowing us to modify delta-times correctly when
//  // separating events out from one to multiple tracks
//  std::map<int [>track*/, unsigned long /*lastTime<]>
//      lastEventTime;
//  lastEventTime[lastTrackNum] = 0;

//  // Meta-events don't have a channel, so we place them in a
//  // fixed track number instead
//  TrackId metaTrack = lastTrackNum;

//  std::string trackName = defaultTrackName;
//  std::string instrumentName;

//  // Remember the last non-meta status byte (-1 if we haven't
//  // seen one)
//  int runningStatus = -1;

//  bool firstTrack = true;

//  RG_DEBUG << "parseTrack(): last track number is "
//           << lastTrackNum;

//  // While there is still data to read in the MIDI file track.
//  // Why "m_trackByteCount > 1" instead of "m_trackByteCount >
//  // 0"?  Since no event and its associated delta time can fit
//  in
//  // just one byte, a single remaining byte in the MIDI file
//  // track has to be padding. This is obscure and non-standard,
//  // but such files do exist; ordinarily there should be no
//  bytes
//  // in the MIDI file track after the last event.
...
//      RG_DEBUG << "parseTrack(): have new status byte"
//               << QString( "0x%1" ).arg( midiByte, 0, 16 );
...
//      RG_DEBUG << "parseTrack(): using running status (byte "
//               << QString( "0x%1" ).arg( midiByte, 0, 16 )
//               << " found)";
//    }
...
//      RG_DEBUG << "parseTrack(): Meta event of type "
//               << QString( "0x%1" ).arg( metaEventCode, 0, 16 )
//               << " and " << messageLength << " bytes found";

//      std::string metaMessage = read( midiFile, messageLength
//      );

//      // Compute the difference between this event and the
//      // previous event on this track.
//      deltaTime = eventTime - lastEventTime[metaTrack];
//      // Store the absolute time of the last event on this
//      track. lastEventTime[metaTrack] = eventTime;

//      // create and store our event
//      MidiEvent *e =
//          new MidiEvent( deltaTime, MIDI_FILE_META_EVENT,
//                         metaEventCode, metaMessage );
...
//    // If this channel hasn't been seen yet in this MIDI file
//    // track
//    if( channelToTrack[channel] == -1 ) {
//      // If this is the first m_midiComposition track we've
//      // used
//      if( firstTrack ) {
//        // We've already allocated an m_midiComposition track
//        for
...
//      RG_DEBUG << "parseTrack(): new channel map entry: channel
//      "
//               << channel << " -> track " << lastTrackNum;
...
//        RG_DEBUG << "parseTrack(): track number for channel "
//                 << channel << " is " << trackNum;
...
//          RG_DEBUG << "parseTrack(): MIDI event for channel "
//                   << channel + 1 << " (track " << trackNum
//                   << ')';
//          RG_DEBUG << *midiEvent;
//        }
//      } break;

//      case MIDI_PROG_CHANGE: // These events have a single data
//                             // byte.
//      case MIDI_CHNL_AFTERTOUCH: {
//        RG_DEBUG << "parseTrack(): Program change (Cn) or "
//                    "channel aftertouch (Dn): time "
//                 << deltaTime << ", code "
//                 << QString( "0x%1" ).arg( statusByte, 0, 16 )
//                 << ", data " << (int)data1 << " going to track
//                 "
//                 << trackNum;
...
//        RG_DEBUG << "parseTrack(): SysEx of " << messageLength
//                 << " bytes found";
...
//          RG_WARNING << "parseTrack() - malformed or "
//                        "unsupported SysEx type";
//          continue;
//        }
...
//            deltaTime, MIDI_SYSTEM_EXCLUSIVE, sysex );
//        m_midiComposition[trackNum].push_back( midiEvent );
//      } break;

//      case MIDI_END_OF_EXCLUSIVE:
//        RG_WARNING << "parseTrack() - Found a stray "
//                      "MIDI_END_OF_EXCLUSIVE";
//        break;

//      default:
//        RG_WARNING
//            << "parseTrack() - Unsupported MIDI Status Byte:  "
//            << QString( "0x%1" ).arg( statusByte, 0, 16 );
//        break;
//    }
//  }

//  // If the MIDI file track has a padding byte, read and
//  discard
//  // it to make sure that the stream is positioned at the
//  // beginning of the following MIDI file track (if there is
//  // one.)
//  if( m_trackByteCount == 1 ) midiFile->ignore();
...
// bool MidiFile::convertToRosegarden( const QString & filename,
//                                    RosegardenDocument *doc ) {
//  Profiler profiler( "MidiFile::convertToRosegarden" );

//  // Read the MIDI file into m_midiComposition.
//  if( !read( filename ) ) return false;

//  if( m_midiComposition.size() != m_trackNames.size() ) {
//    RG_WARNING << "convertToRosegarden(): m_midiComposition and
//    "
//                  "m_trackNames size mismatch";
//    return false;
//  }
...
//  RG_DEBUG << "convertToRosegarden(): MIDI COMP SIZE = "
//           << m_midiComposition.size();

//  if( m_timingFormat == MIDI_TIMING_SMPTE ) {
//    // If we have SMPTE timecode (i.e. seconds and frames,
//    // roughly equivalent to RealTime timestamps) then we need
//    to
//    // add any tempo change events _first_ before we can do any
//    // conversion from SMPTE to musical time, because this
//    // conversion depends on tempo.  Also we need to add tempo
//    // changes in time order, not track order, because their
//    own
//    // timestamps depend on all prior tempo changes.

//    // In principle there's no harm in doing this for non-SMPTE
//    // files as well, but there's no gain and it's probably not
//    a
...
//      // For each event
//      for( MidiTrack::const_iterator midiEvent =
//               m_midiComposition[i].begin();
//           midiEvent != m_midiComposition[i].end();
//           ++midiEvent ) {
//        if( ( *midiEvent )->isMeta() ) {
//          // If we have a set tempo meta-event
//          if( ( *midiEvent )->getMetaEventCode() ==
//              MIDI_SET_TEMPO ) {
//            MidiByte m0    = ( *midiEvent
//            )->getMetaMessage()[0]; MidiByte m1    = (
//            *midiEvent )->getMetaMessage()[1]; MidiByte m2    =
//            ( *midiEvent )->getMetaMessage()[2]; long     tempo
//            = ( ( ( m0 << 8 ) + m1 ) << 8 ) + m2; if( tempo !=
//            0 ) {
...
//          RealTime::frame2RealTime( i->first,
//                                    m_fps * m_subframes ) );
//      composition.addTempoAtTime( t, i->second );
//    }
//  }

//  const int rosegardenPPQ = Note( Note::Crotchet
//  ).getDuration(); const int midiFilePPQ =
//      m_timingDivision ? m_timingDivision : 96;
//  // Conversion factor.
...
//  // For each track
//  // ??? BIG loop.
//  for( TrackId trackId = 0; trackId < m_midiComposition.size();
//       ++trackId ) {
//    if( m_progressDialog ) {
//      if( m_progressDialog->wasCanceled() ) {
//        m_error =
...
//      // 20% total in file import itself (see read()) and then
//      // 80% split over the tracks.
//      int progressValue =
//          20 + static_cast<int>( 80.0 * trackId /
//                                 m_midiComposition.size() );

//      // RG_DEBUG << "convertToRosegarden() progressValue: " <<
//      // progressValue;
...
//    for( MidiTrack::iterator eventIter =
...
//    // Consolidate NOTE ON and NOTE OFF events into NOTE ON
//    // events with a duration.
//    consolidateNoteEvents( trackId );

//    InstrumentId instrumentId = MidiInstrumentBase;

//    // If this track has a channel, use that channel's
//    // instrument.
//    if( m_trackChannelMap.find( trackId ) !=
//        m_trackChannelMap.end() ) {
//      instrumentId =
//          MidiInstrumentBase + m_trackChannelMap[trackId];
//    }
...
//    RG_DEBUG
//        << "convertToRosegarden(): New Rosegarden track: id = "
//        << rosegardenTrackId
//        << ", instrument = " << instrumentId;

//    // Used for filling the space between events with rests.
//    Also
//    // used for padding the end of the track with rests.
...
//        rosegardenTime = static_cast<timeT>( midiAbsoluteTime *
//                                             midiToRgTime );
//        rosegardenDuration =
...
//        rosegardenDuration =
//            composition.getElapsedTimeForRealTime(
//                RealTime::frame2RealTime(
//                    midiAbsoluteTime + midiDuration,
//                    m_fps * m_subframes ) ) -
//            rosegardenTime;
//      }

//      RG_DEBUG << "convertToRosegarden(): MIDI file import: "
//                  "event time "
//               << rosegardenTime << ", duration "
//               << rosegardenDuration << ", event type "
//               << QString( "0x%1" ).arg(
//                      midiEvent.getMessageType(), 0, 16 )
//               << ", previous max time " << maxTime
//               << ", potential max time "
//               << ( rosegardenTime + rosegardenDuration )
//               << ", ev raw time " << midiAbsoluteTime
//               << ", ev raw duration " << midiDuration
//               << ", crotchet "
//               << Note( Note::Crotchet ).getDuration()
//               << ", midiToRgTime " << midiToRgTime << ", sfps
//               "
//               << m_fps * m_subframes;
...
//        // left with rests.
//        // ??? But if we have a loop with a precise start and
//        // end,
//        //     this ruins the start.  This would preserve it:
//        //       endOfLastNote = rosegardenTime;
//        //     If the user wants the beginning on a bar, they
//        can
//        //     easily do that.  We shouldn't be modifying
//        things. endOfLastNote =
//            composition.getBarStartForTime( rosegardenTime );
//      }

//      // The incoming midiEvent is transformed into this
//      // rosegardenEvent.
...
//          case MIDI_LYRIC: {
//            std::string text = midiEvent.getMetaMessage();
//            rosegardenEvent  = Text( text, Text::Lyric )
...
//              RG_WARNING << "convertToRosegarden(): Zero-length
//              "
//                            "track encountered";
...
//            // If there's space between the last note and the
//            // track end, fill it out with rests.
//            if( endOfLastNote < trackEndTime ) {
//              // If there's nothing in the segment yet, then we
//              // shouldn't fill with rests because we don't
//              want
//              // to cause the otherwise empty segment to be
//              // created.
//              if( !segment->empty() )
...
//                // RG_DEBUG << "convertToRosegarden():
//                converted
//                // MIDI tempo " << midiTempo << " to Rosegarden
//                // tempo " << rosegardenTempo;
//                composition.addTempoAtTime( rosegardenTime,
//                                            rosegardenTempo );
//              }
//            }
//            break;
...
//            denominator = 1
//                          << static_cast<int>( metaMessage[1]
//                          );

//            // A MIDI time signature has additional information
//            // that we ignore.  From the spec, section 4 page
//            10:
//            // metaMessage[2] "expresses the number of *MIDI
//            // clocks*
//            //   in a metronome tick."
//            // metaMessage[3] "expresses the number of notated
//            //   32nd-notes in what MIDI thinks of as a
...
//            // dependent.  Casting to signed char guarantees
//            the
//            // correct results on all platforms.

//            int accidentals = abs(
//                static_cast<signed char>( metaMessage[0] ) );
//            bool isSharp = ( static_cast<signed char>(
//                                 metaMessage[0] ) >= 0 );
...
//            try {
//              rosegardenEvent =
//                  Rosegarden::Key( accidentals, isSharp,
//                                   isMinor )
//                      .getAsEvent( rosegardenTime );
//            } catch( ... ) {
//              RG_WARNING << "convertToRosegarden() - badly "
//                            "formed key signature";
//              break;
//            }
...
//          default:
//            RG_WARNING
//                << "convertToRosegarden() - unsupported META "
//                   "event code "
//                << QString( "0x%1" ).arg(
//                       midiEvent.getMetaEventCode(), 0, 16 );
//            break;
//        }
...
//            // Note-off?  Ignore.  Note-ons and note-offs have
//            // been consolidated.  Stray note-offs can be
//            // ignored.
...
//            RG_DEBUG << "convertToRosegarden(): note at "
//                     << rosegardenTime << ", duration "
//                     << rosegardenDuration << ", midi time "
//                     << midiAbsoluteTime << " and duration "
//                     << midiDuration;

//            rosegardenEvent =
//                new Event( Note::EventType, rosegardenTime,
//                           rosegardenDuration );
//            rosegardenEvent->set<Int>( BaseProperties::PITCH,
//                                       midiEvent.getPitch() );
//            rosegardenEvent->set<Int>(
...
//          case MIDI_CTRL_CHANGE:
//            rosegardenEvent = Controller( midiEvent.getData1(),
//                                          midiEvent.getData2()
//                                          )
//                                  .getAsEvent( rosegardenTime
//                                  );
...
//          case MIDI_PITCH_BEND:
//            rosegardenEvent = PitchBend( midiEvent.getData2(),
//                                         midiEvent.getData1() )
//                                  .getAsEvent( rosegardenTime
//                                  );
//            break;
...
//          default:
//            RG_WARNING
//                << "convertToRosegarden() - Unsupported event "
//                   "code = "
//                << QString( "0x%1" ).arg(
//                       midiEvent.getMessageType(), 0, 16 );
//            break;
...
//      if( rosegardenEvent ) {
//        // If there's a gap between the last note and this
//        event if( endOfLastNote < rosegardenTime ) {
//          // Fill it with rests.
//          segment->fillWithRests( endOfLastNote,
//                                  rosegardenTime );
//        }
...
//      if( event.isa( Note::EventRestType ) ) ++restCount;
//    }
...
//    // Note: A conductor track also contains time signatures
//    and
//    //       tempo events, however we've already added those to
//    //       the composition above.  They are not added to the
//    //       segment.
//    if( noteCount == 0 && keySigCount > 0 ) {
//      conductorSegment = segment;

//      // If this conductor segment has nothing but rests and
//      key
//      // sigs, there's no need to add it to the composition.
//      // ??? This probably never happens.  Conductor tracks
//      // almost
//      //     always have a few text events in them.
//      if( nonRestCount == keySigCount ) {
//        // ??? Memory leak.  conductorSegment will not be
//        // deleted.
//        continue;
//      }
//    }

//    // If this track has no key sigs and we have a conductor
//    // segment
...
//        //     it would make more sense to only add the key
//        sigs
//        //     that fall within the segment's time.  If there
//        //     isn't one at the very start of the segment, then
//        //     insert the previous.
...
//      // ??? No need.  It happens anyway.  Must be someone else
//      // doing
//      //     this.
...
//#ifdef DEBUG
//    RG_DEBUG << "convertToRosegarden(): MIDI import: adding "
//                "segment with start time "
//             << segment->getStartTime() << " and end time "
//             << segment->getEndTime();
//    // For this secret event dump to be triggered, a track that
//    // ends on beat 4 (2880 = 3 * 960) is required.  To
//    generate
//    // such a track in rg, set the time signature to 3/4 and
//    // reduce the composition to one bar in length.  Make a
//    // single bar segment and add events to it.  Export as
//    MIDI. if( segment->getEndTime() == 2880 ) {
//      RG_DEBUG << "  events:";
...
//  composition.setEndMarker(
//      composition.getBarEndForTime( maxTime ) );

//  return true;
//}

// void MidiFile::configureInstrument( Track *     track,
//                                    Segment *   segment,
//                                    Instrument *instrument ) {
//  if( !instrument ) return;

//  instrument->setPercussion( instrument->getNaturalChannel() ==
...
//    if( event.getAbsoluteTime() > 0 ) break;
...
//            controller, event.get<Int>( Controller::VALUE ) );
...
//    // Configure the Instrument
//    instrument->setMSB(
//        ( *msbIter )->get<Int>( Controller::VALUE ) );
//    instrument->setLSB(
//        ( *lsbIter )->get<Int>( Controller::VALUE ) );
...
bool MidiFile::convertToMidi( Composition&       comp,
                              std::string const& filename ) {
  return false;
}

bool MidiFile::convertToMidi( RosegardenDocument& doc,
                              std::string const&  filename ) {
  auto& comp         = doc.getComposition();
  auto* m_seqManager = new SequenceManager();
  m_seqManager->setDocument( &doc );
  m_seqManager->resetCompositionMapper();

  MappedBufMetaIterator* metaIterator =
      m_seqManager->makeTempMetaiterator();

  RealTime start =
      comp.getElapsedRealTime( comp.getStartMarker() );
  ...
  // Fetch the channel setup for all MIDI tracks in Fixed channel
  // mode.
  metaIterator->fetchFixedChannelSetup( sorter );

  metaIterator->jumpToTime( start );
  // Copy the events from metaIterator to sorter.
  // Give the end a little margin to make it insert noteoffs at
  // the end.  If they tied with the end they'd get lost.
  metaIterator->fetchEvents( sorter, start,
                             end + RealTime( 0, 1000 ) );
...
void MidiFile::writeInt( std::ofstream* midiFile, int number ) {
  *midiFile << static_cast<MidiByte>( ( number & 0xFF00 ) >> 8 );
  *midiFile << static_cast<MidiByte>( number & 0x00FF );
}

void MidiFile::writeLong( std::ofstream* midiFile,
                          unsigned long  number ) {
  *midiFile << static_cast<MidiByte>( ( number & 0xFF000000 ) >>
                                      24 );
  *midiFile << static_cast<MidiByte>( ( number & 0x00FF0000 ) >>
                                      16 );
  *midiFile << static_cast<MidiByte>( ( number & 0x0000FF00 ) >>
                                      8 );
  *midiFile << static_cast<MidiByte>( number & 0x000000FF );
}

std::string MidiFile::longToVarBuffer( unsigned long value ) {
...
void MidiFile::writeHeader( std::ofstream* midiFile ) {
...
void MidiFile::writeTrack( std::ofstream* midiFile,
                           TrackId        trackNumber ) {
  // For running status.
  ...
  for( MidiTrack::iterator i =
           m_midiComposition[trackNumber].begin();
       i != m_midiComposition[trackNumber].end(); ++i ) {
    ...
    // were originating, and decided to try just stripping them.
    // If you can't do it right, do it badly, and somebody will
    // eventually freak out, then fix it the right way.
    // ??? This is created in
    // ChannelManager::insertControllers().
    // ??? Since we now have the "Allow Reset All Controllers"
    // config
    //     option, we can probably get rid of this and tell
    //     people to use the config option.  If someone complains
    //     that 121's aren't appearing in MIDI files, that's
    //     probably the route we'll have to go.
    if( midiEvent.getData1() == MIDI_CONTROLLER_RESET ) {
      // Keep track of the timestamps from skipped events so we
      // can add them to the next event that makes it through.
    ...
    trackBuffer +=
        longToVarBuffer( midiEvent.getTime() + skippedTime );
    ...
    if( midiEvent.isMeta() ) {
      ...
      // If the event code has changed, or this is a SYSEX event,
      // we can't use running status. Running status is "[f]or
      // Voice and Mode messages only." Sysex is a system
      // message.  See the MIDI spec, Section 2, page 5.
      if( ( midiEvent.getEventCode() != previousEventCode ) ||
          ( midiEvent.getEventCode() ==
            MIDI_SYSTEM_EXCLUSIVE ) ) {
        // Send the normal event code (with encoded channel
        // information)
...
// void MidiFile::consolidateNoteEvents( TrackId trackId ) {
...
//       firstEventIter != track.end(); ++firstEventIter ) {
//    MidiEvent &firstEvent = **firstEventIter;

//    // Not a note-on?  Try the next event.
//    if( firstEvent.getMessageType() != MIDI_NOTE_ON ) continue;

//    // Note-on with velocity 0 is a note-off.  Try the next
//    // event.
//    if( firstEvent.getVelocity() == 0 ) continue;
...
//         secondEventIter != track.end(); ++secondEventIter ) {
//      const MidiEvent &secondEvent = **secondEventIter;

//      bool noteOff =
//          ( secondEvent.getMessageType() == MIDI_NOTE_OFF ||
...
//      // Note: Since all the events in a track are for a single
//      // channel,
//      //       this will never be true.
//      if( secondEvent.getChannelNumber() !=
//          firstEvent.getChannelNumber() )
//        continue;

//      timeT noteDuration =
//          secondEvent.getTime() - firstEvent.getTime();

//      // Some MIDI files floating around in the real world
//      // apparently have note-on followed immediately by
//      note-off
//      // on percussion tracks.  Instead of setting the duration
//      // to 0 in this case, which has no meaning, set it to 1.
//      if( noteDuration == 0 ) {
//        RG_WARNING << "consolidateNoteEvents() - detected MIDI
//        "
//                      "note duration of 0.  Using duration of "
//                      "1.  Touch wood.";
//        noteDuration = 1;
//      }
...
//      firstEvent.setDuration( ( *secondEventIter )->getTime() -
//                              firstEvent.getTime() );
//    }
//  }
//}

// void MidiFile::clearMidiComposition() {
//  // For each track
//  for( MidiComposition::iterator trackIter =
//           m_midiComposition.begin();
//       trackIter != m_midiComposition.end(); ++trackIter ) {
//    MidiTrack &midiTrack = trackIter->second;

//    // For each event on the track.
//    for( MidiTrack::iterator eventIter = midiTrack.begin();
//         eventIter != midiTrack.end(); ++eventIter ) {
//      delete *eventIter;
//    }
