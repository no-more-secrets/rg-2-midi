...
public:
  Impl( NotationQuantizer *const q )
    : m_unit( Note( Note::Demisemiquaver ).getDuration() ),
      ...
      m_provisionalAbsTime(
          "notationquantizer-provisionalAbsTime" ),
      m_provisionalDuration(
          "notationquantizer-provisionalDuration" ),
      m_provisionalNoteType(
          "notationquantizer-provisionalNoteType" ),
      m_provisionalScore(
          "notationquantizer-provisionalScore" ) {}
  ...
  public:
    ProvisionalQuantizer( Impl *i )
      : Quantizer( "blah", "blahblah" ), m_impl( i ) {}
    timeT getQuantizedDuration( const Event *e ) const override {
      return m_impl->getProvisional( (Event *)e, DurationValue );
    }
    timeT getQuantizedAbsoluteTime(
        const Event *e ) const override {
      timeT t = m_impl->getProvisional( (Event *)e,
                                        AbsoluteTimeValue );
#ifdef DEBUG_NOTATION_QUANTIZER
      cout << "ProvisionalQuantizer::getQuantizedAbsoluteTime: "
              "returning "
           << t << endl;
#endif
      return t;
  ...
  void quantizeRange( Segment *, Segment::iterator,
                      Segment::iterator ) const;

  void quantizeAbsoluteTime( Segment *,
                             Segment::iterator ) const;
  long scoreAbsoluteTimeForBase(
      Segment *, const Segment::iterator &, int depth,
      timeT base, timeT sigTime, timeT t, timeT d, int noteType,
      const Segment::iterator &, const Segment::iterator &,
      bool &right ) const;
  void quantizeDurationProvisional( Segment *,
                                    Segment::iterator ) const;
  void quantizeDuration( Segment *, Chord & ) const;

  void scanTupletsInBar(
      Segment *, timeT barStart, timeT barDuration,
...
NotationQuantizer::NotationQuantizer()
  : Quantizer( NotationPrefix ), m_impl( new Impl( this ) ) {
  // nothing else
}

NotationQuantizer::NotationQuantizer( std::string source,
                                      std::string target )
  : Quantizer( source, target ), m_impl( new Impl( this ) ) {
  // nothing else
}

NotationQuantizer::NotationQuantizer(
    const NotationQuantizer &q )
  : Quantizer( q.m_target ), m_impl( new Impl( *q.m_impl ) ) {
  // nothing else
}

NotationQuantizer::~NotationQuantizer() { delete m_impl; }

void NotationQuantizer::setUnit( timeT unit ) {
  m_impl->m_unit = unit;
}

timeT NotationQuantizer::getUnit() const {
  return m_impl->m_unit;
}

void NotationQuantizer::setMaxTuplet( int m ) {
  m_impl->m_maxTuplet = m;
}

int NotationQuantizer::getMaxTuplet() const {
  return m_impl->m_maxTuplet;
}

void NotationQuantizer::setSimplicityFactor( int s ) {
  m_impl->m_simplicityFactor = s;
}

int NotationQuantizer::getSimplicityFactor() const {
  return m_impl->m_simplicityFactor;
}

void NotationQuantizer::setContrapuntal( bool c ) {
  m_impl->m_contrapuntal = c;
}

bool NotationQuantizer::getContrapuntal() const {
  return m_impl->m_contrapuntal;
}

void NotationQuantizer::setArticulate( bool a ) {
  m_impl->m_articulate = a;
}

bool NotationQuantizer::getArticulate() const {
  return m_impl->m_articulate;
}

void NotationQuantizer::Impl::setProvisional( Event *   e,
                                              ValueType v,
                                              timeT t ) const {
...
timeT NotationQuantizer::Impl::getProvisional(
...
void NotationQuantizer::Impl::unsetProvisionalProperties(
    Event *e ) const {
...
void NotationQuantizer::Impl::quantizeAbsoluteTime(
    Segment *s, Segment::iterator i ) const {
  Profiler profiler(
      "NotationQuantizer::Impl::quantizeAbsoluteTime" );
...
#ifdef DEBUG_NOTATION_QUANTIZER
  cout << "quantizeAbsoluteTime: t is " << t << ", d is " << d
       << endl;
#endif

  // scoreAbsoluteTimeForBase wants to know the previous starting
  // note (N) and the previous starting note that ends (roughly)
  // before this one starts (N').  Much more efficient to
  // calculate them once now before the loop.
...
#ifdef DEBUG_NOTATION_QUANTIZER
  if( n != s->end() && n != nprime ) {
    cout << "found n (distinct from nprime) at "
         << ( *n )->getAbsoluteTime() << endl;
  }
  if( nprime != s->end() ) {
  ...
  for( int depth = 0; depth < maxDepth; ++depth ) {
    ...
    bool right = false;
    long score = scoreAbsoluteTimeForBase(
        s, i, depth, base, sigTime, t, d, noteType, n, nprime,
        right );
  ...
  if( bestBase == -2 ) {
#ifdef DEBUG_NOTATION_QUANTIZER
    cout << "Quantizer::quantizeAbsoluteTime: weirdness: no "
            "snap found"
         << endl;
#endif
  } else {
...
#ifdef DEBUG_NOTATION_QUANTIZER
    cout << "snap base is " << bestBase << ", snapped to " << t
         << endl;
#endif
  }
...
long NotationQuantizer::Impl::scoreAbsoluteTimeForBase(
    Segment *s, const Segment::iterator & /* i */, int depth,
    timeT base, timeT sigTime, timeT t, timeT d, int noteType,
    const Segment::iterator &n, const Segment::iterator &nprime,
  ...
  double effectiveDepth =
      pow( depth + 2, simplicityFactor / 10 );

  //!!! use velocity to adjust the effective depth as well? --
  //! louder
  // notes are more likely to be on big boundaries.  Actually,
  // perhaps introduce a generally-useful "salience" score a la
  // Dixon et al
  ...
  for( int ri = 0; ri < 2; ++ri ) {
    ...
    long score =
        long( ( distance + shortTime / 2 ) * effectiveDepth );
    ...
    // We should penalise moving this note to before the
    // performed end of N' and seriously penalise moving it to
    // the same quantized start time as N' -- but we should
    // encourage moving it to the same time as the provisional
    // end of N', or to the same start time as N if N != N'.
    ...
    if( !right ) {
      if( n != s->end() ) {
        ...
        if( nprime != s->end() ) {
          timeT npt =
              getProvisional( *nprime, AbsoluteTimeValue );
          timeT npd = getProvisional( *nprime, DurationValue );
          if( t - distance <= npt )
            penalty2 *= 4;
          else if( t - distance < npt + npd )
            penalty2 *= 2;
          else if( t - distance == npt + npd )
            penalty2 = penalty2 * 2 / 3;
        }
      }
    }

#ifdef DEBUG_NOTATION_QUANTIZER
    cout << "  depth/eff/dist/t/score/pen1/pen2/res: " << depth
         << "/" << effectiveDepth << "/" << distance << "/"
         << ( right ? t + distance : t - distance ) << "/"
...
void NotationQuantizer::Impl::quantizeDurationProvisional(
    Segment *, Segment::iterator i ) const {
  Profiler profiler(
      "NotationQuantizer::Impl::quantizeDurationProvisional" );

  // Calculate a first guess at the likely notation duration
  // based only on its performed duration, without considering
  // start time.
  ...
  if( shortTime != d ) {
    ...
    if( ( shortNote.getDots() > 0 ||
          shortNote.getNoteType() ==
              Note::Shortest ) ) { // can't dot that
    ...
    // we should prefer to round up to a note with fewer dots
    // rather than down to one with more

    //!!! except in dotted time, etc -- we really want this to
    //! work on a
    // similar attraction-to-grid basis to the abstime
    // quantization
...
void NotationQuantizer::Impl::quantizeDuration(
    Segment *s, Chord &c ) const {
  //    static int totalFracCount = 0;
  //    static float totalFrac = 0;

  //    Profiler
  //    profiler("NotationQuantizer::Impl::quantizeDuration");

  //#ifdef DEBUG_NOTATION_QUANTIZER
  //    cout << "quantizeDuration: chord has " << c.size() << "
  //    notes" << endl;
  //#endif
  ...
  //    TimeSignature timeSig;
  ////    timeT t = m_q->getFromSource(*c.getInitialElement(),
  /// AbsoluteTimeValue); /    timeT sigTime =
  /// comp->getTimeSignatureAt(t, timeSig);

  //    timeT d = getProvisional(*c.getInitialElement(),
  //    DurationValue); int noteType =
  //    Note::getNearestNote(d).getNoteType(); int maxDepth = 8 -
  //    noteType; if (maxDepth < 4) maxDepth = 4;
  //    std::vector<int> divisions;
  //    timeSig.getDivisions(maxDepth, divisions);
  ...
  //            // dealt with already in tuplet code, we'd only
  //            mess it up here
  //#ifdef DEBUG_NOTATION_QUANTIZER
  //            cout << "not recalculating duration for tuplet"
  //            << endl;
  //#endif
  //            continue;
  //        }
  ...
  //        if (!m_contrapuntal) {
  //            // if not contrapuntal, give all notes in chord
  //            equal duration if (nonContrapuntalDuration > 0) {
  //#ifdef DEBUG_NOTATION_QUANTIZER
  //                cout << "setting duration trivially to " <<
  //                nonContrapuntalDuration << endl;
  //#endif
  //                setProvisional(**ci, DurationValue,
  //                nonContrapuntalDuration); continue;
  //            } else {
  //                // establish whose duration to use, then set
  //                it at the
  //                // bottom after it's been quantized
  //                Segment::iterator li = c.getLongestElement();
  //                if (li != s->end()) ud =
  //                m_q->getFromSource(*li, DurationValue); else
  ...
  //#ifdef DEBUG_NOTATION_QUANTIZER
  //        cout << "note at time " << (**ci)->getAbsoluteTime()
  //        << " (provisional time " << qt << ")" << endl;
  //#endif
  ...
  //        for (int depth = 0; depth < maxDepth; ++depth) {
  //            if (base >= ud) {
  //                bases = std::pair<timeT, timeT>(base /
  //                divisions[depth], base);
  //            }
  //            base /= divisions[depth];
  //        }
  ...
  //            qd =
  //            Note::getNearestNote(spaceAvailable).getDuration();

  //#ifdef DEBUG_NOTATION_QUANTIZER
  //            cout << "non-contrapuntal segment, rounded
  //            duration down to "
  //                 << qd << " (as only " << spaceAvailable << "
  //                 available)"
  //                 << endl;
  //#endif
  ...
  //            //!!! Note longer than the longest note we have.
  //            Deal with
  //            //this -- how?  Quantize the end time?  Split the
  //            note?
  //            //(Prefer to do that in a separate phase later if
  //            requested.)
  ...
  //                                      comp->getBarEndForTime(qt)
  //                                      - qt);

  //            // We have a really good possibility of staccato
  //            if we have a
  //            // note on a boundary whose base is double the
  //            note duration
  //            // and there's nothing else until the next
  //            boundary and we're
  //            // shorter than about a quaver (i.e. the base is
  //            a quaver or
  //            // less)

  //            if (qd*2 <= absTimeBase && (qd*8/3) >=
  //            absTimeBase &&
  ...
  //                         << " but we're on " << absTimeBase
  //                         << " absTimeBase"
  //                         << " and the next base is " <<
  //                         bases.second
  ...
  //                // Alternatively, if we rounded down but
  //                there's space to
  //                // round up, consider doing so

  //                //!!! mark staccato if necessary, and take
  //                existing marks into account

  //                Note note(Note::getNearestNote(qd));

  //                if (qd < ud || (qd == ud && note.getDots() ==
  //                2)) {
  ...
  //#ifdef DEBUG_NOTATION_QUANTIZER
  //    cout << "totalFrac " << totalFrac << ", totalFracCount "
  //    << totalFracCount << ", avg " << (totalFracCount > 0 ?
  //    (totalFrac / totalFracCount) : 0) << endl;
  //#endif
}

void NotationQuantizer::Impl::scanTupletsInBar(
    Segment *s, timeT barStart, timeT barDuration,
    timeT wholeStart, timeT wholeEnd,
    const std::vector<int> &divisions ) const {
  Profiler profiler(
      "NotationQuantizer::Impl::scanTupletsInBar" );

  //!!! need to further constrain the area scanned so as to cope
  //! with
  // partial bars

  timeT base = barDuration;

  for( int depth = -1; depth < int( divisions.size() ) - 2;
       ++depth ) {
    if( depth >= 0 ) base /= divisions[depth];
    if( base <= Note( Note::Semiquaver ).getDuration() ) break;

#ifdef DEBUG_NOTATION_QUANTIZER
    cout << "\nscanTupletsInBar: trying at depth " << depth
         << " (base " << base << ")" << endl;
#endif

    // check for triplets if our next divisor is 2 and the
    // following one is not 3

    if( divisions[depth + 1] != 2 || divisions[depth + 2] == 3 )
      continue;
...
#ifdef DEBUG_NOTATION_QUANTIZER
      cout << "scanTupletsInBar: testing " << tupletStart << ","
           << base << " at tuplet base " << tupletBase << endl;
#endif

      // find first note within a certain distance whose start
      // time quantized to tupletStart or greater
      Segment::iterator j =
          s->findTime( tupletStart - tupletBase / 3 );
...
void NotationQuantizer::Impl::scanTupletsAt(
    Segment *s, Segment::iterator i, int depth, timeT base,
    timeT sigTime, timeT tupletStart, timeT tupletBase ) const {
...
#ifdef DEBUG_NOTATION_QUANTIZER
      cout << "some notes not provisionally quantized, no good"
           << endl;
#endif
      return;
    }
...
#ifdef DEBUG_NOTATION_QUANTIZER
      cout << "accepting note that happens to lie on original "
              "base"
           << endl;
#endif
      candidates.push_back( *j );
    } else {
      // This is a note that did not quantize to the original
      // base (the first note in the tuplet would have, but we
      // can't tell anything from that).  Reject the entire group
      // if it fails any of the likelihood tests for tuplets.

      if( !isValidTupletAt( s, j, depth, base, sigTime,
                            tupletBase ) ) {
#ifdef DEBUG_NOTATION_QUANTIZER
        cout << "no good" << endl;
#endif
  ...
  // must have at least one note that is not already quantized to
  // the original base
  if( count < 1 ) {
#ifdef DEBUG_NOTATION_QUANTIZER
    cout << "scanTupletsAt: found no note not already quantized "
            "to "
         << base << endl;
#endif
    return;
...
#ifdef DEBUG_NOTATION_QUANTIZER
  cout << "scanTupletsAt: Tuplet group of duration " << base
       << " starting at " << tupletStart << endl;
#endif
  ...
  for( std::vector<Event *>::iterator ei = candidates.begin();
    ...
    //!!! Interesting -- we can't modify rests here, but
    //! Segment's
    // normalizeRests won't insert the correct sort of rest for
    // us... what to do?
    //!!! insert a tupleted rest, and prevent
    ...
    ( *ei )->set<Int>(
        ...
        base / 2 ); //!!! wrong if tuplet count != 3
    ( *ei )->set<Int>( BEAMED_GROUP_TUPLED_COUNT,
                       2 ); //!!! as above
    ( *ei )->set<Int>( BEAMED_GROUP_UNTUPLED_COUNT,
                       base / tupletBase );
  ...
  for( int m = 0; m < base / tupletBase; ++m ) {
    ...
    Event *rest =
        new Event( Note::EventRestType, absTime, duration );

    rest->set<String>( BEAMED_GROUP_TYPE, GROUP_TYPE_TUPLED );
    rest->set<Int>( BEAMED_GROUP_ID, groupId );
    rest->set<Int>( BEAMED_GROUP_TUPLET_BASE,
                    base / 2 ); //!!! wrong if tuplet count != 3
    rest->set<Int>( BEAMED_GROUP_TUPLED_COUNT,
                    2 ); //!!! as above
    rest->set<Int>( BEAMED_GROUP_UNTUPLED_COUNT,
                    base / tupletBase );
...
bool NotationQuantizer::Impl::isValidTupletAt(
    Segment *s, const Segment::iterator &i, int depth,
    timeT /* base */, timeT sigTime, timeT tupletBase ) const {
  Profiler profiler(
      "NotationQuantizer::Impl::isValidTupletAt" );

  //!!! This is basically wrong; we need to be able to deal with
  //! groups
...
#ifdef DEBUG_NOTATION_QUANTIZER
    cout << "\nNotationQuantizer::isValidTupletAt: note too "
            "long at "
         << ( *i )->getDuration() << " (tupletBase is "
         << tupletBase << ")" << endl;
#endif
    return false; // too long
  }

  //!!! This bit is a cop-out.  It means we reject anything that
  //! looks
  ...
  long score = 0;
  if( !( *i )->get<Int>( m_provisionalScore, score ) )
    return false;
  ...
  //!!! not as complete as the calculation we do in the original
  //! scoring
  bool dummy;
  long tupletScore = scoreAbsoluteTimeForBase(
      s, i, depth, tupletBase, sigTime, t, d, noteType, s->end(),
      s->end(), dummy );
#ifdef DEBUG_NOTATION_QUANTIZER
...
void NotationQuantizer::quantizeRange(
    Segment *s, Segment::iterator from,
    Segment::iterator to ) const {
  m_impl->quantizeRange( s, from, to );
}

void NotationQuantizer::Impl::quantizeRange(
    Segment *s, Segment::iterator from,
    Segment::iterator to ) const {
  //  Profiler *profiler =
  //      new Profiler( "NotationQuantizer::Impl::quantizeRange"
  //      );
  ...
  //  // This process does several passes over the data.  It's
  //  // assumed that this is not going to be invoked in any
  //  really
  //  // time-critical place.

  //  // Calculate absolute times on the first pass, so that we
  //  know
  //  // which things are chords.  We need to assign absolute
  //  times
  //  // to all events, but we only need do durations for notes.

  //  PropertyName provisionalBase(
  //      "notationquantizer-provisionalBase" );

  //  // We don't use setToTarget until we have our final values
  //  // ready, as it erases and replaces the events.  Just set
  //  the
  //  // properties.
  ...
  //    timeT t1 = ( *i )->get<Int>( m_provisionalDuration ) +
  //    t0; if( wholeStart == wholeEnd ) {
  //      wholeStart = t0;
  //      wholeEnd   = t1;
  //    } else if( t1 > wholeEnd ) {
  //      wholeEnd = t1;
  ...
  //  // staccato (we now do slurs separately, in
  //  // SegmentNotationHelper::autoSlur)
  ...
  //  delete profiler; // on heap so it updates before the next
  //  line: Profiles::getInstance()->dump();
  //
}
