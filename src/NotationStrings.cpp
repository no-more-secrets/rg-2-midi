...
QString NotationStrings::addDots( QString s, int dots,
                                  bool hyphenate,
                                  bool internationalize ) {
  if( !dots ) return s;
...
QString NotationStrings::getNoteName( Note note, bool plural,
                                      bool triplet ) {
  ...
  if( plural && triplet ) {
    return addDots(
        tr( "%1 triplets" )
            .arg( names[type].toStdString().c_str() ),
        dots, false,
        true ); // TODO PLURAL - this is broken because it
                // assumes there's only 1 plural form
  } else if( plural ) {
...
QString NotationStrings::getAmericanName( Note note, bool plural,
                                          bool triplet ) {
  Note::Type type = note.getNoteType();
  int        dots = note.getDots();
  ...
  if( plural && triplet ) {
    return addDots( QString( "%1 triplets" ).arg( names[type] ),
                    dots, false, false );
  } else if( plural ) {
    return addDots( pluralnames[type], dots, false, false );
  } else if( triplet ) {
    return addDots( QString( "%1 triplet" ).arg( names[type] ),
                    dots, false, false );
  } else {
...
QString NotationStrings::getShortNoteName( Note note,
                                           bool plural,
                                           bool triplet ) {
  Note::Type type = note.getNoteType();
  int        dots = note.getDots();
  ...
  static const QString names[] = {
      tr( "64th" ),  tr( "32nd" ),        tr( "16th" ),
      tr( "8th" ),   tr( "quarter" ),     tr( "half" ),
      tr( "whole" ), tr( "double whole" )};
  static const QString pluralnames[] = {
      tr( "64ths" ),  tr( "32nds" ),        tr( "16ths" ),
      tr( "8ths" ),   tr( "quarters" ),     tr( "halves" ),
      tr( "wholes" ), tr( "double wholes" )};

  if( plural && triplet ) {
    return addDots( tr( "%1 triplets" ).arg( names[type] ), dots,
                    false,
                    true ); // TODO - this is broken because it
                            // assumes there's only 1 plural form
  } else if( plural ) {
    return addDots( pluralnames[type], dots, false, true );
  } else if( triplet ) {
    return addDots( tr( "%1 triplet" ).arg( names[type] ), dots,
                    false, true );
  } else {
    return addDots( names[type], dots, false, true );
  }
...
QString NotationStrings::getReferenceName( Note note,
                                           bool isRest ) {
  Note::Type type = note.getNoteType();
  int        dots = note.getDots();

  static const QString names[] = {
      "hemidemisemi", "demisemi", "semiquaver", "quaver",
...
Note NotationStrings::getNoteForName( QString name ) {
  ...
  if( pos > 0 && pos < 6 && pos < name.length() - 1 ) {
    ...
    if( dots < 2 ) {
      throw MalformedNoteName(
          "Non-numeric or invalid dot count in \"" + origName +
          "\"" );
    }
  }

  if( name.length() > 7 && ( name.left( 7 ) == "dotted " ||
                             name.left( 7 ) == "dotted-" ) ) {
    if( dots == 0 ) dots = 1;
    ...
    if( dots > 1 ) {
      throw MalformedNoteName(
          "Dot count without dotted tag in \"" + origName +
          "\"" );
    }
  }
  ...
  static const char *names[][4] = {
      {"64th", "sixty-fourth", "hemidemisemi",
       "hemidemisemiquaver"},
  ...
  throw MalformedNoteName( "Can't parse note name \"" +
                           origName + "\"" );
}

QString NotationStrings::makeNoteMenuLabel( timeT  duration,
                                            bool   brief,
                                            timeT &errorReturn,
                                            bool   plural ) {
  ...
  //        settings.value
  //        ("notenamestyle",
  //        GeneralConfigurationPage::Local).toUInt();
  ...
  if( brief ) {
    ...
    } else if( ( duration / wholeNote ) * wholeNote ==
               duration ) {
      return QString( "%1/1" ).arg( duration / wholeNote );
    } else if( ( wholeNote / ( duration * 2 / 3 ) ) *
                   ( duration * 2 / 3 ) ==
               wholeNote ) {
      return QString( "3/%1" ).arg( wholeNote /
                                    ( duration * 1 / 3 ) );
    } else {
    ...
    //    noteName = getAmericanName(nearestNote, plural,
    //    triplet); break;
