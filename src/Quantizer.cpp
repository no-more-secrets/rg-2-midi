...
Quantizer::Quantizer( std::string source, std::string target )
  : m_source( source ), m_target( target ) {
  makePropertyNames();
}

Quantizer::Quantizer( std::string target ) : m_target( target ) {
...
void Quantizer::quantize( Segment *s ) const {
  quantize( s, s->begin(), s->getEndMarker() );
}

void Quantizer::quantize( Segment *s, Segment::iterator from,
                          Segment::iterator to ) const {
  m_normalizeRegion.first =
      ( from != s->end() ? ( *from )->getAbsoluteTime()
                         : s->getStartTime() );
  m_normalizeRegion.second =
      ( to != s->end() ? ( *to )->getAbsoluteTime()
                       : s->getEndTime() );
...
void Quantizer::quantize( EventSelection *selection ) {
  ...
  // So that we can retrieve a list of new events we cheat and
  // stop the m_toInsert vector from being cleared automatically.
  ...
  for( int i = 0; i < int( m_toInsert.size() ); ++i ) {
    if( m_toInsert[i]->getAbsoluteTime() <
        segment.getEndTime() ) {
...
void Quantizer::fixQuantizedValues(
  ...
  for( Segment::iterator nextFrom = from; from != to;
       from                       = nextFrom ) {
    ++nextFrom;
...
timeT Quantizer::getQuantizedDuration( const Event *e ) const {
...
timeT Quantizer::getQuantizedAbsoluteTime(
    const Event *e ) const {
  if( m_target == RawEventData ) {
...
timeT Quantizer::getUnquantizedAbsoluteTime( Event *e ) const {
  return getFromSource( e, AbsoluteTimeValue );
}

timeT Quantizer::getUnquantizedDuration( Event *e ) const {
  return getFromSource( e, DurationValue );
}

void Quantizer::quantizeRange( Segment *         s,
                               Segment::iterator from,
                               Segment::iterator to ) const {
  //!!! It is vital that ordering is maintained after
  //! quantization.
  // That is, an event whose absolute time quantizes to a time t
  // must appear in the original segment before all events whose
  // times quantize to greater than t.  This means we must
  // quantize the absolute times of non-note events as well as
  // notes.

  // We don't need to worry about quantizing rests, however;
  // they're only used for notation and will be explicitly
  // recalculated when the notation quantization values change.
...
void Quantizer::unquantize( Segment *s, Segment::iterator from,
                            Segment::iterator to ) const {
  for( Segment::iterator nextFrom = from; from != to;
       from                       = nextFrom ) {
    ++nextFrom;
...
void Quantizer::unquantize( EventSelection *selection ) const {
...
timeT Quantizer::getFromSource( Event *e, ValueType v ) const {
  Profiler profiler( "Quantizer::getFromSource" );

  //    cerr << "Quantizer::getFromSource: source is \"" <<
  //    m_source << "\"" << endl;
...
timeT Quantizer::getFromTarget( Event *e, ValueType v ) const {
...
void Quantizer::setToTarget( Segment *s, Segment::iterator i,
                             timeT absTime,
                             timeT duration ) const {
  Profiler profiler( "Quantizer::setToTarget" );

  // cerr << "Quantizer::setToTarget: target is \"" << m_target
  // << "\", absTime is " << absTime << ", duration is " <<
  // duration << " (unit is " << m_unit << ", original values are
  // absTime " << (*i)->getAbsoluteTime() << ", duration " <<
  // (*i)->getDuration() << ")" << endl;
  ...
  if( m_source != RawEventData && m_target == RawEventData ) {
    haveSt = ( *i )->get<Int>(
        m_sourceProperties[AbsoluteTimeValue], st );
    haveSd = ( *i )->get<Int>( m_sourceProperties[DurationValue],
                               sd );
  }
  ...
  if( m_target == RawEventData ) {
    ...
    cout << "Quantizer: setting " << absTime
         << " to notation absolute time and " << duration
         << " to notation duration" << endl;
#endif
    e = new Event( **i, ( *i )->getAbsoluteTime(),
                   ( *i )->getDuration(),
  ...
  if( m_target == NotationPrefix ) {
    timeT normalizeStart =
        std::min( absTime, ( *i )->getAbsoluteTime() );
    ...
    if( m_normalizeRegion.first != m_normalizeRegion.second ) {
      normalizeStart =
          std::min( normalizeStart, m_normalizeRegion.first );
      normalizeEnd =
          std::max( normalizeEnd, m_normalizeRegion.second );
    }
  ...
  if( haveSt )
    e->setMaybe<Int>( m_sourceProperties[AbsoluteTimeValue],
                      st );
  if( haveSd )
    e->setMaybe<Int>( m_sourceProperties[DurationValue], sd );

  if( m_target != RawEventData && m_target != NotationPrefix ) {
    e->setMaybe<Int>( m_targetProperties[AbsoluteTimeValue],
                      absTime );
    e->setMaybe<Int>( m_targetProperties[DurationValue],
                      duration );
  } else {
    s->erase( i );
    m_toInsert.push_back( e );
...
#ifdef DEBUG_NOTATION_QUANTIZER
  cout << "m_toInsert.size() is now " << m_toInsert.size()
       << endl;
#endif
}

void Quantizer::removeProperties( Event *e ) const {
  if( m_source != RawEventData ) {
...
void Quantizer::removeTargetProperties( Event *e ) const {
  if( m_target != RawEventData ) {
    e->unset( m_targetProperties[AbsoluteTimeValue] );
    e->unset( m_targetProperties[DurationValue] );
  }
...
void Quantizer::makePropertyNames() {
  if( m_source != RawEventData && m_source != NotationPrefix ) {
    m_sourceProperties[AbsoluteTimeValue] =
        m_source + "AbsoluteTimeSource";
    m_sourceProperties[DurationValue] =
        m_source + "DurationSource";
  }

  if( m_target != RawEventData && m_target != NotationPrefix ) {
    m_targetProperties[AbsoluteTimeValue] =
        m_target + "AbsoluteTimeTarget";
    m_targetProperties[DurationValue] =
        m_target + "DurationTarget";
  }
}

void Quantizer::insertNewEvents( Segment *s ) const {
  ...
  for( size_t i = 0; i < sz; ++i ) {
    ...
    if( endTime > 0 && myTime >= endTime ) {
      cerr << "Quantizer::insertNewEvents(): ignoring event "
              "outside the segment at "
           << myTime << endl;
      continue;
    }
  ...
  if( minTime < startTime ) {
    ...
    if( barStart <= startTime ) {
      ...
      minTime = std::max(
          startTime, s->getBarStartForTime( barStart - 1 ) );
    }
  }
  ...
  if( endTime > 0 ) {
    if( maxTime > endTime ) {
      ...
      if( barEnd >= endTime ) {
        ...
        maxTime = std::min( endTime,
                            s->getBarEndForTime( barEnd + 1 ) );
      }
    }
  }
  ...
  if( m_target == NotationPrefix || m_target == RawEventData ) {
    if( m_normalizeRegion.first == m_normalizeRegion.second ) {
      if( sz > 0 ) { s->normalizeRests( minTime, maxTime ); }
    } else {
...
#ifdef DEBUG_NOTATION_QUANTIZER
  cout << "Quantizer: calling normalizeRests(" << minTime << ", "
       << maxTime << ")" << endl;
#endif
