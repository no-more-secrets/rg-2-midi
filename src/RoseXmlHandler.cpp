...
/**
 * Unlike strtod(3) or QString::toDouble(), this is
 * locale-independent and always uses '.' as a decimal point.  We
 * use it when reading things like configuration values from XML
 * files where we want to guarantee the same value is used
 * regardless of surrounding locale.
 */
ROSEGARDENPRIVATE_EXPORT double strtodouble(
    const std::string &s ) {
  int    dp     = 0;
  int    sign   = 1;
  size_t i      = 0; //@@@
...
ROSEGARDENPRIVATE_EXPORT double qstrtodouble(
    const QString &s ) {
...
ConfigurationXmlSubHandler::ConfigurationXmlSubHandler(
    const QString &            elementName,
    Rosegarden::Configuration *configuration )
  : m_configuration( configuration ),
    m_elementName( elementName ) {}

bool ConfigurationXmlSubHandler::startElement(
    const QString &, const QString &, const QString &lcName,
    const QXmlAttributes &atts ) {
  m_propertyName = lcName;
  m_propertyType = atts.value( "type" );

  if( m_propertyName == "property" ) {
    // handle alternative encoding for properties with arbitrary
    // names
...
bool ConfigurationXmlSubHandler::characters(
    const QString &chars ) {
  ...
  if( m_propertyType == "Int" ) {
    long i = ch.toInt();
    // RG_DEBUG << "  setting (int) " << m_propertyName << "=" <<
    // i;
  ...
  if( m_propertyType == "RealTime" ) {
    ...
    // RG_DEBUG << "  setting (RealTimeT) " << m_propertyName <<
    // "=" << rt.sec << "(sec) " << rt.nsec << "(nsec)";

    m_configuration->set<Rosegarden::RealTimeT>(
        qstrtostr( m_propertyName ), rt );
  ...
  if( m_propertyType == "Bool" ) {
    ...
    bool b = ( chLc == "true" || chLc == "1" || chLc == "on" );

    // RG_DEBUG << "  setting (Bool) " << m_propertyName << "="
    // << b;

    m_configuration->set<Rosegarden::Bool>(
        qstrtostr( m_propertyName ), b );

    return true;
  }

  if( m_propertyType.isEmpty() || m_propertyType == "String" ) {
    // RG_DEBUG << "  setting (String) " << m_propertyName << "="
    // << ch;

    m_configuration->set<Rosegarden::String>(
        qstrtostr( m_propertyName ), qstrtostr( ch ) );
...
bool ConfigurationXmlSubHandler::endElement(
    const QString &, const QString &, const QString &lcName,
    bool &finished ) {
  m_propertyName = "";
  m_propertyType = "";
...
RoseXmlHandler::~RoseXmlHandler() { delete m_subHandler; }

Composition &RoseXmlHandler::getComposition() {
  return m_doc->getComposition();
}

Studio &RoseXmlHandler::getStudio() {
  return m_doc->getStudio();
}

// AudioFileManager &RoseXmlHandler::getAudioFileManager() {
//  return m_doc->getAudioFileManager();
//}

// AudioPluginManager *RoseXmlHandler::getAudioPluginManager() {
//  return m_doc->getPluginManager();
//}

bool RoseXmlHandler::startDocument() {
  // Clear tracks
  //
...
bool RoseXmlHandler::startElement( const QString &namespaceURI,
                                   const QString &localName,
                                   const QString &qName,
                                   const QXmlAttributes &atts ) {
  ...
  if( getSubHandler() ) {
    return getSubHandler()->startElement(
        namespaceURI, localName, lcName, atts );
  }

  if( lcName == "event" ) {
    //        RG_DEBUG << "RoseXmlHandler::startElement: found
    //        event, current time is " << m_currentTime;
    ...
    if( m_currentEvent->has( BEAMED_GROUP_ID ) ) {
      ...
      long storedId =
          m_currentEvent->get<Int>( BEAMED_GROUP_ID );

      if( m_groupIdMap.find( storedId ) == m_groupIdMap.end() ) {
        m_groupIdMap[storedId] = m_currentSegment->getNextId();
      }

      m_currentEvent->set<Int>( BEAMED_GROUP_ID,
                                m_groupIdMap[storedId] );

    } else if( m_inGroup ) {
      m_currentEvent->set<Int>( BEAMED_GROUP_ID, m_groupId );
      m_currentEvent->set<String>( BEAMED_GROUP_TYPE,
                                   m_groupType );
      if( m_groupType == GROUP_TYPE_TUPLED ) {
        m_currentEvent->set<Int>( BEAMED_GROUP_TUPLET_BASE,
                                  m_groupTupletBase );
        m_currentEvent->set<Int>( BEAMED_GROUP_TUPLED_COUNT,
                                  m_groupTupledCount );
        m_currentEvent->set<Int>( BEAMED_GROUP_UNTUPLED_COUNT,
                                  m_groupUntupledCount );
      }
    }
    ...
    if( !m_inChord ) {
      m_currentTime =
          m_currentEvent->getAbsoluteTime() + duration;

      //            RG_DEBUG << "RoseXmlHandler::startElement:
      //            (we're not in a chord) ";
    ...
    // RosegardenDocument.cpp.  We only care about major and
    // minor here, not point.
    ...
    if( !smajor.isEmpty() ) {
      ...
      if( major >
          RosegardenDocument::FILE_FORMAT_VERSION_MAJOR ) {
        m_errorString =
            tr( "This file was written by Rosegarden %1, and it "
                "uses\na different file format that cannot be "
      ...
      if( major ==
              RosegardenDocument::FILE_FORMAT_VERSION_MAJOR &&
          minor >
              RosegardenDocument::FILE_FORMAT_VERSION_MINOR ) {
        // StartupLogo::hideIfStillThere();

        // QMessageBox::information(
        //    nullptr, tr( "Rosegarden" ),
        //    tr( "This file was written by Rosegarden %1, which
        //    "
        //        "is more recent than this version.\nThere may "
        //        "be some incompatibilities with the file "
        //        "format." )
        //        .arg( version ) );
      }
    }
    ...
    // In the Studio we clear down everything apart from Devices
    // and Instruments before we reload.  Instruments are derived
    // from the Sequencer, the bank/program information is loaded
    // from the file we're currently examining.
    //
    ...
    if( m_inComposition == false ) {
      m_errorString =
          "TimeSignature object found outside Composition";
      return false;
    }

    timeT   t       = 0;
    QString timeStr = atts.value( "time" );
    if( !timeStr.isEmpty() ) t = timeStr.toInt();

    int     num    = 4;
    QString numStr = atts.value( "numerator" );
    if( !numStr.isEmpty() ) num = numStr.toInt();

    int     denom    = 4;
    QString denomStr = atts.value( "denominator" );
    if( !denomStr.isEmpty() ) denom = denomStr.toInt();

    bool    common    = false;
    QString commonStr = atts.value( "common" );
    if( !commonStr.isEmpty() ) common = ( commonStr == "true" );

    bool    hidden    = false;
    QString hiddenStr = atts.value( "hidden" );
    if( !hiddenStr.isEmpty() ) hidden = ( hiddenStr == "true" );
    ...
    getComposition().addTimeSignature(
        t, TimeSignature( num, denom, common, hidden,
                          hiddenBars ) );
    ...
    if( !recordStr.isEmpty() ) {
      getComposition().setTrackRecording( recordStr.toInt(),
                                          true );
    }
    ...
    if( !recordPlStr.isEmpty() ) {
      ...
      for( QStringList::iterator i = recordList.begin();
           i != recordList.end(); ++i ) {
        getComposition().setTrackRecording( ( *i ).toInt(),
                                            true );
      }
    }
    ...
    if( !selectedTrackStr.isEmpty() ) {
      TrackId selectedTrack = (TrackId)selectedTrackStr.toInt();
    ...
    if( !soloTrackStr.isEmpty() )
      if( soloTrackStr.toInt() == 1 ) m_oldSolo = true;
    ...
    if( !nextTriggerIdStr.isEmpty() ) {
      getComposition().setNextTriggerSegmentId(
          nextTriggerIdStr.toInt() );
    }

    QString copyrightStr = atts.value( "copyright" );
    if( !copyrightStr.isEmpty() ) {
      getComposition().setCopyrightNote(
          qstrtostr( copyrightStr ) );
    }
    ...
    if( !panLawStr.isEmpty() ) {
      ...
      // Since no "panlaw" was found in this tag, apply the
      // default.
    ...
    if( !trackNbStr.isEmpty() ) { id = trackNbStr.toInt(); }

    QString labelStr = atts.value( "label" );
    if( !labelStr.isEmpty() ) { label = qstrtostr( labelStr ); }
    ...
    Track *track =
        new Track( id, instrument, position, label, muted );
    ...
    // track properties affecting newly created segments are
    // initialized to default values in the ctor, so they don't
    ...
    if( !thruRoutingStr.isEmpty() ) {
      track->setThruRouting( static_cast<Track::ThruRouting>(
          thruRoutingStr.toInt() ) );
    }

    QString soloStr = atts.value( "solo" );
    if( soloStr == "true" ) track->setSolo( true );

    QString archivedStr = atts.value( "archived" );
    if( archivedStr == "true" ) track->setArchived( true );

    // If the composition tag had this track set to record, make
    // sure it is armed.
    ...
    if( !segmentType.isEmpty() ) {
      if( segmentType == "audio" ) {
        ...
        // if( getAudioFileManager().fileExists( audioFileId ) ==
        //    false ) {
        //  // We don't report an error as this audio file
        //  might've
    ...
    if( ( !rtDelaySec.isEmpty() ) &&
        ( ( !rtDelaynSec.isEmpty() ) ||
          ( !rtDelayuSec.isEmpty() ) ) ) {
      if( !rtDelaynSec.isEmpty() ) {
        m_currentSegment->setRealTimeDelay( RealTime(
            rtDelaySec.toInt(), rtDelaynSec.toInt() ) );
      } else {
        m_currentSegment->setRealTimeDelay( RealTime(
            rtDelaySec.toInt(), rtDelayuSec.toInt() * 1000 ) );
      }
    }
    ...
    if( !snapGridSizeStr.isEmpty() ) {
      m_currentSegment->setSnapGridSize(
          snapGridSizeStr.toInt() );
    }

    QString viewFeaturesStr = atts.value( "viewfeatures" );
    if( !viewFeaturesStr.isEmpty() ) {
      m_currentSegment->setViewFeatures(
          viewFeaturesStr.toInt() );
    }
    ...
    QString triggerVelocityStr =
        atts.value( "triggerbasevelocity" );
    QString triggerRetuneStr = atts.value( "triggerretune" );
    QString triggerAdjustTimeStr =
        atts.value( "triggeradjusttimes" );

    QString linkerIdStr = atts.value( "linkerid" );

    QString linkerChgKeyStr =
        atts.value( "linkertransposechangekey" );
    QString linkerStepsStr =
        atts.value( "linkertransposesteps" );
    QString linkerSemitonesStr =
        atts.value( "linkertransposesemitones" );
    ...
    if( !triggerIdStr.isEmpty() ) {
      ...
      TriggerSegmentRec *rec =
          getComposition().addTriggerSegment(
              m_currentSegment, triggerIdStr.toInt(), pitch,
              velocity );
      if( rec ) {
        if( !triggerRetuneStr.isEmpty() )
          rec->setDefaultRetune( triggerRetuneStr.toLower() ==
                                 "true" );
        if( !triggerAdjustTimeStr.isEmpty() )
          rec->setDefaultTimeAdjust(
              qstrtostr( triggerAdjustTimeStr ) );
      }
      ...
      if( !linkerIdStr.isEmpty() ) {
        ...
        Segment::LinkTransposeParams params(
            ...
            linkerStepsStr.toInt(), linkerSemitonesStr.toInt(),
      ...
      getComposition().setSegmentStartTime( m_currentSegment,
                                            startTime );
    }
    ...
    // RG_DEBUG << "RoseXmlHandler::startElement - controller
    // type = " << type;
    ...
    if( type == strtoqstr( PitchBend::EventType ) )
      ...
      if( value != "" )
        m_currentSegment->addEventRuler( Controller::EventType,
                                         value.toInt() );
    }
    ...
    std::cerr << "should not be here -- audio section\n";
    // attempt to insert file into AudioFileManager
    // (this checks the integrity of the file at the
    // same time)
    //
    // if( getAudioFileManager().insertFile(
    //        qstrtostr( label ), file, id.toInt() ) == false ) {
    //  // We failed to find the audio file.  First we'll try to
    //  // set the audio file path to "a sensible default", and
    //  if
    ...
    //    } while( getAudioFileManager().insertFile(
    //                 qstrtostr( label ), newFilename,
    //                 id.toInt() ) == false );
    ...
    //  } else {
    //    // AudioPath is modified so set a document post modify
    //    // flag
    //    //
    //    // m_doc->setModified(true);
    //  }
    ...
    if( m_section != InAudioFiles ) {
      m_errorString =
          "Audiopath object found outside AudioFiles section";
      return false;
    }
    ...
    if( !search.startsWith( "/" ) &&
        !search.startsWith( "~" ) ) {
      QString docPath =
          QString::fromStdString( m_doc->getAbsFilePath() );
      QString dirPath = QFileInfo( docPath ).path();
    ...
    if( m_currentSegment->getType() != Segment::Audio ) {
      m_errorString =
          "Found audio begin index in non audio segment";
      return false;
    }
    ...
    m_currentSegment->setAudioStartTime(
    ...
    if( m_currentSegment->getType() != Segment::Audio ) {
      m_errorString =
          "found audio end index in non audio segment";
      return false;
    }
    ...
    if( markerTime < m_currentSegment->getAudioStartTime() ) {
      m_errorString =
          "Audio end index before audio start marker";
      return false;
    }

    m_currentSegment->setAudioEndTime( markerTime );

    // Ensure we set end time according to correct RealTime end
    // of Segment
    //
    RealTime realEndTime =
        getComposition().getElapsedRealTime(
            m_currentSegment->getStartTime() ) +
        m_currentSegment->getAudioEndTime() -
        m_currentSegment->getAudioStartTime();

    timeT absEnd = getComposition().getElapsedTimeForRealTime(
        realEndTime );
    ...
    if( type == "midi" ) {
      ...
      if( direction.isNull() || direction == "" ||
          direction == "play" ) { // ignore inputs

        if( !nameStr.isEmpty() ) {
          addMIDIDevice( nameStr, m_createDevices,
                         "play" ); // also sets m_device
        }
      }
      ...
      if( m_createDevices ) setMIDIDeviceName( nameStr );
      ...
      if( md ) { md->setVariationType( variation ); }
    ...
    // The contact details for the maintainer of the
    // banks/programs information.
    //
    ...
    if( m_device ) // only if we have a device
    {
      if( m_section != InStudio && m_section != InInstrument ) {
        m_errorString =
            "Found Bank outside Studio or Instrument";
        return false;
      }

      QString nameStr = atts.value( "name" );
      m_percussion =
          ( atts.value( "percussion" ).toLower() == "true" );
      ...
      //
      // File formats <1.6.0 assume the existence of bank tag
      // implies send bank select should be set to true.
      m_sendBankSelect =
          !( atts.value( "send" ).toLower() == "false" );

      // To actually create a bank
      //
      if( m_section == InStudio ) {
        // Create a new bank
        MidiBank bank( m_percussion, m_msb, m_lsb,
                       qstrtostr( nameStr ) );
    ...
    if( m_device ) // only if we have a device
    {
      if( m_section == InStudio ) {
        ...
        MidiProgram program(
            MidiBank( m_percussion, m_msb, m_lsb ), pc,
            qstrtostr( nameStr ),
        ...
        if( m_instrument ) {
          ...
          //
          // File formats <1.6.0 assume the existence of bank tag
          // implies send program change should be set to true.
          bool sendProgramChange =
              !( atts.value( "send" ).toLower() == "false" );

          MidiByte id = atts.value( "id" ).toInt();
          m_instrument->setProgramChange( id );
          m_instrument->setSendProgramChange(
              sendProgramChange );
        }
      } else {
        m_errorString =
            "Found Program outside Studio and Instrument";
        return false;
      }
    }
    ...
    // Only clear down the controllers list if we have found some
    // controllers in the RG file
    //
    if( m_device ) {
    ...
    if( !m_device ) {
      //!!! ach no, we can't give this warning -- we might be in
      //! a <device> elt
      // but have no sequencer support, for example.  we need a
      // separate m_inDevice flag
      //        m_deprecation = true;
      //        RG_WARNING << "WARNING: This Rosegarden file uses
      //        a deprecated control parameter structure.  We
      //        recommend re-saving the file from this version of
      //        Rosegarden to assure your ability to re-load it
      //        in future versions";

    } else if( m_device->getType() == Device::Midi ) {
      if( !m_haveControls ) {
        m_errorString =
            "Found ControlParameter outside Controls block";
        return false;
      }
      ...
      ControlParameter con(
          qstrtostr( name ), qstrtostr( type ),
          qstrtostr( descr ), min.toInt(), max.toInt(),
          def.toInt(), MidiByte( conVal.toInt() ),
          colour.toInt(), ipbPosition.toInt() );
      ...
      dynamic_cast<MidiDevice *>( m_device )
          ->addControlParameter( con, true );
             ...
             "reverb" ) { // deprecated but we still read 'em
    ...
    if( m_instrument )
      m_instrument->setControllerValue( MIDI_CONTROLLER_REVERB,
                                        value );

  } else if( lcName ==
             "chorus" ) { // deprecated but we still read 'em
    ...
    if( m_instrument )
      m_instrument->setControllerValue( MIDI_CONTROLLER_CHORUS,
                                        value );

  } else if( lcName ==
             "filter" ) { // deprecated but we still read 'em
    ...
    if( m_instrument )
      m_instrument->setControllerValue( MIDI_CONTROLLER_FILTER,
                                        value );

  } else if( lcName ==
             "resonance" ) { // deprecated but we still read 'em
    ...
    if( m_instrument )
      m_instrument->setControllerValue(
          MIDI_CONTROLLER_RESONANCE, value );

  } else if( lcName ==
             "attack" ) { // deprecated but we still read 'em
    ...
    if( m_instrument )
      m_instrument->setControllerValue( MIDI_CONTROLLER_ATTACK,
                                        value );

  } else if( lcName ==
             "release" ) { // deprecated but we still read 'em
    ...
    if( m_instrument )
      m_instrument->setControllerValue( MIDI_CONTROLLER_RELEASE,
                                        value );
    ...
    if( m_instrument ) {
      if( m_instrument->getType() == Instrument::Audio ||
        ...
        m_deprecation = true;
        m_instrument->setLevel( AudioLevel::multiplier_to_dB(
            float( value ) / 100.0 ) );
      } else {
        m_instrument->setControllerValue( MIDI_CONTROLLER_VOLUME,
                                          value );
    ...
    if( m_section != InBuss &&
        ( m_section != InInstrument ||
          ( m_instrument &&
            ...
            m_instrument->getType() !=
                Instrument::SoftSynth ) ) ) {
      m_errorString =
          "Found Level outside (audio) Instrument or Buss";
      return false;
    }
    ...
    //  if( bpStr.toLower() == "true" ) bypassed = true;
    ...
    //  // string, but we will accept a LADSPA UniqueId if
    //  there's
    ...
    //  RG_DEBUG << "Plugin identifier " << identifier
    //           << " -> plugin " << plugin;
    ...
    //      "
    //               << container->getId()
    //               << " has no plugin position " << position;
    //    } else {
    //      m_plugin->setAssigned( true );
    //      m_plugin->setBypass( bypassed );
    //      m_plugin->setIdentifier(
    //          qstrtostr( plugin->getIdentifier() ) );
    //      RG_DEBUG << "set identifier to plugin at position "
    ...
    //    }
    //  } else {
    //    // we shouldn't be halting import of the RG file just
    //    // because we can't match a plugin
    //    //
    //    QString q = atts.value( "id" );

    //    if( !identifier.isEmpty() ) {
    //      RG_DEBUG << "WARNING: RoseXmlHandler: plugin "
    //               << identifier << " not found";
    //      m_pluginsNotFound.insert( identifier );
    //    } else if( !q.isEmpty() ) {
    //      RG_DEBUG << "WARNING: RoseXmlHandler: plugin uid "
    //               << atts.value( "id" ) << " not found";
    //    } else {
    //      m_errorString =
    //          "No plugin identifier or uid specified";
    //      return false;
    //    }
    //  }
    //} else { // no instrument
    ...
    // if( m_plugin ) {
    //  m_plugin->setConfigurationValue( qstrtostr( key ),
    //                                   qstrtostr( value ) );
    //}
    ...
    if( m_device &&
      ...
      if( !q.isEmpty() )
        metronome.setBarPitch(
            atts.value( "barpitch" ).toInt() );

      q = atts.value( "beatpitch" );
      if( !q.isEmpty() )
        metronome.setBeatPitch(
            atts.value( "beatpitch" ).toInt() );

      q = atts.value( "subbeatpitch" );
      if( !q.isEmpty() )
        metronome.setSubBeatPitch(
            atts.value( "subbeatpitch" ).toInt() );
    ...
    InstrumentId id =
        mapToActualInstrument( atts.value( "id" ).toInt() );
    ...
    if( instrument && instrument->getType() == type ) {
      ...
      // Synth and Audio instruments always have the channel set
      // to 2. Preserve this.
      MidiByte channel =
    ...
    if( m_section == InAppearance ) {
      ...
      } else if( mapName == "generalmap" ) {
        m_colourMap =
            &m_doc->getComposition().getGeneralColourMap();
      } else { // This will change later once we get more of the
               // Appearance code sorted out
      }
    } else {
    ...
    Marker *marker = new Marker( time, qstrtostr( name ),
                                 qstrtostr( descr ) );
...
bool RoseXmlHandler::endElement( const QString &namespaceURI,
                                 const QString &localName,
                                 const QString &qName ) {
  if( getSubHandler() ) {
    bool finished;
    bool res = getSubHandler()->endElement(
        namespaceURI, localName, qName.toLower(), finished );
    if( finished ) setSubHandler( nullptr );
    return res;
  }
  ...
  if( lcName == "rosegarden-data" ) {
    ...
    // Remap all the instrument IDs in track and metronome
    // objects from "file" to "actual" IDs.  See discussion in
    ...
    for( DeviceList::iterator i = studio.getDevices()->begin();
      ...
      SoftSynthDevice *sd =
          dynamic_cast<SoftSynthDevice *>( *i );
      if( md && md->getMetronome() )
        mm = *md->getMetronome();
      else if( sd && sd->getMetronome() )
        mm = *sd->getMetronome();
      else
        continue;
...
bool RoseXmlHandler::characters( const QString &s ) {
  if( m_subHandler ) return m_subHandler->characters( s );

  return true;
}

QString RoseXmlHandler::errorString() const {
  return m_errorString;
}

bool RoseXmlHandler::error(
...
bool RoseXmlHandler::fatalError(
    const QXmlParseException &exception ) {
  m_errorString = QString( "%1 at line %2, column %3" )
                      .arg( exception.message() )
                      .arg( exception.lineNumber() )
...
bool RoseXmlHandler::endDocument() {
...
void RoseXmlHandler::setSubHandler( XmlSubHandler *sh ) {
  delete m_subHandler;
  m_subHandler = sh;
}

void RoseXmlHandler::addMIDIDevice( QString name,
  ...
  if( createAtSequencer ) {
    if( !RosegardenSequencer::getInstance()->addDevice(
            Device::Midi, deviceId, instrumentBase, devDir ) ) {
      return;
    }
  }
...
InstrumentId RoseXmlHandler::mapToActualInstrument(
    InstrumentId oldId ) {
  /*
    When we read a device from the file, we want to be able to
    add it to the studio and the sequencer immediately.  But to
    do so, we (now) need to be able to provide a base instrument
    number for the device.  We can make up a plausible one (we
    know what type of device it is, and that's the main
    determining factor) but we can't know for sure whether it's
    the same one as used in the file until we have continued and
    read the first instrument definition in the device.

    Device and instrument numbers in the file have always been
    problematic in other ways as well.  Rosegarden actually never
    used the device numbers loaded from the file (we always made
    up a new device number for each device as we read it) and it
    was theoretically possible for the instrument definitions to
    end up attached to different devices from the ones they were
    hung onto in the file (because of the way we sometimes
    re-used instruments that already existed in the studio).

    Our new approach is to assume that we will _never_ believe
    the device or instrument numbers found in the file, except
    for the purposes of resolving internal references within the
    file.  (The main examples of these are track -> instrument
    mappings outside of the studio definition, and metronome ->
    instrument mappings within a device.)  Instead, we assign a
    new device ID to each device based on its type and the
    available spare IDs; we assign a new instrument ID to each
    instrument based on the device type; and we maintain a map
    from "file" to "actual" instrument IDs as we go along, using
    this to resolve the track -> instrument and metronome ->
    instrument references at the end of the file (see the
    endElement method for rosegarden-data element).
  */

  /*
    This function is first called for any given instrument when
    first reading that instrument's definition in the context of
    a device definition.  So we know that if the instrument is
    not in the map already, then m_deviceInstrumentBase must
    contain a valid instrument base ID that was set when we added
    the device.

    To map from the number we just read, we subtract
    m_deviceReadInstrumentBase (if it exists; otherwise we set it
    as this is the first instrument for this device) and add
    m_deviceInstrumentBase.
  */

  if( m_actualInstrumentIdMap.find( oldId ) !=
      m_actualInstrumentIdMap.end() ) {
  ...
  if( m_deviceReadInstrumentBase == 0 ||
      id < m_deviceReadInstrumentBase ) {
...
void RoseXmlHandler::skipToNextPlayDevice() {
  for( DeviceList::iterator i =
           getStudio().getDevices()->begin();
