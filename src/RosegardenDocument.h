...
/**
  ...
  */
class ROSEGARDENPRIVATE_EXPORT RosegardenDocument
{
public:
     ...
     */
    RosegardenDocument( bool skipAutoload = false,
...
public:
     ...
     */
    void emitDocumentModified()  { documentModified(true); }
     ...
     */ 
    bool saveDocument(const std::string &filename, std::string& errMsg,
                      bool autosave = false);

    /// Save under a new name.
    bool saveAs(const std::string &newName, std::string &errMsg);

    /**
     * exports all or part of the studio to a file.  If devices is
     * empty, exports all devices.
     */ 
    bool exportStudio(const std::string &filename,
		      std::string &errMsg,
     ...
     */
    void setAbsFilePath(const std::string &filename);

    /**
     * returns the pathname of the current document file
     */
    const std::string &getAbsFilePath() const;
...
*/
    ...
    */
    void addOrphanedRecordedAudioFile(std::string fileName);
    void addOrphanedDerivedAudioFile(std::string fileName);
    ...
    //std::stringList getTimers();
    std::string getCurrentTimer();
    void setCurrentTimer(std::string);
...
private:
     ...
     * @see RoseXmlHandler
     */
    bool xmlParse(std::string fileContents, std::string &errMsg,
                  bool permanent,
                  bool &cancelled);
     ...
     */
    bool saveDocumentActual(const std::string &filename, std::string& errMsg,
                            bool autosave = false);
    ...
    //                 std::string extraAttributes = std::string::null);
    ...
    //static QLockFile *createLock(const std::string &absFilePath);
    //static std::string lockFilename(const std::string &absFilePath);
    ...
    std::vector<std::string> m_orphanedRecordedAudioFiles;
    std::vector<std::string> m_orphanedDerivedAudioFiles;
