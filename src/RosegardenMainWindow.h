...
/**
 * This class sets up the main window and reads the config file
 * as well as providing a menubar, toolbar and statusbar.  The
 * main widget is a RosegardenMainViewWidget, connected to the
 * RosegardenDocument.
 *
 * An instance of this class is created by main() in main.cpp.
 * That instance can be accessed from anywhere via
 * RosegardenMainWindow::self().
 *
 * This class owns many of the key objects in the system,
 * including:
 *
 ...
 *   * m_instrumentParameterBox is the "Instrument Parameters"
 * box.
 *   * getView() returns the RosegardenMainViewWidget instance
 * which contains the TrackEditor.
 *   * getTransport() returns the TransportDialog instance.
 */
class ROSEGARDENPRIVATE_EXPORT RosegardenMainWindow
  : public QMainWindow,
    public ActionFileClient {
  friend class RosegardenMainViewWidget;

public:
  /**
   * constructor of RosegardenMainWindow, calls all init
   * functions to create the application. \arg useSequencer : if
   * true, the sequencer is launched
  ...
  /** Qt generates a QCloseEvent when the user clicks the close
   * button on the title bar.  We also get a close event when
   * slotQuit() calls close().
   *
   * Control passes here where we call queryClose() to ask if the
   * user wants to save a modified document.  If queryClose()
   * returns true, we accept the close event.  Otherwise we
   * ignore it, the closing breaks, and we keep running.
   */
   ...
   * QMainWindow instance and is used by * the View class to
   * access the document object's methods
   */
   ...
   */
  virtual void mergeFile( std::string filePath ) {
    mergeFile( filePath, ImportCheckType );
  }

  /**
   * merge a file, explicitly specifying its type
   */
  void mergeFile( std::string filePath, ImportType type );
   ...
   * export a MIDI file
   */
  void exportMIDIFile( std::string url );

  /**
   * export a Csound scorefile
   */
  void exportCsoundFile( std::string url );

  /**
   * export a Mup file
   */
  void exportMupFile( std::string url );

  /**
   * export a LilyPond file
   */
  bool exportLilyPondFile( std::string url,
                           bool        forPreview = false );

  /**
   * export a MusicXml file
   */
  void exportMusicXmlFile( std::string url );
   ...
   */
  QPointer<DeviceManagerDialog> getDeviceManager() {
  ...
  virtual void jumpToTime( RealTime rt ) {
    slotJumpToTime( rt );
  }
  virtual void startAtTime( RealTime rt ) {
    slotStartAtTime( rt );
  }

  virtual void trackUp() { slotTrackUp(); }
  virtual void trackDown() { slotTrackDown(); }
  //    virtual void toggleRecordCurrentTrack() {
  //    slotToggleRecordCurrentTrack(); }

  /**
   * Create some new audio files for the sequencer and return the
   * paths for them as std::strings.
   */
  QVector<std::string> createRecordAudioFiles(
      const QVector<InstrumentId> & );
   ...
   * The result is dynamically updated depending on the
   * sequencer's status.
   */
  ...
  /*
   * Wait in a sub-event-loop until all modal dialogs from the
   * main window have been cleared
   */
   ...
   */
  AudioPluginOSCGUIManager *getPluginGUIManager() {
    return m_pluginGUIManager;
  }
  ...
  /** Query the AudioFileManager to see if the audio path exists,
   * is readable, writable, etc., and offer to dump the user in
   * the document properties editor if some problem is found.
   * This is older code that simply tests the audio path for
   * validity, and does not create anything if the audio path
   * does not exist.
   */
  bool testAudioPath(
      std::string op ); // and open the dialog to set it if unset
...
protected:
   ...
   */
  RosegardenDocument *createDocumentFromRG21File(
      std::string filePath );
   ...
   */
  //    RosegardenDocument*
  //    createDocumentFromHydrogenFile(std::string filePath);
  ...
  /**
   * creates the centerwidget of the QMainWindow instance and
   * sets it as the view
   */
  void initView();

  /**
   * queryClose is called by closeEvent() to ask whether it is OK
   * to close. This is part of a legacy KDE mechanism, but has
   * been left in place for convenience
   */
  bool queryClose();

  //!!! I left the following code here, but this is related to
  //!KDE session
  // management, and I don't think we can do anything reasonable
  // with any of this now.
  //
  /////////////////////////////////////////////////////////////////////
  /**
   * saves the window properties for each open window during
   * session end to the session config file, including saving the
   * currently opened file by a temporary filename provided by
   * KApplication.
   *
   ...
   * state including the last opened files and documents by
   * reading the temporary files saved by saveProperties()
   *
   ...
   * the framework code; the playback code uses mapped
   * compositions to send these things back asynchronously.
   */
   ...
   */
  AudioManagerDialog *getAudioManagerDialog() {
    return m_audioManagerDialog;
  }
   ...
   */
  std::string getValidWriteFileName( std::string extension,
                                     std::string label );

  /**
   * Find any non-ASCII strings in a composition that has been
   * generated by MIDI import or any other procedure that
   * produces events with unknown text encoding, and ask the user
   * what encoding to translate them from.  This assumes all text
   * strings in the composition are of the same encoding, and
   * that it is not (known to be) utf8 (in which case no
   * transcoding would be necessary).
   */
  ...
  /**
   * Open a file dialog pointing to directory, if target is
   * empty, this opens a generic file open dialog at the last
   * location the user used
   */
  void openFileDialogAt( std::string target );
...
signals:
  void startupStatusMessage( std::string message );
  ...
  /// emitted when the set of selected segments changes (relayed
  /// from RosegardenMainViewWidget)
  void segmentsSelected( const SegmentSelection & );

  /// emitted when the composition state (selected track, solo,
  /// etc...) changes
...
public slots:
  ...
  /** Update the title bar to prepend a * to the document title
   * when the document is modified. (I thought we already did
   * this ages ago, but apparenty not.)
   */
   ...
   * (protocol://foo/bar/file.rg)
   */
  virtual void slotOpenDroppedURL( std::string url );
  ...
  /**
   * clears the document in the actual view to reuse it as the
   * new document
   */
  void slotFileNew();
  ...
  /**
   * opens a file from the recent files menu (according to action
   * name)
   */
  ...
  /**
   * save a document by a new filename; if asTemplate is true,
   * the file will be saved read-only, to make it harder to
   * overwrite by accident in the future
   */
   ...
   * application.  If queryClose() returns false because the user
   * canceled the saveModified() dialog, the closing breaks.
   */
  void slotQuit();
  ...
  /**
   * Paste the clipboard at the current pointer position, moving
   * all subsequent material along to make space.
   */
  void slotPasteRange();
  ...
  /**
   * Paste just tempi and time signatures from the clipboard at
  ...
  /**
   * Quantize the selected segments by repeating the last
   * iterative quantize
   */
  ...
  /**
   * Split the selected segments on silences (or new timesig,
   * etc)
   */
  ...
  /**
   * Split the selected segments by drum, ie. each discrete pitch
   * goes into a separate segment of its own
   */
  ...
  /**
   * Expand the composition to the left by one bar, and move the
   * selected segment(s) into this space by the amount chosen
   * with a dialog
   */
  ...
  /**
   * open the default editor for each of the currently-selected
   * segments
   */
  void slotEdit();

  /**
   * open an event list view for each of the currently-selected
   * segments
   */
  void slotEditInEventList();

  /**
   * open a matrix view for each of the currently-selected
   * segments
   */
  void slotEditInMatrix();

  /**
   * open a percussion matrix view for each of the
   * currently-selected segments
   */
  void slotEditInPercussionMatrix();

  /**
   * open a pitch tracker view for the currently-selected
   * segments (NB: only accepts 1 segment)
   */
  void slotEditInPitchTracker();

  /**
   * open a notation view with all currently-selected segments in
   * it
   */
  ...
  /**
   * Edit the playback pointer position - called from a Transport
   * signal
   */
  ...
  /**
   * Reset m_docConfigDlg when the document properties dialog is
   * closing.
   */
  ...
  /**
   * Not an actual action slot : populates the
   * set_track_instrument sub menu
   */
   ...
   * Import Studio from File
   */
  void slotImportStudioFromFile( const std::string &file );
   ...
   */
  void slotStatusMsg( std::string text );

  /**
   * changes the status message of the whole statusbar for two
   * seconds, then restores the last status. This is used to
   * display statusbar messages that give information about
   * actions for toolbar icons and menuentries.
   *
   * @param text the text that is displayed in the statusbar
   */
  void slotStatusHelpMsg( std::string text );
   ...
   */
  // void slotSequencerExited(QProcess*); QThread::finished has
  // no QProcess param
  //  current slotSequenceExited implementation doesn't use (or
  //  name!) this param
  ...
  /**
   * called by RosegardenApplication when session management
   * tells it to save its state. This is to avoid saving the
   * transport as a 2nd main window
   */
   ...
   */
  void slotActivateTool( std::string toolName );
  ...
  /**
   * Set and unset the loop start/end time from the transport
   * loop start/stop buttons with these slots.
   */
   ...
   * Remove a marker
   */
  void slotDeleteMarker( int id, timeT time, std::string name,
  ...
  /**
   * This slot is here to be connected to
   * RosegardenMainViewWidget's stateChange signal.
   */
  void slotStateChanged( std::string, bool noReverse );
   ...
   */
  void slotPlayListPlay( std::string url );
  ...
  void slotPlayAudioFile( AudioFileId, const RealTime &,
                          const RealTime & );
  ...
  /**
   * Update the monitor levels from the sequencer mmapped file
   * when not playing.  Called by slotUpdateUI().
   */
  ...
  void slotPluginSelected( InstrumentId instrument, int index,
                           int plugin );

  /**
   * An external GUI has requested a port change.
   */
  void slotChangePluginPort( InstrumentId instrument, int index,
                             int portIndex, float value );

  /**
   * Our internal GUI has made a port change -- the
   * PluginPortInstance already contains the new value, but we
   * need to inform the sequencer and update external GUIs.
   */
  void slotPluginPortChanged( InstrumentId instrument, int index,
                              int portIndex );
  ...
  void slotChangePluginProgram( InstrumentId instrument,
                                int index, std::string program );

  /**
   * Our internal GUI has made a program change -- the
   * AudioPluginInstance already contains the new program, but we
   * need to inform the sequencer, update external GUIs, and
   * update the port values for the new program.
   */
  void slotPluginProgramChanged( InstrumentId instrument,
                                 int          index );

  /**
   * An external GUI has requested a configure call.  (This can
   * only happen from an external GUI, we have no way to manage
   * these internally.)
   */
  ...
  void slotPluginDialogDestroyed( InstrumentId instrument,
                                  int          index );
  void slotPluginBypassed( InstrumentId, int index,
                           bool bypassed );
  ...
  void slotShowToolHelp( const std::string & );

  void slotNewerVersionAvailable( std::string );

  void slotSetQuickMarker();

  void slotJumpToQuickMarker();

  void slotDisplayWarning( int type, std::string text,
...
private:
  /** Use QTemporaryFile to obtain a tmp filename that is
   * guaranteed to be unique
   */
  std::string getLilyPondTmpFilename();

  /** Checks to see if the audio path exists.  If it does not,
   * attempts to create it.  If creation fails, sends
   * notification to the user via the WarningWidget.  This was
   * originally an accidental overload of
   * testAudioPath(std::string op), which does some of, but not
   * all of the same work.  This method attempts to create a
   * missing audio path before doing anything else, and then
   * informs the user via the warning widget, instead of a popup
   * dialog.  If they get through all of this without fixing a
   * problem, they might wind up dealing with testAudioPath()
   * further on.
   */
  void checkAudioPath();
  ...
  /**
   * This is NOT a general-purpose progress indicator.  You want
   * to use QProgressDialog for that.
   */
  ...
  /// List of plugin dialogs to make sure we don't launch more
  /// than one.
  ...
  static std::map<QProcess *, QTemporaryFile *>
      m_lilyTempFileMap;
