...
public:
    ...
    unsigned int getSoundDriverStatus(const std::string &guiVersion);

    /// Add an audio file to the sequencer
    /**
     * @see removeAudioFile()
     */
    bool addAudioFile(const std::string &fileName, int id);
     ...
     */
    void renameDevice(unsigned int id, std::string name);
    /**
     ...
     */
    std::string getConnection(Device::DeviceType type,
                          MidiDevice::DeviceDirection direction,
     ...
     */
    std::string getConnection(DeviceId id);
     ...
     */
    std::string getTimer(unsigned int n);
     ...
     */
    void setCurrentTimer(std::string timer);
     ...
     */
    std::string setMappedPropertyList(int id,
                                  const std::string &property,
    ...
    std::vector<std::string> getPropertyList(int id,
                                         const std::string &property);

    /// Get a list of available plugins
    std::vector<std::string> getPluginInformation();

    /**
     * Nasty hack: program name/number mappings are one thing that
     * mapped object properties can't cope with
     */
    std::string getPluginProgram(int id, int bank, int program);

    /// Nastier hack: return value is bank << 16 + program
    unsigned long getPluginProgram(int id, const std::string &name);
    ...
    void segmentModified(std::shared_ptr<MappedEventBuffer>);
    void segmentAdded(std::shared_ptr<MappedEventBuffer>);
    void segmentAboutToBeDeleted(std::shared_ptr<MappedEventBuffer>);
