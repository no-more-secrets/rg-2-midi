...
Segment::Segment( const Segment &segment )
  : EventContainer(),
    ...
    m_percussionPitch( -1 ),
    m_clefKeyList( nullptr ),
    m_notifyResizeLocked(
        false ),      // To copy a segment while notifications
    ...
    m_segmentLinker( nullptr ), // yes, this is intentional.
                                // clone() handles this
    m_isTmp( segment.isTmp() ),
    m_participation( segment.m_participation ),
    m_verseCount( -1 ), // -1 => computation needed
    m_verse( 0 ) // Needs a global recomputation on the whole
                 // composition
{
  for( const_iterator it = segment.begin(); it != segment.end();
       ++it ) {
    insert( new Event( **it ) );
  }
}

Segment *Segment::cloneImpl() const {
  Segment *s = new Segment( *this );

  if( isLinked() ) {
    // if the segment is linked already, link the clone to our
    // SegmentLinker
...
Segment::~Segment() {
  if( !m_observers.empty() ) {
    cerr << "Warning: Segment::~Segment() with "
  ...
  for( it = m_eventRulerList.begin();
       it != m_eventRulerList.end(); ++it )
    delete *it;
...
bool Segment::setAsReference() {
  if( !isLinked() ) return false;
  getLinker()->setReference( this );
  return true;
}

Segment *Segment::getRealSegment() {
  if( isLinked() ) return getLinker()->getReference();
  return this;
}

const Segment *Segment::getRealSegment() const {
  if( isLinked() ) return getLinker()->getReference();
  return this;
}
...
void Segment::setGreyOut() {
  for( iterator it = begin(); it != end(); ++it ) {
    ( *it )->set<Bool>( BaseProperties::TMP, true, false );
  }
}

bool Segment::isTrulyLinked() const {
  // If there is no SegmentLinker the segment is not linked
  if( !m_segmentLinker ) return false;

  // If segment is a temporary one or is out of composition
  // return false That's arbitrary, but this method is designed
  // to be used from segments which are inside the composition.
  ...
  int numOfSegments =
      m_segmentLinker->getNumberOfLinkedSegments();
  if( numOfSegments < 2 ) return false;

  // The segment is truly linked if at least another segment
  // which is inside the composition and which is not a temporary
  // one is referenced by its linker.
  int numOfTmpSegments =
      m_segmentLinker->getNumberOfTmpSegments();
  int numOfOutsideSegments =
      m_segmentLinker->getNumberOfOutOfCompSegments();
  if( ( numOfSegments - numOfTmpSegments -
        numOfOutsideSegments ) > 1 )
    return true;
  else
    return false;
}

bool Segment::isPlainlyLinked() const {
  // A segment which is not a "true link" can't be a "plain link"
  if( !isTrulyLinked() ) return false;

  // TODO: the real work...
  // Currently the features which make the difference between a
  // link and a plain link are at the embryonic stage and are not
  // implemented through the GUI still. That's why I leave this
  // method unfinished.
  return true;
}

bool Segment::isLinkedTo( Segment *seg ) const {
  if( !m_segmentLinker ) return false;
...
void Segment::setTrack( TrackId id ) {
  ...
  if( c )
    c->weakDetachSegment( this ); // sets m_composition to 0
...
timeT Segment::getStartTime() const { return m_startTime; }

timeT Segment::getClippedStartTime() const {
  // If there is a composition, and our start time is before the
  // beginning of the composition, return the composition start
  // time.  Otherwise (all other cases) return our start time.
  if( m_composition ) {
...
timeT Segment::getEndMarkerTime( bool comp ) const {
  timeT endTime;
  ...
  if( m_type == Audio && m_composition ) {
    ...
    if( m_composition && comp ) {
      endTime =
          std::min( endTime, m_composition->getEndMarker() );
    }
  }
...
timeT Segment::getEndTime() const {
  if( m_type == Audio && m_composition ) {
    RealTime startRT =
        m_composition->getElapsedRealTime( m_startTime );
    RealTime endRT = startRT - m_audioStartTime + m_audioEndTime;
...
void Segment::setStartTime( timeT t ) {
  ...
  // reset the time of all events.  can't just setAbsoluteTime on
  // these, partly 'cos we're not allowed, partly 'cos it might
  // screw up the quantizer (which is why we're not allowed)

  // still, this is rather unsatisfactory

  FastVector<Event *> events;

  /** This is effectively calling Segment::erase on each event
  after copyMoving it.  Segment::erase did the following:
  ...
  5. updateRefreshStatuses
    ...
    below.  6 is accomplished via allEventsChanged.  2 is no
  longer wanted since we need to insert the same event.
   **/
  ...
  for( iterator i = begin(); i != end(); ++i ) {
    Event *e = *i;
    // allEventsChanged is allowed to assume the address points
    // to the Event it knew about, so we need the same object to
    // be removed and added, so we can't use copyMoving.
  ...
  if( m_composition )
    m_composition->setSegmentStartTime( this, t );
  else
    m_startTime = t;
  ...
  5. updateRefreshStatuses
      ...
      1 and 7 are done explicitly here.  2, 3 & 5 are done en
  masse. 6 is accomplished via allEventsChanged.  4 works because
  we keep the same event object.
   **/
...
void Segment::setEndMarkerTime( timeT t ) {
  ...
  if( m_type == Audio ) {
    ...
    if( m_composition ) {
      ...
      if( oldAudioEndTime != m_audioEndTime ) {
        notifyEndMarkerChange( m_audioEndTime <
                               oldAudioEndTime );
      }
    }
  } else {
...
void Segment::setEndTime( timeT t ) {
  timeT endTime = getEndTime();
  if( t < m_startTime ) t = m_startTime;
...
Segment::iterator Segment::getEndMarker() const {
  if( m_endMarkerTime ) {
...
bool Segment::isBeforeEndMarker( const_iterator i ) const {
...
void Segment::clearEndMarker() {
  delete m_endMarkerTime;
  m_endMarkerTime = nullptr;
  notifyEndMarkerChange( false );
}

const timeT *Segment::getRawEndMarkerTime() const {
  return m_endMarkerTime;
}

void Segment::updateRefreshStatuses( timeT startTime,
                                     timeT endTime ) {
  Profiler profiler( "Segment::updateRefreshStatuses()" );

  // For each observer, indicate that a refresh is needed for
  // this time span.
  for( size_t i = 0; i < m_refreshStatusArray.size(); ++i )
    m_refreshStatusArray.getRefreshStatus( i ).push( startTime,
                                                     endTime );
}
...
Segment::iterator Segment::insert( Event *e ) {
  ...
  if( t0 < m_startTime ||
    ...
    if( m_composition )
      m_composition->setSegmentStartTime( this, t0 );
    else
      m_startTime = t0;
    notifyStartChanged( m_startTime );
  }
  ...
  if( t1 > m_endTime || begin() == end() ) {
    timeT oldTime = m_endTime;
    m_endTime     = t1;
    notifyEndMarkerChange( m_endTime < oldTime );
  }

  // A segment having the TMP property should be displayed with a
  // gray colour. To do so each event in such a segment needs the
  // TMP property.
...
void Segment::updateEndTime() {
  m_endTime = m_startTime;
  for( iterator i = begin(); i != end(); ++i ) {
    timeT t =
        ( *i )->getAbsoluteTime() + ( *i )->getGreaterDuration();
...
void Segment::erase( iterator pos ) {
  ...
  if( t0 == m_startTime && begin() != end() ) {
    ...
    if( startTime != m_startTime ) {
      if( m_composition )
        m_composition->setSegmentStartTime( this, startTime );
      else
        m_startTime = startTime;
      notifyStartChanged( m_startTime );
    }
  }
  if( t1 == m_endTime ) { updateEndTime(); }
}
...
void Segment::erase( iterator from, iterator to ) {
  ...
  if( to != end() )
    endTime = ( *to )->getAbsoluteTime() +
              ( *to )->getGreaterDuration();
  ...
  if( endTime == m_endTime ) { updateEndTime(); }
...
Segment::iterator Segment::findSingle( Event *e ) {
  iterator res = end();
...
Segment::iterator Segment::findTime( timeT t ) {
  Event dummy( "dummy", t, 0, MIN_SUBORDERING );
  return lower_bound( &dummy );
}

Segment::iterator Segment::findNearestTime( timeT t ) {
...
void Segment::fillWithRests( timeT startTime, timeT endTime ) {
  ...
  if( getComposition() ) {
    sigTime =
        getComposition()->getTimeSignatureAt( startTime, ts );
  }
  ...
  cerr << "fillWithRests (" << startTime << "->" << endTime
       << "), composition "
       << ( getComposition() ? "exists" : "does not exist" )
       << ", sigTime " << sigTime << ", timeSig duration "
       << ts.getBarDuration() << ", restDuration "
       << restDuration << endl;
#endif

  DurationList dl;
  ts.getDurationListForInterval( dl, restDuration,
                                 startTime - sigTime );
...
void Segment::normalizeRests( timeT startTime, timeT endTime ) {
  Profiler profiler( "Segment::normalizeRests" );

#ifdef DEBUG_NORMALIZE_RESTS
  cerr << "normalizeRests (" << startTime << "->" << endTime
       << "), segment starts at " << m_startTime << endl;
#endif
  ...
  // the start and end times, consider separately each of the
  // sections they divide the range up into.

  Composition *composition = getComposition();
  if( composition ) {
    int timeSigNo =
        composition->getTimeSignatureNumberAt( startTime );
    if( timeSigNo < composition->getTimeSignatureCount() - 1 ) {
      timeT nextSigTime =
          composition->getTimeSignatureChange( timeSigNo + 1 )
              .first;
      if( nextSigTime < endTime ) {
#ifdef DEBUG_NORMALIZE_RESTS
        cerr << "normalizeRests: divide-and-conquer on timesig "
                "at "
             << nextSigTime << endl;
#endif
  ...
  // existing rest events can form a boundary for the range.
  // (Other events, such as controllers, may appear at any time
  // including times that are not reasonable to quantize rest
  // positions or durations to.)
  ...
  if( ib == end() ) {
    if( ib != begin() ) {
      --ib;
      // if we're pointing at the real-end-time of the last
      // event, use its notation-end-time instead
      if( endTime ==
          ( *ib )->getAbsoluteTime() + ( *ib )->getDuration() ) {
  ...
  // right duration, then we need to normalize it too.  (This
  // should be fairly harmless, all it can do wrong is extend the
  // region of interest too far and make us work too hard)
...
#ifdef DEBUG_NORMALIZE_RESTS
        cerr << "normalizeRests: scooting back to " << startTime
             << endl;
#endif
        ia = scooter;
      }
...
#ifdef DEBUG_NORMALIZE_RESTS
      cerr << "normalizeRests: erasing rest at "
...
#ifdef DEBUG_NORMALIZE_RESTS
    cerr << "normalizeRests: new end time " << m_endTime
         << " is earlier than previous segment end time "
         << segmentEndTime
         << ", extending our working end time again" << endl;
#endif
  ...
  for( iterator i = ia; i != ib && i != end(); ++i ) {
    // Boundary events for sets of rests may be notes
    // (obviously), text events (because they need to be
    // "attached" to something that has the correct timing), or
    // rests (any remaining rests in this area have tuplet data
    // so should be treated as "hard" rests);
...
#ifdef DEBUG_NORMALIZE_RESTS
    cerr << "normalizeRests: scanning: thisNoteStarts "
         << thisNoteStarts << ", lastNoteStarts "
         << lastNoteStarts << ", lastNoteEnds " << lastNoteEnds
         << endl;
#endif

    /* BR #988185: "Notation: Rest can be simultaneous with note
    but follow it"

       This conditional tested whether a note started before the
       preceding note ended, and if so inserted rests
    simultaneous with the preceding note to make up the gap.
    Without the ability to lay out those rests partwise, this is
    never any better than plain confusing.  Revert the change.
    ...
    */
    ...
    if( thisNoteStarts > lastNoteEnds ) {
#ifdef DEBUG_NORMALIZE_RESTS
      cerr << "normalizeRests: found gap between note/text/rest "
              "events from "
           << lastNoteEnds << " to " << thisNoteStarts << endl;
#endif
      gaps.push_back( std::pair<timeT, timeT>(
          lastNoteEnds, thisNoteStarts - lastNoteEnds ) );
    }

    lastNoteStarts = thisNoteStarts;
    lastNoteEnds =
        thisNoteStarts + ( *i )->getNotationDuration();
  }

  if( endTime > lastNoteEnds ) {
#ifdef DEBUG_NORMALIZE_RESTS
    cerr << "normalizeRests: need to fill up gap from last "
            "note/text/rest event end at "
         << lastNoteEnds << " to normalize end time at "
         << endTime << endl;
#endif
    gaps.push_back( std::pair<timeT, timeT>(
...
#ifdef DEBUG_NORMALIZE_RESTS
    cerr << "normalizeRests: gap " << gi << ": "
         << gaps[gi].first << " -> "
         << ( gaps[gi].first + gaps[gi].second ) << endl;
#endif
...
void Segment::getTimeSlice( timeT absoluteTime, iterator &start,
                            iterator &end ) {
  Event dummy( "dummy", absoluteTime, 0, MIN_SUBORDERING );

  // No, this won't work -- we need to include things that don't
  // compare equal because they have different suborderings, as
  // long as they have the same times
...
void Segment::getTimeSlice( timeT           absoluteTime,
...
void Segment::setDelay( timeT delay ) {
  m_delay = delay;
  if( m_composition ) {
    // don't updateRefreshStatuses() - affects playback only
    m_composition->notifySegmentEventsTimingChanged(
        this, delay, RealTime::zeroTime );
  }
}

void Segment::setRealTimeDelay( RealTime delay ) {
  m_realTimeDelay = delay;
  if( m_composition ) {
    // don't updateRefreshStatuses() - affects playback only
    m_composition->notifySegmentEventsTimingChanged( this, 0,
                                                     delay );
  }
}

void Segment::setTranspose( int transpose ) {
  m_transpose = transpose;
  notifyTransposeChange();
  if( m_composition ) {
    // don't updateRefreshStatuses() - affects playback only
    m_composition->notifySegmentTransposeChanged( this,
                                                  transpose );
  }
}

void Segment::setAudioFileId( unsigned int id ) {
  m_audioFileId = id;
  updateRefreshStatuses( getStartTime(), getEndTime() );
}

void Segment::setUnstretchedFileId( unsigned int id ) {
  m_unstretchedFileId = id;
}

void Segment::setStretchRatio( float ratio ) {
  m_stretchRatio = ratio;
}

void Segment::setAudioStartTime( const RealTime &time ) {
  m_audioStartTime = time;
  updateRefreshStatuses( getStartTime(), getEndTime() );
}

void Segment::setAudioEndTime( const RealTime &time ) {
  RealTime oldAudioEndTime = m_audioEndTime;
  m_audioEndTime           = time;
  updateRefreshStatuses( getStartTime(), getEndTime() );
...
void Segment::setAutoFade( bool value ) {
  m_autoFade = value;
  updateRefreshStatuses( getStartTime(), getEndTime() );
}

void Segment::setFadeInTime( const RealTime &time ) {
  m_fadeInTime = time;
  updateRefreshStatuses( getStartTime(), getEndTime() );
}

void Segment::setFadeOutTime( const RealTime &time ) {
  m_fadeOutTime = time;
  updateRefreshStatuses( getStartTime(), getEndTime() );
}
...
bool Segment::ClefKeyCmp::operator()( const Event *e1,
...
Clef Segment::getClefAtTime( timeT time ) const {
  timeT ctime;
  return getClefAtTime( time, ctime );
}

Clef Segment::getClefAtTime( timeT time, timeT &ctime ) const {
...
bool Segment::getNextClefTime( timeT  time,
                               timeT &nextTime ) const {
...
Key Segment::getKeyAtTime( timeT time ) const {
  timeT ktime;
  return getKeyAtTime( time, ktime );
}

Key Segment::getKeyAtTime( timeT time, timeT &ktime ) const {
  ...
  try {
    ...
    Key k( **i );
    //        std::cerr << "Segment::getKeyAtTime: Requested time
    //        " << time << ", found key " << k.getName() << " at
    //        time " << ktime << std::endl;
    return k;
  } catch( const Exception &e ) {
    std::cerr
...
bool Segment::getNextKeyTime( timeT  time,
                              timeT &nextTime ) const {
  if( !m_clefKeyList ) return false;
...
void Segment::getFirstClefAndKey( Clef &clef, Key &key ) {
  bool keyFound  = false;
  ...
  iterator i = begin();
  while( i != end() ) {
    // Keep current clef and key as soon as a note or rest event
    // is found
    if( ( *i )->isa( Note::EventRestType ) ||
...
void Segment::enforceBeginWithClefAndKey() {
  bool keyFound  = false;
  bool clefFound = false;
  ...
  while( i != end() ) {
    ...
    if( ( *i )->isa( Clef::EventType ) ) {
      clefFound = true;
      // and stop looking for next events if a key has already
      // been found
      if( keyFound ) break;
    }
    ...
    if( ( *i )->isa( Key::EventType ) ) {
      keyFound = true;
      // and stop looking for next events if a clef has already
      // been found
      if( clefFound ) break;
    }
...
timeT Segment::getRepeatEndTime() const {
...
void Segment::checkInsertAsClefKey( Event *e ) const {
...
void Segment::notifyRemove( Event *e ) const {
  ...
  if( m_clefKeyList && ( e->isa( Clef::EventType ) ||
                         e->isa( Key::EventType ) ) ) {
    ClefKeyList::iterator i;
    for( i = m_clefKeyList->find( e ); i != m_clefKeyList->end();
         ++i ) {
      // fix for bug#1485643 (crash erasing a duplicated key
      // signature)
      if( ( *i ) == e ) {
...
void Segment::notifyAppearanceChange() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Segment::notifyStartChanged( timeT newTime ) {
...
void Segment::notifyEndMarkerChange( bool shorten ) {
...
void Segment::notifyTransposeChange() {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Segment::notifySourceDeletion() const {
  for( ObserverSet::const_iterator i = m_observers.begin();
       i != m_observers.end(); ++i ) {
...
void Segment::lockResizeNotifications() {
  m_notifyResizeLocked = true;
  m_memoStart          = m_startTime;
  m_memoEndMarkerTime =
      m_endMarkerTime ? new timeT( *m_endMarkerTime ) : nullptr;
}

void Segment::unlockResizeNotifications() {
  m_notifyResizeLocked = false;
  if( m_startTime != m_memoStart )
    notifyStartChanged( m_startTime );
...
void Segment::setColourIndex( const unsigned int input ) {
  m_colourIndex = input;
  updateRefreshStatuses( getStartTime(), getEndTime() );
  if( m_composition ) m_composition->updateRefreshStatuses();
  notifyAppearanceChange();
...
void Segment::addEventRuler( const std::string &type,
                             int controllerValue, bool active ) {
  EventRulerListConstIterator it;

  for( it = m_eventRulerList.begin();
       it != m_eventRulerList.end(); ++it )
    if( ( *it )->m_type == type &&
        ( *it )->m_controllerValue == controllerValue )
      return;

  m_eventRulerList.push_back(
      new EventRuler( type, controllerValue, active ) );
}

bool Segment::deleteEventRuler( const std::string &type,
                                int controllerValue ) {
  EventRulerListIterator it;
...
Segment::EventRuler *Segment::getEventRuler(
    const std::string &type, int controllerValue ) {
  EventRulerListConstIterator it;
  for( it = m_eventRulerList.begin();
       it != m_eventRulerList.end(); ++it )
    if( ( *it )->m_type == type &&
...
SegmentMultiSet &Segment::getCompositionSegments() {
...
void SegmentRefreshStatus::push( timeT from, timeT to ) {
  if( !needsRefresh() ) { // don't do anything subtle - just
                          // erase the old data
...
/// YG: Only for debug
void Segment::dumpObservers() {}

void SegmentObserver::allEventsChanged( const Segment *s ) {
...
EventContainer::iterator EventContainer::findEventOfType(
    EventContainer::iterator i, const std::string &type ) {
  for( ; i != end(); ++i ) {
    Event *e = *i;
