...
SegmentLinker::SegmentLinker() {
  ...
  // &CommandHistory::updateLinkedSegments,
...
SegmentLinker::findParamsItrForSegment( Segment *s ) {
  ...
  for( itr = m_linkedSegmentParamsList.begin();
    ...
    if( itr->m_linkedSegment == s ) { break; }
  }

  return itr;
}

void SegmentLinker::addLinkedSegment( Segment *s ) {
  LinkedSegmentParamsList::iterator itr =
      findParamsItrForSegment( s );
  if( itr == m_linkedSegmentParamsList.end() ) {
    m_linkedSegmentParamsList.push_back(
        LinkedSegmentParams( s ) );
    s->setLinker( this );
  }
}

void SegmentLinker::removeLinkedSegment( Segment *s ) {
  LinkedSegmentParamsList::iterator itr =
      findParamsItrForSegment( s );
  if( itr != m_linkedSegmentParamsList.end() ) {
...
/*static*/ Segment *SegmentLinker::createLinkedSegment(
    Segment *s ) {
...
/*static*/ void SegmentLinker::handleImpliedCMajor(
    Segment *s ) {
  // need to handle implied C Major in segments without a key
  // change event at the start of them

  // rather than keep having to write "handle implied C Major key
  // in segments with no key change event at segment start" code,
  // i'm just going to stick a null-op C Major key at the start
  // (if no key already exists there)
  ...
  for( Segment::const_iterator itr = itrFrom; itr != itrTo;
       ++itr ) {
...
/*static*/ bool SegmentLinker::unlinkSegment( Segment *s ) {
  bool retVal = false;
  ...
  if( s->isLinked() ) {
    ...
    // in the case that the linker has no more linked segments,
    // delete the segment linker
...
void SegmentLinker::slotUpdateLinkedSegments(
    Command *command ) {
  ////only the first segment with an invalidated refresh region
  ///will be /processed. If there are others, their changes will
  ///be ignored.
  ...
  //    linkedSeg->getRefreshStatus(refreshStatusId);
  ...
  //        }
  //    } else {
  //        RG_WARNING << "oops, trying to update linked segment
  //        set twice!";
  //    }
...
void SegmentLinker::linkedSegmentChanged( Segment *   s,
                                          const timeT from,
                                          const timeT to ) {
  // go through the other linked segments which aren't s, and
  // copy the events in the range [from,to] to them, accounting
  // for time and pitch shifts
  ...
  for( itr = m_linkedSegmentParamsList.begin();
    ...
    if( s == linkedSegToUpdate ) { continue; }
    ...
    Segment::iterator itrFrom =
        linkedSegToUpdate->findTime( segFrom );
    Segment::iterator itrTo =
        linkedSegToUpdate->findTime( segTo );
    lyricsChanged = eraseNonIgnored( linkedSegToUpdate, itrFrom,
                                     itrTo, lyricsChanged );

    // now go through s from 'from' to 'to', inserting the
    // equivalent event in linkedSegToUpdate
    for( Segment::const_iterator itr = s->findTime( from );
         itr != s->findTime( to ); ++itr ) {
      const Event *e = *itr;

      timeT eventT =
          ( e->getAbsoluteTime() - sourceSegStartTime ) +
          segStartTime;

      timeT eventNotationT =
          ( e->getNotationAbsoluteTime() - sourceSegStartTime ) +
          segStartTime;

      int semitones = linkedSegToUpdate->getLinkTransposeParams()
                          .m_semitones -
                      s->getLinkTransposeParams().m_semitones;
      int steps =
          linkedSegToUpdate->getLinkTransposeParams().m_steps -
          s->getLinkTransposeParams().m_steps;

      lyricsChanged = insertMappedEvent(
          linkedSegToUpdate, e, eventT, eventNotationT,
          semitones, steps, lyricsChanged );
    }

    // Fix verses count if lyrics have been modified
    if( lyricsChanged )
      linkedSegToUpdate->invalidateVerseCount();

    // Now only send one resize notification to observers if
    // needed.
...
bool SegmentLinker::insertMappedEvent(
    Segment *seg, const Event *e, timeT t, timeT nt,
    int semitones, int steps, bool lyricsAlreadyInserted ) {
  bool lyricInserted = lyricsAlreadyInserted;

  bool ignore;
  if( e->get<Bool>( BaseProperties::LINKED_SEGMENT_IGNORE_UPDATE,
                    ignore ) &&
      ignore ) {
  ...
  Event *refSegEvent =
      new Event( *e, t, e->getDuration(), e->getSubOrdering(),
                 nt, e->getNotationDuration() );
  ...
  if( semitones != 0 ) {
    if( e->isa( Note::EventType ) ) {
      ...
      Rosegarden::Key trKey = ( Rosegarden::Key( *e ) )
  ...
  if( needsInsertion ) {
    if( !lyricInserted ) {
      ...
      if( e->isa( Text::EventType ) ) {
        std::string textType;
        lyricInserted =
            e->get<String>( Text::TextTypePropertyName,
                            textType ) &&
            ( textType == Text::Lyric );
      }
    }
...
bool SegmentLinker::eraseNonIgnored(
    Segment *s, Segment::const_iterator itrFrom,
    Segment::const_iterator itrTo, bool lyricsAlreadyErased ) {
  bool lyricErased = lyricsAlreadyErased;

  // only erase items which aren't ignored for link purposes
  Segment::iterator eraseItr;
  for( eraseItr = itrFrom;
       eraseItr != s->end() && eraseItr != itrTo; ) {
    bool ignore = false;
    ( *eraseItr )
        ->get<Bool>(
            BaseProperties::LINKED_SEGMENT_IGNORE_UPDATE,
            ignore );
    if( !ignore ) {
      if( !lyricErased ) {
...
void SegmentLinker::clearRefreshStatuses() {
  LinkedSegmentParamsList::iterator itr;
  for( itr = m_linkedSegmentParamsList.begin();
       itr != m_linkedSegmentParamsList.end(); ++itr ) {
    LinkedSegmentParams &linkedSegParams = *itr;
...
void SegmentLinker::refreshSegment( Segment *seg ) {
  timeT startTime = seg->getStartTime();
  eraseNonIgnored( seg, seg->begin(), seg->end(), true );
  // Last parameter set to true to avoid an useless search for
  // lyrics
  ...
  for( segitr = sourceSeg->begin(); segitr != sourceSeg->end();
       ++segitr ) {
    const Event *refEvent = *segitr;

    timeT refEventTime =
        refEvent->getAbsoluteTime() - sourceSegStartTime;
    timeT freshEventTime = refEventTime + startTime;
    timeT refEventNotationTime =
        refEvent->getNotationAbsoluteTime() - sourceSegStartTime;
    timeT freshEventNotationTime =
        refEventNotationTime + startTime;

    insertMappedEvent(
        seg, refEvent, freshEventTime, freshEventNotationTime,
        seg->getLinkTransposeParams().m_semitones,
        seg->getLinkTransposeParams().m_steps, true );
    // Last parameter set to true to avoid an useless search for
    // lyrics
  }

  if( tempClone ) { delete tempClone; }
}

int SegmentLinker::getNumberOfTmpSegments() const {
  int count = 0;
...
int SegmentLinker::getNumberOfOutOfCompSegments() const {
  int count = 0;

  LinkedSegmentParamsList::const_iterator it;
  for( it = m_linkedSegmentParamsList.begin();
       it != m_linkedSegmentParamsList.end(); ++it ) {
    if( ( *it ).m_linkedSegment->isTmp() )
      continue; // Ignore tmp segs
...
SegmentLinker::LinkedSegmentParams::LinkedSegmentParams(
    Segment *s )
  : m_linkedSegment( s ),
