...
//! calls
// to getAbsoluteTime/getDuration -- the vast majority should
// almost certainly now be using
// getNotationAbsoluteTime/getNotationDuration

Segment::iterator
SegmentNotationHelper::findNotationAbsoluteTime( timeT t ) {
  iterator i( segment().findTime( t ) );

  // We don't know whether the notation absolute time t will
  // appear before or after the real absolute time t.  First scan
  // backwards until we find a notation absolute time prior to
  // (or equal to) t, and then scan forwards until we find the
  // first one that isn't prior to t
...
Segment::iterator
SegmentNotationHelper::findNearestNotationAbsoluteTime(
    timeT t ) {
  iterator i( segment().findTime( t ) );

  // Exactly findNotationAbsoluteTime, only with the two scan
  // loops in the other order
...
void SegmentNotationHelper::setNotationProperties(
  ...
  for( Segment::iterator i = from;
    ...
    if( ( *i )->has( BEAMED_GROUP_TUPLET_BASE ) ) {
      ...
      if( tcount == 0 ) {
        ...
        // nominal duration is longer than actual (sounding)
        // duration
        ...
        /*!!!
                        if ((*i)->has(IS_GRACE_NOTE) &&
                            ...
                            if (!justSeenGraceNote) {
                                graceNoteStart =
           (*i)->getNotationAbsoluteTime(); justSeenGraceNote =
           true;
                            }
                            ...
                            duration +=
           (*i)->getNotationAbsoluteTime() - graceNoteStart;
...
timeT SegmentNotationHelper::getNotationEndTime( Event *e ) {
  return e->getNotationAbsoluteTime() + e->getNotationDuration();
}

Segment::iterator SegmentNotationHelper::getNextAdjacentNote(
    iterator i, bool matchPitch, bool allowOverlap ) {
  iterator j( i );
  ...
  long  ip = 0, jp = 0;
  if( !( *i )->get<Int>( PITCH, ip ) && matchPitch )
    return end();

  while( true ) {
    if( !isBeforeEndMarker( j ) || !isBeforeEndMarker( ++j ) )
      return end();
...
Segment::iterator SegmentNotationHelper::getPreviousAdjacentNote(
    iterator i, timeT rangeStart, bool matchPitch,
    bool allowOverlap ) {
  iterator j( i );
  if( !isBeforeEndMarker( i ) ) return i;
  if( !( *i )->isa( Note::EventType ) ) return end();
  ...
  while( true ) {
    ...
    // don't consider notes that end after i ends or before i
    // begins
...
Segment::iterator SegmentNotationHelper::findContiguousNext(
    iterator el ) {
  std::string elType = ( *el )->getType(), reject, accept;
...
Segment::iterator SegmentNotationHelper::findContiguousPrevious(
    iterator el ) {
...
bool SegmentNotationHelper::noteIsInChord( Event *note ) {
  ...
  /*!!!
      ...
      segment().getTimeSlice(note->getAbsoluteTime(), first,
     second);
...
//!!! This doesn't appear to be used any more and may well not
//! work.
// Ties are calculated in several different places, and it's odd
// that we don't have a decent API for them
Segment::iterator SegmentNotationHelper::getNoteTiedWith(
    Event *note, bool forwards ) {
  bool tied = false;

  if( !note->get<Bool>( forwards ? BaseProperties::TIED_FORWARD
  ...
  for( ;; ) {
    i = forwards ? findContiguousNext( i )
                 : findContiguousPrevious( i );

    if( !isBeforeEndMarker( i ) ) return end();
    if( ( *i )->getAbsoluteTime() == myTime ) continue;

    if( forwards &&
        ( ( *i )->getAbsoluteTime() != myTime + myDuration ) ) {
      return end();
    }
    if( !forwards && ( ( ( *i )->getAbsoluteTime() +
                         ( *i )->getDuration() ) != myTime ) ) {
      return end();
    }
    ...
    if( ( *i )->get<Int>( BaseProperties::PITCH ) == myPitch )
      return i;
  }
...
bool SegmentNotationHelper::collapseRestsIfValid(
    Event *e, bool &collapseForward ) {
  iterator elPos = segment().findSingle( e );
  if( elPos == end() ) return false;

  timeT myDuration = ( *elPos )->getNotationDuration();

  // findContiguousNext won't return an iterator beyond the end
  // marker
  ...
  // need to check adjacency using e.g. getNextAdjacentNote if
  // this method were to work for notes as well as rests.

  // collapse to right if (a) not at end...
  if( nextEvent != end() &&
      // ...(b) rests can be merged to a single, valid unit
      isCollapseValid( ( *nextEvent )->getNotationDuration(),
                       myDuration ) &&
  ...
  // logic is exactly backwards from collapse to right logic
  // above
  if( previousEvent != end() &&
      isCollapseValid( ( *previousEvent )->getNotationDuration(),
                       myDuration ) &&
      ( *previousEvent )->getAbsoluteTime() >
          segment().getBarStartForTime(
              e->getAbsoluteTime() ) ) {
    // collapse left is OK; collapse e with previousEvent
    Event *e1( new Event(
        **previousEvent, ( *previousEvent )->getAbsoluteTime(),
        e->getDuration() + ( *previousEvent )->getDuration() ) );
...
bool SegmentNotationHelper::isSplitValid( timeT a, timeT b ) {
  return ( isViable( a ) && isViable( b ) );
}

Segment::iterator SegmentNotationHelper::splitIntoTie(
    iterator &i, timeT baseDuration ) {
  if( i == end() ) return end();
  iterator i2;
...
Segment::iterator SegmentNotationHelper::splitIntoTie(
    iterator &from, iterator to, timeT baseDuration ) {
  // so long as we do the quantization checks for validity before
  // calling this method, we should be fine splitting precise
  // times in this method. only problem is deciding not to split
  // something if its duration is very close to requested
  // duration, but that's probably not a task for this function
  ...
  if( segment().isBeforeEndMarker( ni ) &&
      segment().isBeforeEndMarker( ++ni ) ) {
  ...
  for( iterator i = from; i != to; ++i ) {
    ...
    if( baseDuration >= eventDuration ) { continue; }
...
bool SegmentNotationHelper::isViable( timeT duration,
  ...
  */
  ...
  timeT nearestDuration =
      Note::getNearestNote( duration, dots >= 0 ? dots : 2 )
          .getDuration();
...
Event *SegmentNotationHelper::makeThisNoteViable(
    iterator noteItr, bool splitAtBars ) {
  // We don't use quantized values here; we want a precise
  // division. Even if it doesn't look precise on the score
  // (because the score is quantized), we want any playback to
  // produce exactly the same duration of note as was originally
  // recorded
  ...
  // A list of durations that will satisfactorily break up this
  // event.
  DurationList dl;
  ...
  timeT maxdur =
      ( *i )->getAbsoluteTime() + ( *i )->getDuration() - qt;

  if( maxdur <= 0 ) { return *noteItr; }

  // While we've not yet accumulated the required amount of time,
  // build the duration list (dl).
  while( acc < required ) {
    timeT remaining = required - acc;
    if( splitAtBars ) {
      timeT thisNoteStart = qt + acc;
      timeT toNextBar =
          segment().getBarEndForTime( thisNoteStart ) -
          thisNoteStart;
      if( toNextBar > 0 && remaining > toNextBar )
        remaining = toNextBar;
    }
    timeT component =
        Note::getNearestNote( remaining ).getDuration();
    timeT dur =
        ( component > ( required - acc ) ? ( required - acc )
                                         : component );

    // #1517: In the scenario where there is a whole note (4
    // beats) in a single measure of 2/2, maxdur == dur and this
    // is exactly as it should be.  I dug around in here and all
    // the numbers were perfect for that case, and the only fault
    // was the <= test condition was kicking this out when the
    // two values were ==.  I changed the test to a < and fixed
    // the bug.  The most obvious consequence of this change is
    // that it is no longer possible to put a semibreve into a
    // single measure of 4/4 time.  It splits into two tied whole
    // notes.  This seems perfectly acceptable and reasonable to
    // me (dmm) so I decided to just go with this and move along.
    if( maxdur < dur ) {
  ...
  // For each duration
  for( DurationList::iterator dli = dl.begin(); dli != dl.end();
       ++dli ) {
    ...
    if( !splits.first || !splits.second ) {
      // Bug #3466912
      // There's a situation where an event might be
      // unsplittable, and it could cause an endless loop because
      // its notation duration is longer than the event duration.
      // The following check will make sure the notation duration
      // is truncated to the event duration thus preventing the
      // endless loop.

      // Bug #1419
      //
      // The above referenced check obviously didn't work
      // correctly.  The following code block used to be inside a
      // test to see if performance and notation duration were
      // the same, which is antithetical to the above stated goal
      // of the following code block.  Without doing any thinking
      // on the greater picture here, I decided to try removing
      // the test entirely.  The thought was if we're in this "no
      // valid split" code block anyway, then let's just hit it
      // with a hammer in every case and try to avoid infinite
      // loops!
      //
      // It successfully addresses the infinite loop in #1419
      // with no immediately obvious consequences, although I'm
      // sure there are hidden consequences.

      // Create a new event with the notation abs time and
      // duration set to the event abs time and duration.
      Event *e1 = new Event(
          *e, e->getAbsoluteTime(), e->getDuration(), // event
          e->getSubOrdering(), e->getAbsoluteTime(),
          e->getDuration() ); // notation
...
void SegmentNotationHelper::makeNotesViable( iterator from,
                                             iterator to,
                                             bool splitAtBars ) {
  //  std::vector<Event *> toInsert;
  ...
  for( Segment::iterator i = from, j = i;
    ...
    // We keep a second iterator to make sure we aren't modifying
    // the event that our iterator points to.
    ++j;
...
void SegmentNotationHelper::makeNotesViable( timeT startTime,
                                             timeT endTime,
                                             bool splitAtBars ) {
...
Segment::iterator SegmentNotationHelper::insertNote(
    timeT absoluteTime, Note note, int pitch,
    Accidental explicitAccidental ) {
  Event *e = new Event( Note::EventType, absoluteTime,
                        note.getDuration() );
...
Segment::iterator SegmentNotationHelper::insertNote(
  ...
  // existing event, and if we're inserting over a rest, split
  // the rest at the insertion time first.

  if( i != end() && ( *i )->getAbsoluteTime() < absoluteTime &&
...
Segment::iterator SegmentNotationHelper::insertRest(
    timeT absoluteTime, Note note ) {
  iterator i, j;
  ...
  Event *modelEvent = new Event(
      Note::EventRestType, absoluteTime, note.getDuration(),
...
// return an iterator pointing to the "same" event as the
// original iterator (which will have been replaced)

Segment::iterator SegmentNotationHelper::collapseRestsForInsert(
    iterator i, timeT desiredDuration ) {
  ...
  iterator j = findContiguousNext(
      i ); // won't return itr after end marker
...
Segment::iterator SegmentNotationHelper::insertSomething(
    iterator i, int duration, Event *modelEvent,
    bool tiedBack ) {
  // Rules:
  //
  // 1. If we hit a bar line in the course of the intended
  // inserted note, we should split the note rather than make the
  // bar the wrong length.  (Not implemented yet) [NB. This is
  // now implemented, but not here -- see end of
  // NoteInsertionCommand::modifySegment --cc, 20110428]
  //
  // 2. If there's nothing at the insertion point but rests (and
  // enough of them to cover the entire duration of the new
  // note), then we should insert the new note/rest literally and
  // remove rests as appropriate.  Rests should never prevent us
  // from inserting what the user asked for.
  //
  // 3. If there are notes in the way of an inserted note,
  // however, we split whenever "reasonable" and truncate our
  // user's note if not reasonable to split.  We can't always
  // give users the Right Thing here, so to hell with them.
  ...
  if( i == end() ) {
    return insertSingleSomething( i, duration, modelEvent,
                                  tiedBack );
  }

  // If there's a rest at the insertion position, merge it with
  // any following rests, if available, until we have at least
  // the duration of the new note.
  i = collapseRestsForInsert( i, duration );

  timeT existingDuration = ( *i )->getNotationDuration();

  //    RG_DEBUG << "SegmentNotationHelper::insertSomething:
  //    asked to insert duration " << duration
  ...
  if( duration == existingDuration ) {
    ...
    // 2. If the new note or rest is shorter than an existing
    // one, split the existing one and chord or replace the first
    // part.

    if( ( *i )->isa( Note::EventType ) ) {
      if( !isSplitValid( duration,
                         existingDuration - duration ) ) {
        //                RG_DEBUG << "Bad split, coercing new
        //                note";

        // not reasonable to split existing note, so force new
        // one to same duration instead
        duration = ( *i )->getNotationDuration();

      } else {
        //                RG_DEBUG << "Good split, splitting old
        //                event";
      ...
      // (we duck out of this if we find we're in a tupleted
      // zone)

      if( last != end() &&
          !( *last )->has( BEAMED_GROUP_TUPLET_BASE ) ) {
        makeRestViable( last );
      }
    }
    ...
    // the first part is the same duration as the existing note
    // or rest, and recurse to step 1 with both the first and the
    // second part in turn.

    bool needToSplit = true;

    // special case: existing event is a rest, and it's at the
    // end of the segment
    ...
    if( needToSplit ) {
      ...
      i = insertSingleSomething( i, existingDuration, modelEvent,
                                 tiedBack );
      ...
      while( i != end() &&
             ( ( *i )->getNotationAbsoluteTime() <
               ( insertedTime + existingDuration ) ) )
        ++i;

      return insertSomething( i, duration - existingDuration,
                              modelEvent, true );
...
Segment::iterator SegmentNotationHelper::insertSingleSomething(
    iterator i, int duration, Event *modelEvent,
    bool tiedBack ) {
  timeT time;
  timeT notationTime;
  ...
  Event *e =
      new Event( *modelEvent, time, effectiveDuration,
                 modelEvent->getSubOrdering(), notationTime );

  // If the model event already has group info, I guess we'd
  // better use it!
...
void SegmentNotationHelper::setInsertedNoteGroup( Event *  e,
                                                  iterator i ) {
  // Formerly this was posited on the note being inserted between
  // two notes in the same group, but that's quite wrong-headed:
  // we want to place it in the same group as any existing note
  // at the same time, and otherwise leave it alone.
  ...
  while( isBeforeEndMarker( i ) &&
    ...
    if( ( *i )->has( BEAMED_GROUP_ID ) ) {
      ...
      if( ( *i )->has( BEAMED_GROUP_TUPLET_BASE ) ) {
        e->set<Int>(
            BEAMED_GROUP_TUPLET_BASE,
            ( *i )->get<Int>( BEAMED_GROUP_TUPLET_BASE ) );
        e->set<Int>(
            BEAMED_GROUP_TUPLED_COUNT,
            ( *i )->get<Int>( BEAMED_GROUP_TUPLED_COUNT ) );
        e->set<Int>(
            BEAMED_GROUP_UNTUPLED_COUNT,
            ( *i )->get<Int>( BEAMED_GROUP_UNTUPLED_COUNT ) );
      }
...
Segment::iterator SegmentNotationHelper::insertClef(
    timeT absoluteTime, Clef clef ) {
  return insert( clef.getAsEvent( absoluteTime ) );
}

Segment::iterator SegmentNotationHelper::insertSymbol(
    timeT absoluteTime, Symbol symbol ) {
  return insert( symbol.getAsEvent( absoluteTime ) );
}

Segment::iterator SegmentNotationHelper::insertKey(
    timeT absoluteTime, Key key ) {
  return insert( key.getAsEvent( absoluteTime ) );
}

Segment::iterator SegmentNotationHelper::insertText(
    timeT absoluteTime, Text text ) {
  return insert( text.getAsEvent( absoluteTime ) );
}

void SegmentNotationHelper::deleteNote( Event *e,
                                        bool   collapseRest ) {
  ...
  if( ( *i )->has( TIED_BACKWARD ) &&
      ( *i )->get<Bool>( TIED_BACKWARD ) ) {
    iterator j = getPreviousAdjacentNote(
        i, segment().getStartTime(), true, false );
    if( j != end() ) {
      ( *j )->unset(
          TIED_FORWARD ); // don't even check if it has it set
    }
  }

  if( ( *i )->has( TIED_FORWARD ) &&
      ( *i )->get<Bool>( TIED_FORWARD ) ) {
    iterator j = getNextAdjacentNote( i, true, false );
    if( j != end() ) {
      ( *j )->unset(
          TIED_BACKWARD ); // don't even check if it has it set
    }
  }

  // If any notes start at the same time as this one but end
  // first, or start after this one starts but before it ends,
  // then we go for the delete-event-and-normalize-rests option.
  // Otherwise (the notationally simpler case) we go for the
  // replace-note-by-rest option.  We still lose in the case
  // where another note starts before this one, overlaps it, but
  // then also ends before it does -- but I think we can live
  // with that.
  ...
  while( j != end() && ( *j )->getAbsoluteTime() < endTime ) {
    ...
    } else if( ( ( *j )->getAbsoluteTime() +
                 ( *j )->getDuration() ) < endTime ) {
      complicatedOverlap = true;
    }
  ...
  if( noteIsInChord( e ) ) {
    ...
    if( e->has( BEAMED_GROUP_TUPLET_BASE ) == false ) {
      // replace with a rest
      Event *newRest =
          new Event( Note::EventRestType, e->getAbsoluteTime(),
                     dur, Note::EventRestSubOrdering );
      insert( newRest );
      erase( i );
      ...
      if( count > 1 ) {
        ...
        insertRest( ( *i )->getAbsoluteTime(),
                    Note( note_type, 0 ) );
      } else {
        // replace with a rest
        timeT  time    = ( *begin )->getAbsoluteTime();
        Event *newRest = new Event(
            Note::EventRestType, ( *begin )->getAbsoluteTime(),
...
bool SegmentNotationHelper::hasEffectiveDuration( iterator i ) {
  ...
  if( ( *i )->isa( Note::EventType ) ) {
    iterator i0( i );
    if( ++i0 != end() && ( *i0 )->isa( Note::EventType ) &&
...
void SegmentNotationHelper::makeBeamedGroup( timeT  from,
                                             timeT  to,
                                             string type ) {
  makeBeamedGroupAux( segment().findTime( from ),
                      segment().findTime( to ), type, false );
}

void SegmentNotationHelper::makeBeamedGroup( iterator from,
                                             iterator to,
                                             string   type ) {
  makeBeamedGroupAux(
      ( from == end() )
      ...
      ( to == end() )
          ? to
          : segment().findTime( ( *to )->getAbsoluteTime() ),
      type, false );
}

void SegmentNotationHelper::makeBeamedGroupExact( iterator from,
                                                  iterator to,
                                                  string type ) {
...
void SegmentNotationHelper::makeBeamedGroupAux(
    iterator from, iterator to, string type, bool groupGraces ) {
  //    RG_DEBUG << "SegmentNotationHelper::makeBeamedGroupAux:
  //    type " << type; if (from == to) RG_DEBUG << "from == to";

  int  groupId         = segment().getNextId();
  bool beamedSomething = false;

  for( iterator i = from; i != to; ++i ) {
    //        RG_DEBUG << "looking at " << (*i)->getType() << "
    //        at " << (*i)->getAbsoluteTime();
    ...
    if( ( *i )->isa( Note::EventType ) &&
      ...
      while( ++j != to ) {
        if( ( *j )->getType() == Note::EventType &&
            ( *j )->getNotationAbsoluteTime() >
                ( *i )->getNotationAbsoluteTime() &&
            ( *j )->getNotationDuration() <
                Note( Note::Crotchet ).getDuration() ) {
          somethingLeft = true;
          break;
        }
...
void SegmentNotationHelper::makeTupletGroup( timeT t,
                                             int   untupled,
                                             int   tupled,
                                             timeT unit ) {
  int groupId = segment().getNextId();
  ...
  for( iterator i = segment().findTime( t ); i != end(); ++i ) {
    ...
    if( ( *i )->getNotationAbsoluteTime() >=
        notationTime + ( untupled * unit ) )
      break;

    timeT offset =
        ( *i )->getNotationAbsoluteTime() - notationTime;
    timeT duration = ( *i )->getNotationDuration();

    if( ( *i )->isa( Note::EventRestType ) &&
        ( ( offset + duration ) > ( untupled * unit ) ) ) {
      fillWithRestsTo = std::max(
          fillWithRestsTo, notationTime + offset + duration );
    ...
    Event *e = new Event(
        **i, notationTime + ( offset * tupled / untupled ),
...
void SegmentNotationHelper::unbeam( timeT from, timeT to ) {
  unbeamAux( segment().findTime( from ),
             segment().findTime( to ) );
}

void SegmentNotationHelper::unbeam( iterator from,
                                    iterator to ) {
  unbeamAux(
      ( from == end() )
          ? from
...
void SegmentNotationHelper::unbeamAux( iterator from,
                                       iterator to ) {
  for( iterator i = from; i != to; ++i ) {
...
/*
  ...
  Auto-beaming code derived from X11 Rosegarden's
  ItemListAutoBeam and ItemListAutoBeamSub in
  editor/src/ItemList.c.

*/

void SegmentNotationHelper::autoBeam( timeT from, timeT to,
                                      string type ) {
  /*
  RG_DEBUG << "autoBeam from " << from << " to " << to << " on
  segment start time " << segment().getStartTime() << ", end time
  " << segment().getEndTime() << ", end marker " <<
  segment().getEndMarkerTime();
  */

  autoBeam( segment().findTime( from ), segment().findTime( to ),
            type );
}

void SegmentNotationHelper::autoBeam( iterator from, iterator to,
                                      string type ) {
  // This can only manage whole bars at a time, and it will split
  // the from-to range out to encompass the whole bars in which
  // they each occur

  if( !segment().getComposition() ) { return; }
  ...
  // Get bar range for selection.  Since this is inclusive, we
  // subtract 1 from the "to" time to ensure we don't get an
  // additional bar (bug #703).

  int fromBar =
      comp->getBarNumber( ( *from )->getAbsoluteTime() );
  int toBar = comp->getBarNumber(
      segment().isBeforeEndMarker( to )
          ? ( *to )->getAbsoluteTime() - 1
          : segment().getEndMarkerTime() - 1 );

  for( int barNo = fromBar; barNo <= toBar; ++barNo ) {
    std::pair<timeT, timeT> barRange =
        comp->getBarRange( barNo );
    ...
    while( barStart != segment().end() &&
           ( *barStart )->getNotationAbsoluteTime() <
               barRange.first )
      ++barStart;
    ...
    while( barEnd != segment().end() &&
           ( *barEnd )->getNotationAbsoluteTime() <
               barRange.second )
      ++barEnd;
    ...
    TimeSignature timeSig =
        segment().getComposition()->getTimeSignatureAt(
            barRange.first );
...
/*
  ...
  its sleep-disturbing aura, because it sounds like the sort of
  music which would be going around in the gunman's head as he
  trains a laser sight into your bedroom through the narrow gap
  in your curtains and dances the little red dot around nervously
  on your wall."

*/

void SegmentNotationHelper::autoBeamBar( iterator      from,
                                         iterator      to,
                                         TimeSignature tsig,
                                         string        type ) {
  ...
  if( denom == 2 || denom == 4 ) {
    ...
    if( num % 3 == 0 &&
        denom == 8 ) { // special hack for 6/8, 12/8 etc...
...
void SegmentNotationHelper::autoBeamBar(
    iterator from, iterator to, timeT average, timeT minimum,
  ...
  for( iterator i = from; i != to && i != e; ++i ) {
    ...
    if( accumulator % average ==
      ...
      iterator k =
          end(); // best-so-far last item in group;
      ...
      for( iterator j = i; j != to; ++j ) {
        ...
        if( ( count > maximum ) || ( longerThanDemi > 4 ) ||
            ( ++jnext == to ) ||
...
// based on X11 Rosegarden's GuessItemListClef in
// editor/src/MidiIn.c

Clef SegmentNotationHelper::guessClef( iterator from,
                                       iterator to ) {
  long totalHeight = 0;
  int  noteCount   = 0;
  ...
  // Let's try these new extents and see how the fare.  Ideally
  // these should pick plain treble and bass clefs a reasonable
  // amount of the time, and not be too prone to picking
  // transposed clefs, while picking transposed clefs when really
  // necessary.
  if( average < -12 )
    return Clef( Clef::Bass, -2 );
  else if( average < -9 )
    return Clef( Clef::Bass, -1 );
  else if( average < -6 )
    return Clef( Clef::Bass );
  else if( average < -3 )
    return Clef( Clef::Tenor );
  else if( average < 1 )
    return Clef( Clef::Alto );
  else if( average < 12 )
    return Clef( Clef::Treble );
  else if( average < 24 )
    return Clef( Clef::Treble, 1 );
  else if( average < 48 )
    return Clef( Clef::Treble, 2 );
  else
    return Clef( Clef::Treble );
}

bool SegmentNotationHelper::removeRests( timeT  time,
                                         timeT &duration,
                                         bool   testOnly ) {
  ...
  if( eventTime < finalTime ) {
    ...
    if( !testOnly ) {
      // can't safely change the absolute time of an event in a
      // segment
      Event *newEvent =
          new Event( **lastEvent, finalTime,
                     ( *lastEvent )->getDuration() -
                         ( finalTime - eventTime ) );
      duration =
          finalTime + ( *lastEvent )->getDuration() - time;
      bool same = ( from == to );
...
void SegmentNotationHelper::collapseRestsAggressively(
    timeT startTime, timeT endTime ) {
  reorganizeRests(
      startTime, endTime,
...
void SegmentNotationHelper::reorganizeRests(
    timeT startTime, timeT endTime, Reorganizer reorganizer ) {
  ...
  //    RG_DEBUG << "SegmentNotationHelper::reorganizeRests (" <<
  //    startTime << ","
  //         << endTime << ")";
...
void SegmentNotationHelper::normalizeContiguousRests(
  ...
  DurationList dl;
  ts.getDurationListForInterval( dl, duration,
                                 startTime - sigTime );

  timeT acc = startTime;

  for( DurationList::iterator i = dl.begin(); i != dl.end();
       ++i ) {
...
void SegmentNotationHelper::mergeContiguousRests(
    timeT startTime, timeT duration,
    std::vector<Event *> &toInsert ) {
  while( duration > 0 ) {
...
Segment::iterator
SegmentNotationHelper::collapseNoteAggressively(
    Event *note, timeT rangeEnd ) {
  iterator i = segment().findSingle( note );
  if( i == end() ) return end();
  ...
  if( j == end() ||
      ( *j )->getNotationAbsoluteTime() >= rangeEnd )
    return end();
  ...
  timeT iEnd = ( *i )->getNotationAbsoluteTime() +
               ( *i )->getNotationDuration();
  timeT jEnd = ( *j )->getNotationAbsoluteTime() +
               ( *j )->getNotationDuration();

  Event *newEvent =
      new Event( **i, ( *i )->getNotationAbsoluteTime(),
                 ( std::max( iEnd, jEnd ) -
                   ( *i )->getNotationAbsoluteTime() ) );
...
SegmentNotationHelper::splitPreservingPerformanceTimes(
    Event *e, timeT q1 ) {
  ...
  Event *e1 =
      new Event( *e, ut, u1, e->getSubOrdering(), qt, q1 );
  Event *e2 = new Event( *e, ut + u1, u2, e->getSubOrdering(),
                         qt + q1, qd - q1 );
...
void SegmentNotationHelper::deCounterpoint( timeT startTime,
                                            timeT endTime ) {
  // How this should work: scan through the range and, for each
  // note "n" found, if the next following note "m" not at the
  // same absolute time as n starts before n ends, then split n
  // at m-n.

  // also, if m starts at the same time as n but has a different
  // duration, we should split the longer of n and m at the
  // shorter one's duration.
  ...
  for( Segment::iterator i = segment().findTime( startTime );
    ...
    RG_DEBUG
        << "SegmentNotationHelper::deCounterpoint: event at "
        << ( *i )->getAbsoluteTime() << " notation "
        << ( *i )->getNotationAbsoluteTime() << ", duration "
        << ( *i )->getNotationDuration() << ", type "
        << ( *i )->getType();
#endif
    ...
    while( segment().isBeforeEndMarker( k ) ) {
      if( ( *k )->isa( Note::EventType ) ) {
        ...
        if( ( *k )->getNotationAbsoluteTime() > ti ||
            ( *k )->getNotationDuration() != di )
          break;
      }
      ++k;
    }

    if( !segment().isBeforeEndMarker( k ) )
      break; // no split, no more notes
    ...
    RG_DEBUG << "k is at "
             ...
             << ", duration " << ( *k )->getNotationDuration();
#endif
    ...
    if( tk == ti && dk != di ) {
      ...
      if( di > dk ) { // split *i
#ifdef DEBUG_DECOUNTERPOINT
        RG_DEBUG << "splitting i into " << dk << " and "
                 << ( di - dk );
#endif
...
#ifdef DEBUG_DECOUNTERPOINT
        RG_DEBUG << "splitting k into " << di << " and "
                 << ( dk - di );
#endif
...
#ifdef DEBUG_DECOUNTERPOINT
      RG_DEBUG << "splitting i[*] into " << ( tk - ti )
               << " and " << ( di - ( tk - ti ) );
#endif
...
void SegmentNotationHelper::autoSlur( timeT startTime,
                                      timeT endTime,
                                      bool  legatoOnly ) {
  ...
  for( iterator i = from;
    ...
    if( groupId >= 0 && count > 1 &&
        ( !legatoOnly || prevLegato ) ) {
...
int SegmentNotationHelper::findBorderTuplet( iterator  it,
                                             iterator &start,
                                             iterator &end ) {
  iterator beginB =
      segment().findTime( segment().getBarStartForTime(
          ( *it )->getAbsoluteTime() ) );
  iterator endB = segment().findTime(
      segment().getBarEndForTime( ( *it )->getAbsoluteTime() ) );
  int  maxcount  = 0;
  int  count     = 0;
  int  index     = 0;
  ...
  if( ( *beginB )->getType() == "clefchange" ) { ++beginB; }
  ...
  for( ; beginB != endB; ++beginB ) {
    ...
    if( ( *beginB )->has( BEAMED_GROUP_TUPLET_BASE ) == true ) {
      maxcount =
          ( *beginB )->get<Int>( BEAMED_GROUP_UNTUPLED_COUNT );
      if( ( *beginB )->getType() == "note" ) { count++; }
