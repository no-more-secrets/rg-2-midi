...
EventSelection::EventSelection( Segment &t, timeT beginTime,
                                timeT endTime, bool overlap )
  : m_originalSegment( t ),
    m_beginTime( 0 ),
    m_endTime( 0 ),
    m_haveRealStartTime( false ) {
  ...
  if( i != t.end() ) {
    ...
    while( i != j ) {
      m_endTime = ( *i )->getAbsoluteTime() +
                  ( *i )->getGreaterDuration();
  ...
  if( overlap ) {
    ...
    while( i != t.begin() && i != t.end() && i != j ) {
      if( ( *i )->getAbsoluteTime() +
              ( *i )->getGreaterDuration() >
          beginTime ) {
        m_segmentEvents.insert(
            *i ); // duplicates are filtered automatically
        m_beginTime = ( *i )->getAbsoluteTime();
      } else
        break;
...
bool EventSelection::operator==( const EventSelection &s ) {
  ...
  if( m_haveRealStartTime != s.m_haveRealStartTime )
    return false;
  if( m_segmentEvents != s.m_segmentEvents ) return false;
  return true;
}

void EventSelection::insertThisEvent( Event *e ) {
  ...
  if( eventEndTime > m_endTime ) { m_endTime = eventEndTime; }
...
void EventSelection::eraseThisEvent( Event *e ) {
  ...
  for( EventContainer::iterator it = interval.first;
    ...
    if( *it == e ) {
      ...
      // Exit, since we found one good match.  Don't worry about
      // others.
      break;
    }
  }
...
int EventSelection::addRemoveEvent( Event *      e,
                                    EventFuncPtr insertEraseFn,
                                    bool ties, bool forward ) {
  const Segment::const_iterator baseSegmentItr =
      m_originalSegment.find( e );
  ...
  // Always add/remove at least the one Event we were called
  // with.
  ...
  // Now we handle the tied notes themselves.  If the event we're
  // adding is tied, then we iterate forward and back to try to
  // find all of its linked neighbors, and treat them as though
  // they were one unit.  Musically, they ARE one unit, and
  // having selections treat them that way solves a lot of
  // usability problems.
  //
  ...
  if( e->has( BaseProperties::TIED_FORWARD ) &&
    ...
    long oldPitch = 0;
    if( e->has( BaseProperties::PITCH ) )
      e->get<Int>( BaseProperties::PITCH, oldPitch );

    // Set iterator to the next element in container after
    // baseSegmentItr;
    ...
    for( ; si != m_originalSegment.end(); ++si ) {
      ...
      if( ( *si )->getAbsoluteTime() > eventEndTime ) {
        // Break the loop.  There are no more events tied to the
        // original event in this direction
        break;
      }

      long newPitch = 0;
      if( ( *si )->has( BaseProperties::PITCH ) )
        ( *si )->get<Int>( BaseProperties::PITCH, newPitch );

      // forward from the target, find all notes that are tied
      // backwards, until hitting the end of the segment or the
      // first note at the same pitch that is not tied backwards.
      if( oldPitch == newPitch ) {
        if( ( *si )->has( BaseProperties::TIED_BACKWARD ) ) {
          ...
          eventEndTime =
              ( *si )->getAbsoluteTime() + eventDuration;
        }
      }
    }
  }
  ...
  if( e->has( BaseProperties::TIED_BACKWARD ) &&
      ( m_originalSegment.begin() !=
        m_originalSegment.end() ) ) {
    long oldPitch = 0;
    if( e->has( BaseProperties::PITCH ) )
      e->get<Int>( BaseProperties::PITCH, oldPitch );
    ...
    for( Segment::const_iterator si = baseSegmentItr;
      ...
      // First step moves iterator to element prior to
      // baseSegmentItr
      --si;
      if( !( *si )->isa( Note::EventType ) ) continue;

      if( ( ( *si )->getAbsoluteTime() +
            ( *si )->getDuration() ) < eventStartTime ) {
        // Break the loop.  There are no more events tied to the
        // original event in this direction
        break;
      }
      ...
      // back from the target, find all notes that are tied
      // forward, until hitting the end of the segment or the
      // first note at the same pitch that is not tied forward.
      if( oldPitch == newPitch ) {
...
void EventSelection::removeObserver(
    EventSelectionObserver *obs ) {
  m_observers.remove( obs );
}

int EventSelection::addEvent( Event *e, bool ties,
                              bool forward ) {
  return addRemoveEvent( e, &EventSelection::insertThisEvent,
                         ties, forward );
}
...
int EventSelection::removeEvent( Event *e, bool ties,
                                 bool forward ) {
  return addRemoveEvent( e, &EventSelection::eraseThisEvent,
                         ties, forward );
}

bool EventSelection::contains( Event *e ) const {
  std::pair<EventContainer::const_iterator,
...
bool EventSelection::contains( const std::string &type ) const {
  for( EventContainer::const_iterator i =
           m_segmentEvents.begin();
...
timeT EventSelection::getNotationStartTime() const {
  timeT start = 0;
  bool  first = true;
  // inefficient, but the simplest way to be sure (since events
  // are not ordered in notation time)
  for( EventContainer::const_iterator i =
           m_segmentEvents.begin();
       i != m_segmentEvents.end(); ++i ) {
...
timeT EventSelection::getNotationEndTime() const {
  timeT end   = 0;
  bool  first = true;
  // inefficient, but the simplest way to be sure (since events
  // are not ordered in notation time)
...
timeT EventSelection::getTotalNotationDuration() const {
  timeT start = 0, end = 0;
  bool  first = true;
  // inefficient, but the simplest way to be sure (since events
  // are not ordered in notation time)
...
EventSelection::RangeList EventSelection::getRanges() const {
  RangeList ranges;

  Segment::iterator i =
      m_originalSegment.findTime( getStartTime() );
  Segment::iterator j = i;
  Segment::iterator k =
      m_originalSegment.findTime( getEndTime() );
...
EventSelection::RangeTimeList EventSelection::getRangeTimes()
    const {
  RangeList     ranges( getRanges() );
  RangeTimeList rangeTimes;

  for( RangeList::iterator i = ranges.begin(); i != ranges.end();
       ++i ) {
    ...
    rangeTimes.push_back(
        RangeTimeList::value_type( startTime, endTime ) );
  }

  return rangeTimes;
}

void EventSelection::eventRemoved( const Segment *s, Event *e ) {
  // contains() checked a bit deeper now.
  if( s == &m_originalSegment ) { removeEvent( e ); }
}

void EventSelection::segmentDeleted( const Segment * ) {
  /*
  RG_DEBUG << "WARNING: EventSelection notified of segment
  deletion: this is probably a bug "
        << "(selection should have been deleted before segment)";
            */
}

void EventSelection::dump() const {}

/** Templates that define methods of TimewiseSelection **/

template<typename ElementInfo>
void TimewiseSelection<ElementInfo>::RemoveFromComposition(
    Composition *composition ) {
  for( typename Container::const_iterator i = begin();
       i != end(); ++i ) {
...
template<typename ElementInfo>
void TimewiseSelection<ElementInfo>::AddToComposition(
    Composition *composition ) {
  for( typename Container::const_iterator i = begin();
       i != end(); ++i ) {
...
TimeSignatureSelection::TimeSignatureSelection(
    Composition &composition, timeT beginTime, timeT endTime,
...
void TimeSignatureSelection::addTimeSignature(
    timeT t, TimeSignature timeSig ) {
  m_timeSignatures.insert(
      timesigcontainer::value_type( t, timeSig ) );
}
void TimeSignatureSelection::RemoveFromComposition(
    Composition *composition ) {
  for( TimeSignatureSelection::timesigcontainer::const_iterator
           i = begin();
       i != end(); ++i ) {
    int n = composition->getTimeSignatureNumberAt( i->first );
    if( n >= 0 ) { composition->removeTimeSignature( n ); }
  }
}
void TimeSignatureSelection::AddToComposition(
    Composition *composition ) {
  for( TimeSignatureSelection::timesigcontainer::const_iterator
           i = begin();
       i != end(); ++i ) {
...
TempoSelection::TempoSelection( Composition &composition,
  ...
  for( int i = composition.getTempoChangeNumberAt( beginTime );
    ...
    std::pair<timeT, tempoT> change =
        composition.getTempoChange( i );
...
void TempoSelection::addTempo( timeT t, tempoT tempo,
                               tempoT targetTempo ) {
  m_tempos.insert( tempocontainer::value_type(
      t, tempochange( tempo, targetTempo ) ) );
}

void TempoSelection::RemoveFromComposition(
    Composition *composition ) {
  for( TempoSelection::tempocontainer::const_iterator i =
           begin();
       i != end(); ++i ) {
    int n = composition->getTempoChangeNumberAt( i->first );
    if( n >= 0 ) { composition->removeTempoChange( n ); }
  }
}

void TempoSelection::AddToComposition(
    Composition *composition ) {
  for( TempoSelection::tempocontainer::const_iterator i =
           begin();
       i != end(); ++i ) {
    composition->addTempoAtTime( i->first, i->second.first,
                                 i->second.second );
  }
}
...
void MarkerElementInfo::RemoveFromComposition(
    Composition *composition, const value_type &element ) {
  composition->detachMarker( element );
}

void MarkerElementInfo::AddToComposition(
    Composition *composition, const value_type &element ) {
  composition->addMarker( element );
}

// Explicit template instantiation, so other cpp files don't need
// to see the method definitions.
template class TimewiseSelection<MarkerElementInfo>;

/** Methods of MarkerSelection **/

MarkerSelection::MarkerSelection( Composition &composition,
                                  timeT        beginTime,
                                  timeT        endTime ) {
