...
SequencerDataBlock *SequencerDataBlock::getInstance() {
  static SequencerDataBlock *instance = nullptr;
  if( !instance ) instance = new SequencerDataBlock();
  return instance;
}

SequencerDataBlock::SequencerDataBlock() { clearTemporaries(); }

bool SequencerDataBlock::getVisual( MappedEvent &ev ) {
  // If there is no visual event, or setVisual() is working, bail
  if( !m_haveVisualEvent ) return false;
  ...
  if( thisEventIndex == m_getVisualIndex ) return false;

  // ??? A call to setVisual() could happen at this point and
  // modify
  //     m_visualEvent while we are reading it.  This isn't as
  //     safe as it appears.  I think we need a mutex for this.
  //     Though since it is just the MIDI OUT display on
  //     playback, it's probably not worth worrying about.
...
void SequencerDataBlock::setVisual( const MappedEvent *ev ) {
...
int SequencerDataBlock::getRecordedEvents(
    MappedEventList &mC ) {
  ...
  // While there are events in the record buffer, copy each event
  // to the user's list.
...
void SequencerDataBlock::addRecordedEvents(
    MappedEventList *mC ) {
  ...
  // Once the buffer is in a consistent state, move the record
  // index so that the other thread will read the new events.
  // ??? Is this guaranteed to be atomic and therefore thread
  // safe?
  //     I believe so, and that's why this has always worked.
  m_recordEventIndex = index;
}

int SequencerDataBlock::instrumentToIndex(
...
int SequencerDataBlock::instrumentToIndexCreating(
    InstrumentId id ) {
  int i;
...
bool SequencerDataBlock::getInstrumentLevel(
    InstrumentId id, LevelInfo &info ) const {
  // ??? Move statics to class scope non-static and clear them
  // appropriately
  //     in clearTemporaries().
  static int
      lastUpdateIndex[SEQUENCER_DATABLOCK_MAX_NB_INSTRUMENTS];
...
bool SequencerDataBlock::getInstrumentLevelForMixer(
    InstrumentId id, LevelInfo &info ) const {
  // ??? Move statics to class scope non-static and clear them
  // appropriately
  //     in clearTemporaries().
...
void SequencerDataBlock::setInstrumentLevel(
    InstrumentId id, const LevelInfo &info ) {
...
bool SequencerDataBlock::getInstrumentRecordLevel(
    InstrumentId id, LevelInfo &info ) const {
  // ??? Move statics to class scope non-static and clear them
  // appropriately
  //     in clearTemporaries().
...
bool SequencerDataBlock::getInstrumentRecordLevelForMixer(
    InstrumentId id, LevelInfo &info ) const {
  // ??? Move statics to class scope non-static and clear them
  // appropriately
  //     in clearTemporaries().
...
void SequencerDataBlock::setInstrumentRecordLevel(
    InstrumentId id, const LevelInfo &info ) {
  int index = instrumentToIndexCreating( id );
  if( index < 0 ) return;
...
void SequencerDataBlock::setTrackLevel( TrackId          id,
                                        const LevelInfo &info ) {
  setInstrumentLevel(
      ControlBlock::getInstance()->getInstrumentForTrack( id ),
      info );
}

bool SequencerDataBlock::getTrackLevel( TrackId    id,
...
bool SequencerDataBlock::getSubmasterLevel(
    int submaster, LevelInfo &info ) const {
  // ??? Move statics to class scope non-static and clear them
  // appropriately
  //     in clearTemporaries().
  static int
      lastUpdateIndex[SEQUENCER_DATABLOCK_MAX_NB_SUBMASTERS];
...
void SequencerDataBlock::setSubmasterLevel(
    int submaster, const LevelInfo &info ) {
  if( submaster < 0 ||
      submaster >= SEQUENCER_DATABLOCK_MAX_NB_SUBMASTERS ) {
    return;
  }
...
bool SequencerDataBlock::getMasterLevel(
...
void SequencerDataBlock::setMasterLevel(
    const LevelInfo &info ) {
  m_masterLevel = info;
  ++m_masterLevelUpdateIndex;
}

void SequencerDataBlock::clearTemporaries() {
  m_positionSec  = 0;
  m_positionNsec = 0;
  ...
  *( (MappedEvent *)&m_visualEvent ) = MappedEvent();
  ...
  memset( m_levelUpdateIndices, 0,
          sizeof( m_levelUpdateIndices ) );
  memset( m_levels, 0, sizeof( m_levels ) );

  memset( m_recordLevelUpdateIndices, 0,
          sizeof( m_recordLevelUpdateIndices ) );
