...
ControlList SoftSynthDevice::m_controlList;
...
SoftSynthDevice::SoftSynthDevice( DeviceId           id,
...
SoftSynthDevice::SoftSynthDevice( const SoftSynthDevice &dev )
  ...
  if( dev.m_metronome )
    m_metronome = new MidiMetronome( *dev.m_metronome );
}

SoftSynthDevice::~SoftSynthDevice() { delete m_metronome; }

void SoftSynthDevice::createInstruments() {
  for( uint i = 0; i < SoftSynthInstrumentCount; ++i ) {
    Instrument *instrument =
        new Instrument( SoftSynthInstrumentBase + i,
                        Instrument::SoftSynth, "", i, this );
    addInstrument( instrument );
  }
  renameInstruments();
}

void SoftSynthDevice::renameInstruments() {
  for( uint i = 0; i < SoftSynthInstrumentCount; ++i ) {
    m_instruments[i]->setName( getName() + " #" +
                               std::to_string( i + 1 ) );
  }
}

void SoftSynthDevice::checkControlList() {
...
const ControlParameter *SoftSynthDevice::getControlParameter(
    int index ) const {
...
void SoftSynthDevice::setMetronome(
    const MidiMetronome &metronome ) {
  delete m_metronome;
  m_metronome = new MidiMetronome( metronome );
}

std::string SoftSynthDevice::toXmlString() const {
  std::stringstream              ssiDevice;
  InstrumentList::const_iterator iit;

  ssiDevice << "    <device id=\"" << m_id << "\" name=\""
            << m_name << "\" type=\"softsynth\">" << std::endl;
...
//
void SoftSynthDevice::addInstrument( Instrument *instrument ) {
