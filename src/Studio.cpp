...
Studio::Studio()
  ...
  // And we always have one audio and one soft-synth device,
  // whose IDs match the base instrument numbers (for no good
  // reason except easy identifiability)
...
Studio::~Studio() {
  ...
  for( ; dIt != m_devices.end(); ++dIt ) delete( *dIt );
...
void Studio::addDevice( const std::string &name, DeviceId id,
  ...
  switch( type ) {
    case Device::Midi:
      d = new MidiDevice( id, baseInstrumentId, name,
                          MidiDevice::Play );
      break;

      // case Device::Audio: d = new AudioDevice( id, name );
      // break;
...
void Studio::removeDevice( DeviceId id ) {
...
DeviceId Studio::getSpareDeviceId(
  ...
  if( !foundInstrument ) {
    ...
    baseInstrumentId =
        ( ( highestMidiInstrumentId / 128 ) + 1 ) * 128;
  }
...
InstrumentList Studio::getAllInstruments() {
...
InstrumentList Studio::getPresentationInstruments() const {
  InstrumentList list;
  ...
  for( DeviceList::const_iterator it = m_devices.begin();
    ...
    // get sub list
    InstrumentList subList =
        ( *it )->getPresentationInstruments();
...
Instrument *Studio::getInstrumentById( InstrumentId id ) {
...
//
Instrument *Studio::getInstrumentFromList( int index ) {
  std::vector<Device *>::iterator it;
  InstrumentList                  list;
  InstrumentList::iterator        iit;
  int                             count = 0;
...
Instrument *Studio::getInstrumentFor( Segment *segment ) {
...
Instrument *Studio::getInstrumentFor( Track *track ) {
  if( !track ) return nullptr;
  InstrumentId iid = track->getInstrument();
  return getInstrumentById( iid );
}

BussList Studio::getBusses() { return m_busses; }

Buss *Studio::getBussById( BussId id ) {
  for( BussList::iterator i = m_busses.begin();
       i != m_busses.end(); ++i ) {
...
void Studio::setBussCount( unsigned newBussCount ) {
  // We have to have at least one for the master.
  if( newBussCount < 1 ) return;
  // Reasonable limit.  Adjust if needed.
  if( newBussCount > 16 ) return;
  // No change?  Bail.
  if( newBussCount == m_busses.size() ) return;
...
// PluginContainer *Studio::getContainerById( InstrumentId id ) {
//  PluginContainer *pc = getInstrumentById( id );
//  if( pc )
//    return pc;
//  else
//    return getBussById( id );
//}

RecordIn *Studio::getRecordIn( int number ) {
...
void Studio::setRecordInCount( unsigned newRecordInCount ) {
  // Can't have zero.
  if( newRecordInCount < 1 ) return;
  if( newRecordInCount > 32 ) return;
  // No change?  Bail.
  if( newRecordInCount == m_recordIns.size() ) return;
...
std::string Studio::toXmlString() const {
  return toXmlString( std::vector<DeviceId>() );
}

std::string Studio::toXmlString(
  ...
  studio << "<studio thrufilter=\"" << m_midiThruFilter
         ...
         << "\" amwshowaudiosubmasters=\""
         << amwShowAudioSubmasters
         << "\" amwshowunassignedfaders=\""
         << amwShowUnassignedFaders << "\">" << endl
         << endl;
  ...
  if( devices.empty() ) { // export all devices and busses
    ...
    for( std::vector<DeviceId>::const_iterator di(
             devices.begin() );
         di != devices.end(); ++di ) {
...
//
const MidiMetronome *Studio::getMetronomeFromDevice(
    DeviceId id ) {
  std::vector<Device *>::iterator it;

  for( it = m_devices.begin(); it != m_devices.end(); ++it ) {
    MidiDevice *midiDevice = dynamic_cast<MidiDevice *>( *it );
    if( midiDevice && midiDevice->getId() == id &&
        midiDevice->getMetronome() ) {
      return midiDevice->getMetronome();
    }

    SoftSynthDevice *ssDevice =
        dynamic_cast<SoftSynthDevice *>( *it );
    if( ssDevice && ssDevice->getId() == id &&
...
Instrument *Studio::assignMidiProgramToInstrument(
    MidiByte program, int msb, int lsb, bool percussion ) {
  ...
  for( it = m_devices.begin(); it != m_devices.end(); ++it ) {
    ...
    if( midiDevice &&
        midiDevice->getDirection() == MidiDevice::Play ) {
      instList = ( *it )->getPresentationInstruments();

      for( iit = instList.begin(); iit != instList.end();
           ++iit ) {
        if( firstInstrument == nullptr ) firstInstrument = *iit;

        // If we find an Instrument sending the right program
        // already.
        //
  ...
  // Okay, if we've got this far and we have a new Instrument to
  // use then use it.
  //
  if( newInstrument != nullptr ) {
...
//
//
void Studio::unassignAllInstruments() {
  // MidiDevice *                         midiDevice;
  ...
  //        ( *iit )->setFixedChannel();
  //        // ??? This is a "reset" of the instrument.  It
  //        doesn't
...
void Studio::clearMidiBanksAndPrograms() {
  MidiDevice *                    midiDevice;
  std::vector<Device *>::iterator it;
...
Device *Studio::getDevice( DeviceId id ) const {
...
Device *Studio::getSoftSynthDevice() {
  std::vector<Device *>::iterator it;
...
std::string Studio::getSegmentName( InstrumentId id ) {
  MidiDevice *                         midiDevice;
  std::vector<Device *>::iterator      it;
  Rosegarden::InstrumentList::iterator iit;
  Rosegarden::InstrumentList           instList;
...
InstrumentId Studio::getAudioPreviewInstrument() {
  // AudioDevice *                   audioDevice;
  // std::vector<Device *>::iterator it;
...
bool Studio::haveMidiDevices() const {
