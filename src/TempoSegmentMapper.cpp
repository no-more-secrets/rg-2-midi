...
void TempoSegmentMapper::fillBuffer() {
  ...
  for( int i = 0; i < comp.getTempoChangeCount(); ++i ) {
    ...
    std::pair<bool, tempoT> rampTo =
        comp.getTempoRamping( i, false );

    RealTime eventTime =
        comp.getElapsedRealTime( tempoChange.first );
  ...
  if( !wroteInitialTempo ) { mapTempoAtZero( comp ); }
}

void TempoSegmentMapper::mapTempoAtZero( Composition& comp ) {
  int  number = comp.getTempoChangeNumberAt( 0 );
  bool ramping;
  if( number >= 0 ) {
    std::pair<bool, tempoT> rampTo =
        comp.getTempoRamping( number, false );
...
void TempoSegmentMapper::mapATempo( RealTime eventTime,
                                    tempoT   tempo,
                                    bool     ramping ) {
  MappedEvent e;
  ...
  // data1 holds whether we are ramping.  This is only used by
  // the MIDI exporter and it doesn't need to know where we're
  // ramping to.
  e.setData1( ramping ? 1 : 0 );
#ifdef DEBUG_TEMPO_SEGMENT_MAPPER
  SEQUENCER_DEBUG
      << "TempoSegmentMapper::mapATempo inserting tempo" << tempo
      << "at" << eventTime << endl;
#endif
...
int TempoSegmentMapper::calculateSize() {
  return m_doc->getComposition().getTempoChangeCount() + 1;
}

// Tempo changes always "play"
bool TempoSegmentMapper::shouldPlay( MappedEvent* /*evt*/,
                                     RealTime /*startTime*/ ) {
  return true;
}
